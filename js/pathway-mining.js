(() => {
  const root = document.querySelector('[data-mining-pathway]');
  ...
  async function assignmentApi(method = 'GET', body = null) {
    ...
    if (body) {
      ...
      options.body = JSON.stringify({ ...body, activity:'mining' });
    }
    const response = await fetch(`/api/pathway/assignments?activity=mining&_=${Date.now()}`, options);
  ...
  function render() {
    if (!assignments?.eligible) {
      content.innerHTML = '<div class="pathway-empty"><strong>Mining is not in your saved pathway yet.</strong><br>Select Mining above and save your pathway to generate a Mining assignment chain.</div>';
    ...
    content.innerHTML = `
      <article class="ax-route-card">
        <div class="ax-route-head">
          <div><span class="ax-route-kicker">Your Mining development route</span><h3>${esc(route.title)}</h3><p>${esc(route.subtitle)}</p></div>
          <div class="pathway-recommendation-badges"><span class="pathway-badge">${esc(route.band || experienceLabels[assignments.experience] || 'Mining')}</span><span class="pathway-badge is-improve">Mining Route</span></div>
        </div>
        ...
        <div class="ax-route-actions">${assignments.canChooseAnother ? '<button class="btn btn-ghost" type="button" data-mining-another-route>Give Me Another Route</button>' : ''}<button class="ax-text-button" type="button" data-mining-reset-route>Reset this route</button></div>
      </article>

      ${allDone ? `
        <article class="ax-current-assignment is-graduate">
          <span class="ax-assignment-stage">Route Complete</span>
          <h3>Mining route complete</h3>
          <p>${esc(route.outcome)}</p>
          <a class="btn btn-primary" href="../guides/mining/">Open Mining Field Manual</a>
        </article>` : `
        <article class="ax-current-assignment">
          ...
          <div class="ax-current-copy">
            <span class="ax-next-label">Your Next Mining Assignment</span>
            ...
            <div class="ax-assignment-actions">
              ...
              <button class="btn btn-ghost" type="button" data-mining-task-status="skipped" data-mining-task-id="${esc(current.id)}">Skip for Now</button>
              ${taskLink(current)}
            </div>
          </div>
        </article>`}
      ...
      <details class="ax-assignment-list">
        ...
        <div class="ax-assignment-list-body">
          ${route.tasks.map(task => `<article class="ax-list-task${task.id === assignments.currentTaskId ? ' is-current' : ''}">
            ...
            <div class="ax-list-status status-${esc(task.status)}"><span>${esc(taskStatusLabels[task.status] || task.status)}</span>${task.status !== 'pending' ? `<button type="button" data-mining-task-status="pending" data-mining-task-id="${esc(task.id)}">Reopen</button>` : ''}</div>
          </article>`).join('')}
        </div>
      </details>
      ...
      <div class="ax-action-status" data-mining-action-status></div>`;

    content.querySelector('[data-mining-another-route]')?.addEventListener('click', () => runAction({ action:'another_route' }, 'Picking another Mining route…'));
    content.querySelector('[data-mining-reset-route]')?.addEventListener('click', () => {
      if (!window.confirm(`Reset all progress on ${route.title}?\n\nThis only resets this Mining route. Your pathway preferences are unchanged.`)) return;
      runAction({ action:'reset_route' }, 'Resetting this Mining route…');
    });
    content.querySelectorAll('[data-mining-task-status]').forEach(button => button.addEventListener('click', () => {
      const status = button.dataset.miningTaskStatus;
  ...
  async function runAction(body, workingText) {
    if (busy) return;
    busy = true;
    const status = content.querySelector('[data-mining-action-status]');
    ...
    try {
      ...
      render();
      const nextStatus = content.querySelector('[data-mining-action-status]');
      if (nextStatus) { nextStatus.textContent = 'Progress saved.'; nextStatus.dataset.state = 'success'; }
    } catch (error) {
      console.error('Could not update Mining pathway assignment', error);
      if (status) { status.textContent = error?.message || 'Could not update the Mining assignment.'; status.dataset.state = 'error'; }
  ...
  async function load() {
    try {
      ...
      root.hidden = !savedSelected.has('mining');
      if (!savedSelected.has('mining')) {
        assignments = null;
        content.innerHTML = '';
        return;
      }
      if (loading) { loading.hidden = false; loading.textContent = 'Building your Mining assignment…'; }
      assignments = await assignmentApi();
      render();
    } catch (error) {
      root.hidden = true;
      console.error('Could not load Mining pathway assignments', error);
    }
  }
