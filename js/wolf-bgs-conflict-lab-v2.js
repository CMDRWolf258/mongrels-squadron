(() => {
  ...
  if(!Array.isArray(s.unknownHistory))s.unknownHistory=[];
  s.dayKnown=s.dayKnown!==false;
  ...
  function resolvedState(score,completed,dayKnown=true){
    ...
    if(!dayKnown)return{resolved:false,side:'',reason:''};
  ...
  function pressureFromObservedHistory(objective,history=s.manualHistory,label='Manual',countRepeated=false){
    ...
    if(objective==='tie'){
      ...
      let level='contested',quiet=0,heavyLock=false;
      let reason=leader?'First '+label.toLowerCase()+' unequal score starts Tie recovery at Contested.':label+' score is tied; no corrective work is required.';
      for(let i=1;i<history.length;i++){
        const before=history[i-1],after=history[i];
        const newLeader=after.a===after.b?'':(after.a>after.b?'a':'b');
        if(!newLeader){leader='';level='contested';quiet=0;heavyLock=false;reason=label+' score returned to a tie; corrective work stops.';continue;}
        if(!leader||newLeader!==leader){leader=newLeader;level='contested';quiet=0;heavyLock=false;reason=label+' score shows a new leader; Tie recovery restarts Contested.';continue;}
    ...
    let reason=first.a===0&&first.b===0?'Opening '+label.toLowerCase()+' 0–0 starts Routine.'
      :first[desired]>first[opponent]?'First '+label.toLowerCase()+' score has the intended winner ahead, so automation starts Routine.'
      :'First '+label.toLowerCase()+' score starts Contested; prior opposition is not reconstructed.';
    for(let i=1;i<history.length;i++){
      const before=history[i-1],after=history[i];
  ...
  function context(){
    if(s.source==='missing'){
      ...
      if(!latest)return{source:'missing',score:null,completed:null,dayKnown:s.dayKnown,pressure:null,stale:false,manual:true};
      const p=pressureFromObservedHistory(s.objective,s.manualHistory,'Manual');
      return{source:'manual',score:{a:latest.a,b:latest.b},completed:null,dayKnown:s.dayKnown,pressure:p,stale:false,manual:true};
    }
    if(!s.dayKnown){
    ...
    return{source:s.source,score,completed,dayKnown:true,pressure:p,stale:s.source==='stale',manual:false};
  }
  ...
  function recommendation(){
    ...
    const completed=C.completed===null&&C.dayKnown!==false?s.throughDay:C.completed;
    const end=resolvedState(C.score,completed,C.dayKnown!==false);
    if(end.resolved){
    ...
    return{
      ...
      detail:(C.dayKnown===false?'DAY UNKNOWN':C.completed===null?'Manual score':'Start of Day '+(completed+1))+' · '+p.a+' '+C.score.a+'–'+C.score.b+' '+p.b+'. '+why,
  ...
  function sourceLabel(C){
    if(C.source==='fresh')return['AUTHORITATIVE SCORE',C.dayKnown===false?'Conflict day unknown':'Fresh simulated source'];
    if(C.source==='stale')return['SCORE STALE',C.dayKnown===false?'Conflict day unknown · automation frozen':'Automation frozen at the last fresh observation'];
  ...
  function currentDayOptions(){
    return '<option value="unknown" '+(!s.dayKnown?'selected':'')+'>Unknown</option>'+dayOptions(s.throughDay);
  }
  ...
  function unknownScoreMarkup(pairInfo){
    if(s.dayKnown||s.source==='missing')return '';
    const latest=s.unknownHistory[s.unknownHistory.length-1];
    const frozen=s.source==='stale';
    return '<div class="wolf-unknown-day-score">'+
      '<div class="wolf-unknown-day-head"><div><b>DAY UNKNOWN · AUTHORITATIVE SCORE KNOWN</b><small>Only score changes observed from this point forward affect pressure. No prior days, quiet streaks, or remaining-day count are inferred.</small></div></div>'+
      '<div class="wolf-manual-score">'+
        '<label><span>'+esc(pairInfo?.a||'Faction A')+' wins</span><input type="number" min="0" max="4" step="1" data-unknown-score="a" value="'+esc(s.unknownDraftA)+'" '+(frozen?'disabled':'')+'></label>'+
        '<span class="wolf-score-dash">–</span>'+
        '<label><span>'+esc(pairInfo?.b||'Faction B')+' wins</span><input type="number" min="0" max="4" step="1" data-unknown-score="b" value="'+esc(s.unknownDraftB)+'" '+(frozen?'disabled':'')+'></label>'+
        '<button type="button" class="btn btn-primary btn-compact" data-save-unknown-score '+(frozen?'disabled':'')+'>SAVE OBSERVED SCORE</button>'+
      '</div>'+
      '<small>'+(frozen?'Score is stale. Switch the source back to Fresh authoritative before adding another observed score.':latest?'Baseline / latest observation: '+latest.a+'–'+latest.b+' · '+s.unknownHistory.length+' observed score'+(s.unknownHistory.length===1?'':'s'):'Enter the known score to establish the observation baseline.')+'</small>'+
    '</div>';
  }
  ...
  function render(){
    ...
    const dayUnknown=C.dayKnown===false?'<b class="wolf-day-unknown">DAY UNKNOWN</b>':'';
    ...
    h.innerHTML=
      ...
      '<div class="wolf-conflict-command-strip">'+
        '<div class="wolf-command-score"><span>'+esc(p?.type==='election'?'ELECTION':p?.type==='civil-war'?'CIVIL WAR':'WAR')+'</span><strong>'+esc(p?.a||'Faction A')+' <b>'+score+'</b> '+esc(p?.b||'Faction B')+'</strong>'+dayUnknown+'<small>'+esc(src[0])+' · '+esc(src[1])+'</small></div>'+
      ...
      '<details class="wolf-conflict-sandbox-controls" open><summary><span><b>Sandbox controls</b><small>Simulate score history, late discovery, stale data, and manual fallback</small></span><strong>+</strong></summary>'+
        '<div class="wolf-conflict-v2-settings">'+
          ...
          '<label><span>Current conflict day</span><select data-v2="throughDay">'+currentDayOptions()+'</select></label>'+
          '<label><span>First observed score</span><select data-v2="observeFrom" '+(!s.dayKnown?'disabled':'')+'>'+dayOptions(s.observeFrom)+'</select></label>'+
        '</div>'+
        '<div class="wolf-sandbox-help"><span>'+(s.dayKnown?'<b>First observed score</b> is the moment automation began watching. A conflict first discovered at 0–3 starts Contested; prior opposition is never invented.':'<b>Day Unknown</b> trusts the score but does not infer conflict day, remaining days, prior opposition, or quiet days. Save later observed scores below to test pressure changes.')+'</span><button type="button" class="btn btn-secondary btn-compact" data-reset-operational-history>RESET CONFLICT TEST</button></div>'+
        (s.dayKnown?'<div class="wolf-conflict-day-grid">'+dayGrid()+'</div>':unknownScoreMarkup(p))+
      '</details>'+
    ...
    h.querySelector('[data-v2="throughDay"]').value=s.dayKnown?String(s.throughDay):'unknown';
  ...
  function saveManualScore(){
    ...
    const end=resolvedState({a,b},s.dayKnown?s.throughDay:null,s.dayKnown);
    if(end.resolved)s.blitz=false;
    save();render();
  }

  function saveUnknownScore(){
  ...
  document.addEventListener('change',e=>{
    ...
    if(e.target.matches('[data-v2]')){
      ...
      if(key==='throughDay'){
        ...
        if(!s.dayKnown&&previousObjective!==s.objective&&s.unknownHistory.length){
          const last=s.unknownHistory[s.unknownHistory.length-1];
          s.unknownHistory=[{...last,objective:s.objective,at:new Date().toISOString()}];
        }
        syncPairObjective();
      }
      const C=context();
      if(C.score){
        const completed=C.completed===null&&C.dayKnown!==false?s.throughDay:C.completed;
        if(resolvedState(C.score,completed,C.dayKnown!==false).resolved)s.blitz=false;
      }
      save();render();
      return;
    }
    ...
    if(e.target.matches('[data-unknown-score]')){
      const key=e.target.dataset.unknownScore==='a'?'unknownDraftA':'unknownDraftB';
      s[key]=clamp(e.target.value,0,4);save();return;
    }
  ...
  document.addEventListener('click',e=>{
    ...
    if(e.target.closest('[data-save-unknown-score]')){saveUnknownScore();return;}
