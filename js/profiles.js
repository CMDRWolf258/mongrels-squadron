(() => {
  ...
  function renderRoster(){
    ...
    activity.innerHTML='<option value="">All pursuits</option>'+acts.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('');
  ...
  function pursuitsField(selected){
    const chips=list(selected).map(x=>`<span class="profile-chip activity-chip">${esc(x)}</span>`).join('');
    return `<fieldset class="profile-tag-picker"><legend>Mongrel Pursuits</legend><p class="muted">Pursuits are managed separately so the website and Discord stay synchronized.</p><div class="profile-chip-row">${chips||'<span class="muted">No pursuits selected yet.</span>'}</div><div style="margin-top:12px"><a class="btn btn-ghost" href="../pursuits/">Manage Mongrel Pursuits</a></div></fieldset>`;
  }
