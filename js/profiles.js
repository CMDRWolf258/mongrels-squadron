(() => {
  ...
  function renderRosterGrid(){
    ...
    profiles=profiles.filter(p=>{
      ...
      const groupOK=state.group==='all'||(state.group==='leadership'&&p.leadershipRole)||(state.group==='carriers'&&Number(p.contributions?.carriers)>0)||(state.group==='help'&&['Available to Help','Looking for Group'].includes(p.availabilityStatus));
  ...
  function profileDetail(p){
    ...
    const carriers=list(p.registeredCarriers).map(x=>`<article class="profile-mini-card"><strong>${esc(x.name)}</strong><span>${esc(x.callsign)}${x.role?` · ${esc(x.role)}`:''}</span>${x.currentSystem?`<small>Current system · ${esc(x.currentSystem)}</small>`:''}<a href="../carriers/?member=${member}">Open Carrier Registry →</a></article>`).join('');
