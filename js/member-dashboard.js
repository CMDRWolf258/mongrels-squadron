(() => {
  ...
  const renderOrders = payload => {
    ...
    const active = orders.filter(order => !['complete','completed'].includes(String(order.status || '').toLowerCase()));
    ...
    if (summary) summary.textContent = payload?.configured
    ...
    visible.slice(0, 3).forEach(order => {
      ...
      const meta = document.createElement('span'); meta.textContent = [order.priority, order.system].filter(Boolean).join(' · ') || 'Operational Task';
      const task = document.createElement('strong'); task.textContent = order.task || 'Operational task';
      row.append(meta, task); preview.appendChild(row);
    });
    if (visible.length > 3) { const more = document.createElement('small'); more.textContent = `+${visible.length - 3} more task${visible.length - 3 === 1 ? '' : 's'}`; preview.appendChild(more); }
  };
  ...
  const projectScore = item => {
    let score = 0;
    if (item.official) score += 1000;
    if (item.kind === 'event' && item.deadline) {
      const days = (new Date(`${item.deadline}T12:00:00`).getTime() - Date.now()) / 86400000;
      if (days >= -1 && days <= 30) score += 800 - Math.max(0, days) * 10;
    }
    if (item.status === 'active') score += 300;
    if (item.isMine) score += 120;
    score += Math.min(100, Number(item.progress) || 0);
    score += Math.min(100, Math.floor((Date.parse(item.updatedAt || '') || 0) / 1e11));
    return score;
  };
  ...
  const renderProjects = payload => {
    ...
    if (projectSummary) projectSummary.textContent = active.length
      ? 'The most relevant active projects, help requests, and upcoming events are shown below.'
      : 'No active member projects or squad events are posted right now.';
    if (!projectPreview) return;
    projectPreview.replaceChildren();
    active.sort((a,b) => projectScore(b) - projectScore(a)).slice(0,3).forEach(item => {
      const row = document.createElement('div'); row.className = 'member-order-preview-row';
      const meta = document.createElement('span');
      meta.textContent = [item.kind === 'event' ? 'Event' : (item.official ? 'Squad Project' : item.category), item.system, item.deadline ? formatDate(item.deadline) : ''].filter(Boolean).join(' · ');
      const title = document.createElement('strong'); title.textContent = item.title;
      row.append(meta, title); projectPreview.appendChild(row);
    });
  };
  ...
  const init = async () => {
    try {
      ...
      console.error('Could not load member dashboard', error); renderUnavailable();
    }
  };
