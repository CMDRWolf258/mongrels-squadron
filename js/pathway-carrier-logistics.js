(() => {
  const root = document.querySelector('[data-carrier-logistics-pathway]');
  const loading = document.querySelector('[data-carrier-logistics-loading]');
  const content = document.querySelector('[data-carrier-logistics-content]');
  ...
  async function assignmentApi(method = 'GET', body = null) {
    ...
    if (body) {
      ...
      options.body = JSON.stringify({ ...body, activity:'carrier-logistics' });
    }
    const response = await fetch(`/api/pathway/assignments?activity=carrier-logistics&_=${Date.now()}`, options);
  ...
  function render() {
    if (!assignments?.eligible) {
      content.innerHTML = '<div class="pathway-empty"><strong>Carrier Logistics is not in your saved pathway yet.</strong><br>Select Carrier Logistics above and save your pathway to generate a carrier assignment chain.</div>';
    ...
    content.innerHTML = `
      <article class="ax-route-card">
        <div class="ax-route-head">
          <div><span class="ax-route-kicker">Your Carrier Logistics development route</span><h3>${esc(route.title)}</h3><p>${esc(route.subtitle)}</p></div>
          <div class="pathway-recommendation-badges"><span class="pathway-badge">${esc(route.band || experienceLabels[assignments.experience] || 'Carrier Logistics')}</span><span class="pathway-badge is-improve">Carrier Route</span></div>
        </div>
        ...
        <div class="ax-route-actions">${assignments.canChooseAnother ? '<button class="btn btn-ghost" type="button" data-carrier-logistics-another-route>Give Me Another Route</button>' : ''}<button class="ax-text-button" type="button" data-carrier-logistics-reset-route>Reset this route</button></div>
      </article>

      ${allDone ? `
        <article class="ax-current-assignment is-graduate">
          <span class="ax-assignment-stage">Route Complete</span>
          <h3>Carrier Logistics route complete</h3>
          <p>${esc(route.outcome)}</p>
          <a class="btn btn-primary" href="../carriers/">Open Carrier Network</a>
        </article>` : `
        <article class="ax-current-assignment">
          ...
          <div class="ax-current-copy">
            <span class="ax-next-label">Your Next Carrier Assignment</span>
            ...
            <div class="ax-assignment-actions">
              <button class="btn btn-primary" type="button" data-carrier-logistics-task-status="complete" data-carrier-logistics-task-id="${esc(current.id)}">Complete</button>
              <button class="btn btn-ghost" type="button" data-carrier-logistics-task-status="known" data-carrier-logistics-task-id="${esc(current.id)}">Already Know / Have This</button>
              <button class="btn btn-ghost" type="button" data-carrier-logistics-task-status="skipped" data-carrier-logistics-task-id="${esc(current.id)}">Skip for Now</button>
              ${taskLink(current)}
            </div>
          </div>
        </article>`}
      ...
      <details class="ax-assignment-list">
        ...
        <div class="ax-assignment-list-body">
          ${route.tasks.map(task => `<article class="ax-list-task${task.id === assignments.currentTaskId ? ' is-current' : ''}">
            ...
            <div class="ax-list-status status-${esc(task.status)}"><span>${esc(taskStatusLabels[task.status] || task.status)}</span>${task.status !== 'pending' ? `<button type="button" data-carrier-logistics-task-status="pending" data-carrier-logistics-task-id="${esc(task.id)}">Reopen</button>` : ''}</div>
          </article>`).join('')}
        </div>
      </details>
      ...
      <div class="ax-action-status" data-carrier-logistics-action-status></div>`;

    content.querySelector('[data-carrier-logistics-another-route]')?.addEventListener('click', () => runAction({ action:'another_route' }, 'Picking another Carrier Logistics route…'));
    content.querySelector('[data-carrier-logistics-reset-route]')?.addEventListener('click', () => {
      if (!window.confirm(`Reset all progress on ${route.title}?\n\nThis only resets this Carrier Logistics route. Your pathway preferences are unchanged.`)) return;
      runAction({ action:'reset_route' }, 'Resetting this Carrier Logistics route…');
    });
    content.querySelectorAll('[data-carrier-logistics-task-status]').forEach(button => button.addEventListener('click', () => {
      const status = button.dataset.carrierLogisticsTaskStatus;
      const taskId = button.dataset.carrierLogisticsTaskId;
  ...
  async function runAction(body, workingText) {
    if (busy) return;
    busy = true;
    const status = content.querySelector('[data-carrier-logistics-action-status]');
    ...
    try {
      ...
      render();
      const nextStatus = content.querySelector('[data-carrier-logistics-action-status]');
      if (nextStatus) { nextStatus.textContent = 'Progress saved.'; nextStatus.dataset.state = 'success'; }
    } catch (error) {
      console.error('Could not update Carrier Logistics pathway assignment', error);
      if (status) { status.textContent = error?.message || 'Could not update the Carrier Logistics assignment.'; status.dataset.state = 'error'; }
  ...
  async function load() {
    try {
      ...
      root.hidden = !savedSelected.has('carrier-logistics');
      if (!savedSelected.has('carrier-logistics')) {
        assignments = null;
        content.innerHTML = '';
        return;
      }
      if (loading) { loading.hidden = false; loading.textContent = 'Building your Carrier Logistics assignment…'; }
      assignments = await assignmentApi();
      render();
    } catch (error) {
      root.hidden = true;
      console.error('Could not load Carrier Logistics pathway assignments', error);
    }
  }
