(() => {
  ...
  const dateLabel = value => {
    ...
    return !d || Number.isNaN(d.getTime()) ? '—' : new Intl.DateTimeFormat(undefined, { month:'short', day:'numeric', year:'numeric', hour:'numeric', minute:'2-digit' }).format(d);
  };
  ...
  const answer = (label, value, wide = false) => {
  ...
  function historyHtml(app) {
    const history = Array.isArray(app.decisionHistory) ? app.decisionHistory : [];
    if (!history.length) return '';
    const entries = history.map(item => `<div class="application-answer wide"><span>Cycle ${esc(item.cycle || '')} · ${esc(statusLabel(item.status))}</span><p>Submitted: ${esc(dateLabel(item.submittedAt))} · Reviewed: ${esc(dateLabel(item.reviewedAt || item.declinedAt))}</p>${item.applicantMessage ? `<p><strong>Applicant message:</strong> ${esc(item.applicantMessage)}</p>` : ''}${item.officerNotes ? `<p><strong>Private notes:</strong> ${esc(item.officerNotes)}</p>` : ''}</div>`).join('');
    return `<details><summary>Previous application cycles (${history.length})</summary><div class="application-answer-grid">${entries}</div></details>`;
  }

  function actionHtml(app) {
    const save = '<button class="btn btn-ghost" type="button" data-review-action="save">Save Note / Verification</button>';
    if (app.status === 'accepted') return `${save}<span class="application-save-status">Accepted is terminal. Membership changes belong in member-management tools.</span>`;
    if (app.status === 'declined') return `${save}<button class="btn btn-primary" type="button" data-review-special="allow_reapplication">Allow Reapplication</button><span class="application-save-status">Declined is closed until leadership explicitly reopens it.</span>`;
    const review = app.status !== 'under_review' ? '<button class="btn btn-ghost" type="button" data-review-action="under_review">Mark Under Review</button>' : '';
    return `${save}${review}<button class="btn btn-primary" type="button" data-review-action="accepted">Approve & Grant Member Access</button><button class="btn btn-ghost" type="button" data-review-action="declined">Decline</button>`;
  }
  ...
  function render() {
    ...
    list.innerHTML = rows.map(app => {
      ...
      const applicantMessage = app.applicantMessage || '';
      const applicantMessageReadonly = ['accepted','declined'].includes(app.status) ? 'readonly' : '';
      const decisionMeta = app.status === 'declined' && app.declinedAt
        ? `<span>Declined: ${esc(dateLabel(app.declinedAt))}${app.declinedBy ? ` by ${esc(app.declinedBy)}` : ''}</span>`
        : '';
      return `<article class="application-review-card" data-application-id="${esc(app.id)}">
        <div class="application-review-head"><div><p class="eyebrow">${esc(statusLabel(app.status))}</p><h2>${esc(a.commanderName || 'Unnamed Commander')}</h2><div class="application-review-meta"><span>Discord: ${esc(app.ownerName || 'Unknown')}${app.discordUsername ? ` · @${esc(app.discordUsername)}` : ''}</span><span>Submitted: ${esc(dateLabel(app.submittedAt))}</span>${app.reviewedBy ? `<span>Reviewed by: ${esc(app.reviewedBy)}</span>` : ''}${app.status === 'accepted' && app.acceptedAt ? `<span>Accepted: ${esc(dateLabel(app.acceptedAt))}</span>` : ''}${decisionMeta}${Number(app.reapplicationCycle || 1) > 1 ? `<span>Application cycle: ${esc(app.reapplicationCycle)}</span>` : ''}</div></div><span class="application-status-badge ${esc(app.status)}">${esc(statusLabel(app.status))}</span></div>
        ...
        <label class="application-note"><span>Message to Applicant${app.status === 'declined' ? ' · Sent with decline decision' : ' · Optional if declined'}</span><textarea rows="3" maxlength="1000" data-applicant-message ${applicantMessageReadonly} placeholder="Optional. If left blank, the applicant receives a neutral standard message.">${esc(applicantMessage)}</textarea></label>
        ...
        <div class="application-review-actions">${actionHtml(app)}<span class="application-save-status" data-card-status></span></div>
      </article>`;
    }).join('');
  }

  async function updateApplication(card, requestedAction) {
    const id = card.dataset.applicationId;
    const current = applications.find(app => app.id === id);
    if (!current) return;
    const status = requestedAction === 'save' ? current.status : requestedAction;
    const commander = current?.answers?.commanderName || 'this Commander';
    const newlyAccepted = status === 'accepted' && current.status !== 'accepted';
    const newlyDeclined = status === 'declined' && current.status !== 'declined';
    const inGameVerified = Boolean(card.querySelector('[data-in-game-verified]')?.checked || current.inGameApplicationVerified);
    const applicantMessage = card.querySelector('[data-applicant-message]')?.value?.trim() || '';
    ...
    if (newlyDeclined) {
      const preview = applicantMessage || 'Neutral standard decline message';
      const approved = window.confirm(
        `Decline ${commander}?\n\nThis closes the application and sends the applicant a Discord DM.\n\nApplicant message: ${preview}\n\nA declined application cannot return to review unless leadership explicitly chooses Allow Reapplication.`,
      );
      if (!approved) return;
    }
    ...
    const buttons = [...card.querySelectorAll('button[data-review-action], button[data-review-special]')];
    buttons.forEach(button => { button.disabled = true; });
    if (state) {
      state.textContent = newlyAccepted ? 'Granting Member access…' : newlyDeclined ? 'Closing application…' : 'Saving…';
      state.dataset.state = '';
    }
    ...
    try {
      const response = await fetch('/api/applications', {
        ...
        body:JSON.stringify({ id, status, officerNotes:notes, applicantMessage, inGameApplicationVerified:inGameVerified, overrideInGameRequirement }),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) {
        if (data.error === 'ingame_application_not_verified') throw new Error('The in-game Squadron application has not been verified. Confirm it in Elite or explicitly approve as an exception.');
        ...
        if (data.error === 'application_terminal') throw new Error(data.detail || 'This decision is terminal and cannot be moved back into normal review.');
        if (data.error === 'invalid_status_transition') throw new Error(data.detail || 'That application state change is not allowed.');
      ...
      } else if (newlyDeclined) {
        const dmNote = ['failed','cooldown','storage_unavailable'].includes(data.declineNotice?.status)
          ? '\n\nThe decision was saved, but the decline DM may not have been delivered. Contact the applicant manually if needed.'
          : '\n\nThe applicant notification was sent.';
        window.alert(`Declined ${commander}.${dmNote}`);
      }
      render();
    } catch (error) {
  ...
  async function allowReapplication(card) {
    const id = card.dataset.applicationId;
    const current = applications.find(app => app.id === id);
    if (!current || current.status !== 'declined') return;
    const commander = current?.answers?.commanderName || 'this Commander';
    const approved = window.confirm(
      `Allow ${commander} to reapply?\n\nThe declined application will be preserved in leadership history. A new Draft cycle will be opened with the applicant's previous answers prefilled, but they must submit a fresh in-game Squadron application acknowledgement before resubmitting.`,
    );
    if (!approved) return;
    ...
    const buttons = [...card.querySelectorAll('button[data-review-action], button[data-review-special]')];
    buttons.forEach(button => { button.disabled = true; });
    if (state) { state.textContent = 'Opening a new application cycle…'; state.dataset.state = ''; }

    try {
      const response = await fetch('/api/applications', {
        method:'PUT', credentials:'same-origin', cache:'no-store',
        headers:{ Accept:'application/json', 'Content-Type':'application/json', 'X-Mongrels-Request':'application-review' },
        body:JSON.stringify({ id, action:'allow_reapplication' }),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.detail || data.error || `Request failed (${response.status})`);
      applications = applications.filter(app => app.id !== id);
      render();
      window.alert(`Reapplication opened for ${commander}.\n\nTheir previous decision is preserved in application history. The new cycle is now a Draft and will not return to the officer queue until they resubmit.`);
    } catch (error) {
      console.error('Could not open reapplication', error);
      if (state) { state.textContent = error?.message || 'Could not open reapplication.'; state.dataset.state = 'error'; }
      buttons.forEach(button => { button.disabled = false; });
    }
  }

  list.addEventListener('click', event => {
    const special = event.target.closest('button[data-review-special]');
    if (special) {
      const card = special.closest('[data-application-id]');
      if (card && special.dataset.reviewSpecial === 'allow_reapplication') allowReapplication(card);
      return;
    }
