(() => {
  const root = document.querySelector('[data-engineering-pathway]');
  const loading = document.querySelector('[data-engineering-loading]');
  const content = document.querySelector('[data-engineering-content]');
  ...
  async function assignmentApi(method = 'GET', body = null) {
    ...
    if (body) {
      ...
      options.body = JSON.stringify({ ...body, activity:'engineering' });
    }
    const response = await fetch(`/api/pathway/assignments?activity=engineering&_=${Date.now()}`, options);
  ...
  function render() {
    if (!assignments?.eligible) {
      content.innerHTML = '<div class="pathway-empty"><strong>Engineering & Shipbuilding is not in your saved pathway yet.</strong><br>Select Engineering & Shipbuilding above and save your pathway to generate an Engineering assignment chain.</div>';
    ...
    content.innerHTML = `
      <article class="ax-route-card">
        <div class="ax-route-head">
          <div><span class="ax-route-kicker">Your Engineering development route</span><h3>${esc(route.title)}</h3><p>${esc(route.subtitle)}</p></div>
          <div class="pathway-recommendation-badges"><span class="pathway-badge">${esc(route.band || experienceLabels[assignments.experience] || 'Engineering')}</span><span class="pathway-badge is-improve">Engineering Route</span></div>
        </div>
        ...
        <div class="ax-route-actions">${assignments.canChooseAnother ? '<button class="btn btn-ghost" type="button" data-engineering-another-route>Give Me Another Route</button>' : ''}<button class="ax-text-button" type="button" data-engineering-reset-route>Reset this route</button></div>
      </article>

      ${allDone ? `
        <article class="ax-current-assignment is-graduate">
          <span class="ax-assignment-stage">Route Complete</span>
          <h3>Engineering route complete</h3>
          <p>${esc(route.outcome)}</p>
          <a class="btn btn-primary" href="../guides/engineering/">Open Engineering Guides</a>
        </article>` : `
        <article class="ax-current-assignment">
          ...
          <div class="ax-current-copy">
            <span class="ax-next-label">Your Next Engineering Assignment</span>
            ...
            <div class="ax-assignment-actions">
              <button class="btn btn-primary" type="button" data-engineering-task-status="complete" data-engineering-task-id="${esc(current.id)}">Complete</button>
              <button class="btn btn-ghost" type="button" data-engineering-task-status="known" data-engineering-task-id="${esc(current.id)}">Already Know / Have This</button>
              <button class="btn btn-ghost" type="button" data-engineering-task-status="skipped" data-engineering-task-id="${esc(current.id)}">Skip for Now</button>
              ${taskLink(current)}
            </div>
          </div>
        </article>`}
      ...
      <details class="ax-assignment-list">
        ...
        <div class="ax-assignment-list-body">
          ${route.tasks.map(task => `<article class="ax-list-task${task.id === assignments.currentTaskId ? ' is-current' : ''}">
            ...
            <div class="ax-list-status status-${esc(task.status)}"><span>${esc(taskStatusLabels[task.status] || task.status)}</span>${task.status !== 'pending' ? `<button type="button" data-engineering-task-status="pending" data-engineering-task-id="${esc(task.id)}">Reopen</button>` : ''}</div>
          </article>`).join('')}
        </div>
      </details>
      ...
      <div class="ax-action-status" data-engineering-action-status></div>`;

    content.querySelector('[data-engineering-another-route]')?.addEventListener('click', () => runAction({ action:'another_route' }, 'Picking another Engineering route…'));
    content.querySelector('[data-engineering-reset-route]')?.addEventListener('click', () => {
      if (!window.confirm(`Reset all progress on ${route.title}?\n\nThis only resets this Engineering route. Your pathway preferences are unchanged.`)) return;
      runAction({ action:'reset_route' }, 'Resetting this Engineering route…');
    });
    content.querySelectorAll('[data-engineering-task-status]').forEach(button => button.addEventListener('click', () => {
      const status = button.dataset.engineeringTaskStatus;
  ...
  async function runAction(body, workingText) {
    if (busy) return;
    busy = true;
    const status = content.querySelector('[data-engineering-action-status]');
    ...
    try {
      ...
      render();
      const nextStatus = content.querySelector('[data-engineering-action-status]');
      if (nextStatus) { nextStatus.textContent = 'Progress saved.'; nextStatus.dataset.state = 'success'; }
    } catch (error) {
      console.error('Could not update Engineering pathway assignment', error);
      if (status) { status.textContent = error?.message || 'Could not update the Engineering assignment.'; status.dataset.state = 'error'; }
  ...
  async function load() {
    try {
      ...
      root.hidden = !savedSelected.has('engineering');
      if (!savedSelected.has('engineering')) {
        assignments = null;
        content.innerHTML = '';
        return;
      }
      if (loading) { loading.hidden = false; loading.textContent = 'Building your Engineering assignment…'; }
      assignments = await assignmentApi();
      render();
    } catch (error) {
      root.hidden = true;
      console.error('Could not load Engineering pathway assignments', error);
    }
  }
