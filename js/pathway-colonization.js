(() => {
  const root = document.querySelector('[data-colonization-pathway]');
  const loading = document.querySelector('[data-colonization-loading]');
  const content = document.querySelector('[data-colonization-content]');
  ...
  async function assignmentApi(method = 'GET', body = null) {
    ...
    if (body) {
      ...
      options.body = JSON.stringify({ ...body, activity:'colonization' });
    }
    const response = await fetch(`/api/pathway/assignments?activity=colonization&_=${Date.now()}`, options);
  ...
  function suppressGenericColonizationCard() {
    if (root.hidden || !previewRoot) return;
    previewRoot.querySelectorAll('.pathway-recommendation').forEach(card => {
      if (card.querySelector('h3')?.textContent?.trim() === 'Colonization') card.remove();
    });
  }

  function render() {
    if (!assignments?.eligible) {
      content.innerHTML = '<div class="pathway-empty"><strong>Colonization is not in your saved pathway yet.</strong><br>Select Colonization above and save your pathway to generate a Colonization assignment chain.</div>';
    ...
    content.innerHTML = `
      <article class="ax-route-card">
        <div class="ax-route-head">
          <div><span class="ax-route-kicker">Your Colonization development route</span><h3>${esc(route.title)}</h3><p>${esc(route.subtitle)}</p></div>
          <div class="pathway-recommendation-badges"><span class="pathway-badge">${esc(route.band || experienceLabels[assignments.experience] || 'Colonization')}</span><span class="pathway-badge is-improve">Colonization Route</span></div>
        </div>
        ...
        <div class="ax-route-actions">${assignments.canChooseAnother ? '<button class="btn btn-ghost" type="button" data-colonization-another-route>Give Me Another Route</button>' : ''}<button class="ax-text-button" type="button" data-colonization-reset-route>Reset this route</button></div>
      </article>

      ${allDone ? `
        <article class="ax-current-assignment is-graduate">
          <span class="ax-assignment-stage">Route Complete</span>
          <h3>Colonization route complete</h3>
          <p>${esc(route.outcome)}</p>
          <a class="btn btn-primary" href="../projects/">Open Projects & Events</a>
        </article>` : `
        <article class="ax-current-assignment">
          ...
          <div class="ax-current-copy">
            <span class="ax-next-label">Your Next Colonization Assignment</span>
            ...
            <div class="ax-assignment-actions">
              <button class="btn btn-primary" type="button" data-colonization-task-status="complete" data-colonization-task-id="${esc(current.id)}">Complete</button>
              <button class="btn btn-ghost" type="button" data-colonization-task-status="known" data-colonization-task-id="${esc(current.id)}">Already Know / Have This</button>
              <button class="btn btn-ghost" type="button" data-colonization-task-status="skipped" data-colonization-task-id="${esc(current.id)}">Skip for Now</button>
              ${taskLink(current)}
            </div>
          </div>
        </article>`}
      ...
      <details class="ax-assignment-list">
        ...
        <div class="ax-assignment-list-body">
          ${route.tasks.map(task => `<article class="ax-list-task${task.id === assignments.currentTaskId ? ' is-current' : ''}">
            ...
            <div class="ax-list-status status-${esc(task.status)}"><span>${esc(taskStatusLabels[task.status] || task.status)}</span>${task.status !== 'pending' ? `<button type="button" data-colonization-task-status="pending" data-colonization-task-id="${esc(task.id)}">Reopen</button>` : ''}</div>
          </article>`).join('')}
        </div>
      </details>
      ...
      <details class="ax-sources">
        ...
        <div>${sources.map(source => {
          const external = source.external || /^https?:\/\//i.test(source.url || '');
          return `<a href="${esc(source.url)}"${external ? ' target="_blank" rel="noopener noreferrer"' : ''}>${esc(source.label)}${external ? ' ↗' : ''}</a>`;
        }).join('')}</div>
      </details>
      <div class="ax-action-status" data-colonization-action-status></div>`;

    content.querySelector('[data-colonization-another-route]')?.addEventListener('click', () => runAction({ action:'another_route' }, 'Picking another Colonization route…'));
    content.querySelector('[data-colonization-reset-route]')?.addEventListener('click', () => {
      if (!window.confirm(`Reset all progress on ${route.title}?\n\nThis only resets this Colonization route. Your pathway preferences are unchanged.`)) return;
      runAction({ action:'reset_route' }, 'Resetting this Colonization route…');
    });
    content.querySelectorAll('[data-colonization-task-status]').forEach(button => button.addEventListener('click', () => {
      const status = button.dataset.colonizationTaskStatus;
      const taskId = button.dataset.colonizationTaskId;
    ...
    if (loading) loading.hidden = true;
    suppressGenericColonizationCard();
  }

  async function runAction(body, workingText) {
    if (busy) return;
    busy = true;
    const status = content.querySelector('[data-colonization-action-status]');
    ...
    try {
      ...
      render();
      const nextStatus = content.querySelector('[data-colonization-action-status]');
      if (nextStatus) { nextStatus.textContent = 'Progress saved.'; nextStatus.dataset.state = 'success'; }
    } catch (error) {
      console.error('Could not update Colonization pathway assignment', error);
      if (status) { status.textContent = error?.message || 'Could not update the Colonization assignment.'; status.dataset.state = 'error'; }
  ...
  async function load() {
    try {
      ...
      root.hidden = !savedSelected.has('colonization');
      if (!savedSelected.has('colonization')) {
        assignments = null;
        content.innerHTML = '';
        return;
      }
      if (loading) { loading.hidden = false; loading.textContent = 'Building your Colonization assignment…'; }
      assignments = await assignmentApi();
      render();
    } catch (error) {
      root.hidden = true;
      console.error('Could not load Colonization pathway assignments', error);
    }
  }

  if (previewRoot) {
    new MutationObserver(suppressGenericColonizationCard).observe(previewRoot, { childList:true, subtree:true });
  }
