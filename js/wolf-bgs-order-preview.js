(() => {
  ...
  let remote = {
    rules:{
      workload:{ missionInfPerCmdr:25, missionInfStretchPerCmdr:40, bountyMillionsPerCmdr:20, tradeProfitMillionsPerCmdr:20, preferredOperators:3 },
  ...
  async function refreshRemote() {
    const [rules, sliders] = await Promise.all([fetchJson(RULES_API), fetchJson(SLIDERS_API)]);
    remote = { ...remote, ...rules, ...sliders, rules:rules.rules || remote.rules };
  }
  ...
  function currentRules() {
    ...
    return { workload:w, balancing:b, safety:s };
  }
  ...
  function calibrationMarkup(card) {
    ...
    return `<div class="wolf-calibration-block"><div class="wolf-order-subhead"><div><strong>System balancing calibration</strong><span>Add/subtract from the global baseline for this system. Percentage scales with workload; flat INF is applied after the percentage adjustment.</span></div><span class="wolf-chip">${savedAt?`Saved ${esc(fmt(savedAt))}`:'Global baseline'}</span></div><div class="wolf-calibration-grid">
      <label><span>Bounty adjustment</span><div class="input-unit"><input type="number" min="-100" max="300" step="1" data-calibration="bountyPercentAdjustment" value="${esc(v('bountyPercentAdjustment'))}"><em>%</em></div></label>
      <label><span>Bounty flat adjustment</span><div class="input-unit"><input type="number" min="-100" max="100" step="1" data-calibration="bountyFlatInfAdjustment" value="${esc(v('bountyFlatInfAdjustment'))}"><em>INF</em></div></label>
      <label><span>Trade adjustment</span><div class="input-unit"><input type="number" min="-100" max="300" step="1" data-calibration="tradePercentAdjustment" value="${esc(v('tradePercentAdjustment'))}"><em>%</em></div></label>
      <label><span>Trade flat adjustment</span><div class="input-unit"><input type="number" min="-100" max="100" step="1" data-calibration="tradeFlatInfAdjustment" value="${esc(v('tradeFlatInfAdjustment'))}"><em>INF</em></div></label>
    </div><div class="wolf-calibration-actions"><span data-calibration-message>${savedAt?`Calibration saved by ${esc(savedBy||'Wolf')}.`:'No system-specific calibration saved.'}</span><div><button type="button" class="btn btn-secondary btn-compact" data-reset-calibration>Reset Calibration</button><button type="button" class="btn btn-secondary btn-compact" data-save-calibration>Save Calibration</button></div></div></div>`;
  }

  function sectionMarkup(card) {
    return `<section class="wolf-section wolf-order-preview-section" data-order-preview-section><h3>Order Preview / Generator</h3><p class="wolf-section-intro">Preview-only deterministic orders from the current on-screen board, faction intent, slider objectives, target bands, and balancing calibration. Nothing here publishes Daily Orders.</p>${calibrationMarkup(card)}<div class="wolf-order-toolbar"><span data-order-preview-meta>Uses current on-screen values, including unsaved edits.</span><button type="button" class="btn btn-primary btn-compact" data-generate-order-preview>Generate / Refresh Preview</button></div><div class="wolf-order-preview-output" data-order-preview-output><div class="wolf-order-empty">Generating preview…</div></div></section>`;
  }
  ...
  function addMissionTask(tasks, faction, amount, reason, stop) {
    if (!amount) return;
    let task=tasks.find(item=>item.kind==='mission-inf'&&norm(item.faction)===norm(faction));
    if (!task) { task={kind:'mission-inf',faction,amount,reason:[reason],stop:[stop].filter(Boolean)}; tasks.push(task); return; }
    task.amount=Math.max(task.amount,amount); if(reason&&!task.reason.includes(reason))task.reason.push(reason); if(stop&&!task.stop.includes(stop))task.stop.push(stop);
  }
  ...
  function generatePlan(card) {
    ...
    const findCounterweight=(source,amount,reason)=>{
      const candidates=candidateList(board,source,rules);
      const allocated=allocateCounterweight(amount,candidates.eligible,rules);
      if (!allocated.length) { warnings.push(`No safe counterweight faction found for ${source.name}: ${reason}`); return; }
      allocated.forEach(item=>addMissionTask(tasks,item.faction,item.amount,`Counterweight for ${source.name}: ${reason}. ${item.reason}`,`Stop/review before ${item.faction} reaches its configured ceiling or enters a conflict.`));
    ...
    for (const row of board) {
      ...
      if (row.intent==='support') {
        const shouldRaise=row.targetMax===null?true:(row.influence!==null&&row.influence<row.targetMax);
        const alreadyInBand=row.targetMin!==null&&row.targetMax!==null&&row.influence!==null&&row.influence>=row.targetMin&&row.influence<=row.targetMax;
      ...
      if (row.intent==='maintain') {
        ...
        if (above) { const candidates=candidateList(board,row,rules); const recipient=candidates.eligible[0]; if(recipient)addMissionTask(tasks,recipient.name,missionGoal,`Maintain / hold ${row.name} is above its target band; redirect positive work to ${recipient.name}.`,`Stop/review once ${row.name} returns to its target band.`); else warnings.push(`${row.name} is above its Maintain / hold band but no safe positive-work recipient is available.`); }
      }
      if (norm(row.name)===norm(MONGREL) && above && row.economy!=='raise' && row.security!=='raise') {
        ...
        if(recipient)addMissionTask(tasks,recipient.name,missionGoal,`Mongrels are above the ${row.targetMax}% ceiling; use positive work elsewhere rather than pushing Mongrels.`,`Stop/review when Mongrels return to the target band or ${recipient.name} reaches its own ceiling.`);
      ...
      for (const activity of ['security','economy']) {
        ...
        if (guard.blocked) { warnings.push(`${activity==='security'?'Security':'Economy'} ↑ for ${row.name} is not generated: ${guard.reason}`); continue; }
        const workload=activity==='security'?Number(rules.workload?.bountyMillionsPerCmdr??20):Number(rules.workload?.tradeProfitMillionsPerCmdr??20);
        tasks.push({kind:activity==='security'?'bounties':'trade',faction:row.name,amount:workload,reason:[`${activity==='security'?'Raise Security':'Raise Economy'} objective.`],stop:[row.targetMax!==null?`Reassess if ${row.name} influence moves beyond ${row.targetMax}%.`:'Reassess after the next trusted board update.']});
        if (guard.needed) {
          const calc=balanceMath(activity,workload,rules,calibration); math.push({...calc,source:row.name,reason:guard.reason});
          if (!calc.calibrated) warnings.push(`${activity==='security'?'Security':'Economy'} ↑ for ${row.name} needs influence balancing, but no ${activity==='security'?'bounty':'trade'} counterweight baseline is configured.`);
          else if (calc.final>0) findCounterweight(row,calc.final,guard.reason);
        } else checks.push(`${activity==='security'?'Security':'Economy'} ↑ for ${row.name}: ${guard.reason}`);
      }
    }
  ...
  function taskMarkup(task,index) {
    const label=task.kind==='mission-inf'?'MISSION INF':task.kind==='bounties'?'SECURITY / BOUNTIES':'ECONOMY / TRADE';
    const instruction=task.kind==='mission-inf'?`Complete about ${task.amount} INF for ${task.faction}`:task.kind==='bounties'?`Turn in about ${task.amount}M Cr of bounty vouchers for ${task.faction}`:`Generate about ${task.amount}M Cr profitable trade for ${task.faction}`;
    return `<article class="wolf-order-task"><div class="wolf-order-task-number">${String(index+1).padStart(2,'0')}</div><div><span class="wolf-order-task-type">${esc(label)}</span><strong>${esc(instruction)}</strong><p>${task.reason.map(esc).join(' ')}</p>${task.stop.length?`<small><b>Stop / review:</b> ${task.stop.map(esc).join(' ')}</small>`:''}</div></article>`;
  }
  ...
  function wire(card) {
    ...
    card.addEventListener('change',event=>{if(event.target.matches('[data-calibration],[data-faction-strategy],[data-slider-objective],[data-setting="targetMin"],[data-setting="targetMax"],[data-faction="influence"],[data-faction="state"],[data-faction="pending"],[data-faction="recovering"]'))generate(card);});
  }
  ...
  function enhanceAll(){document.querySelectorAll('.wolf-system-card').forEach(mount);}
  ...
  async function init() {
    ...
    window.addEventListener('wolf-bgs-rules-updated',async()=>{try{await refreshRemote();}catch{}enhanceAll();document.querySelectorAll('.wolf-system-card').forEach(card=>generate(card));});
