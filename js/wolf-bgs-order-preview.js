(() => {
  ...
  function sectionMarkup(card) {
    return `<section class="wolf-section wolf-order-preview-section" data-order-preview-section><h3>Order Preview / Generator</h3><p class="wolf-section-intro">Deterministic orders from the current on-screen board, faction intent, slider objectives, target bands, workload doctrine, and balancing calibration. Generating a preview never publishes by itself; reviewed live-system previews must be explicitly queued and published.</p>${calibrationMarkup(card)}<div class="wolf-order-toolbar"><span data-order-preview-meta>Uses current on-screen values, including unsaved edits.</span><button type="button" class="btn btn-primary btn-compact" data-generate-order-preview>Generate / Refresh Preview</button></div><div class="wolf-order-preview-output" data-order-preview-output><div class="wolf-order-empty">Generating preview…</div></div></section>`;
  }
  ...
  function renderPlan(card,plan) {
    ...
    const warnings=plan.warnings.length?`<div class="wolf-order-warnings"><strong>Needs attention before publishing</strong><ul>${plan.warnings.map(item=>`<li>${esc(item)}</li>`).join('')}</ul></div>`:'<div class="wolf-order-ready"><strong>Preview checks complete.</strong><span>No blocking configuration warning was found. Review the tasks before adding this system to the Publish Queue.</span></div>';
    host.innerHTML=`<div class="wolf-order-preview-head"><div><span class="wolf-chip preview">REVIEW</span><strong>${esc(plan.system)} — Proposed Orders</strong><small>${plan.tasks.length} generated task${plan.tasks.length===1?'':'s'} · ${esc(String(plan.rules.workload?.preferredOperators??3))} preferred CMDRs/objective</small></div></div><div class="wolf-order-task-list">${taskHtml}</div>${math}${warnings}${checks}<p class="wolf-order-footnote">Mission INF means mission reward influence pips/ticks, not faction percentage points. Overlapping same-faction counterweight needs keep the higher INF workload rather than being blindly added. Exploration is a separate diversification bucket and is not assigned a fake INF conversion.</p>`;
