(() => {
  ...
  function render(){
    ...
    visible.forEach(item => {
      ...
      const discordError=item.canEdit&&item.discord?.error
        ? `<p class="bounty-discord-error" role="status"><strong>Discord sync failed:</strong> ${safe(item.discord.error)}</p>`
        : '';
      card.innerHTML = `
        <div class="bounty-card-head"><div><span class="bounty-status">${safe(label(item.status))}</span></div><div class="bounty-card-actions">${discordAction}${item.canEdit?'<button type="button" class="btn btn-secondary bounty-edit-btn">Edit</button>':''}</div></div>
        ${discordError}
