(() => {
  const root = document.querySelector('[data-exploration-pathway]');
  const loading = document.querySelector('[data-exploration-loading]');
  const content = document.querySelector('[data-exploration-content]');
  ...
  async function assignmentApi(method = 'GET', body = null) {
    ...
    if (body) {
      ...
      options.body = JSON.stringify({ ...body, activity:'exploration' });
    }
    const response = await fetch(`/api/pathway/assignments?activity=exploration&_=${Date.now()}`, options);
  ...
  function suppressGenericExplorationCard() {
    if (root.hidden || !previewRoot) return;
    previewRoot.querySelectorAll('.pathway-recommendation').forEach(card => {
      if (card.querySelector('h3')?.textContent?.trim() === 'Exploration') card.remove();
    });
  }

  function render() {
    if (!assignments?.eligible) {
      content.innerHTML = '<div class="pathway-empty"><strong>Exploration is not in your saved pathway yet.</strong><br>Select Exploration above and save your pathway to generate an Exploration assignment chain.</div>';
    ...
    content.innerHTML = `
      <article class="ax-route-card">
        <div class="ax-route-head">
          <div><span class="ax-route-kicker">Your Exploration development route</span><h3>${esc(route.title)}</h3><p>${esc(route.subtitle)}</p></div>
          <div class="pathway-recommendation-badges"><span class="pathway-badge">${esc(route.band || experienceLabels[assignments.experience] || 'Exploration')}</span><span class="pathway-badge is-improve">Exploration Route</span></div>
        </div>
        ...
        <div class="ax-route-actions">${assignments.canChooseAnother ? '<button class="btn btn-ghost" type="button" data-exploration-another-route>Give Me Another Route</button>' : ''}<button class="ax-text-button" type="button" data-exploration-reset-route>Reset this route</button></div>
      </article>

      ${allDone ? `
        <article class="ax-current-assignment is-graduate">
          <span class="ax-assignment-stage">Route Complete</span>
          <h3>Exploration route complete</h3>
          <p>${esc(route.outcome)}</p>
          <a class="btn btn-primary" href="../activities/#exploration">Open Exploration & Discovery Hub</a>
        </article>` : `
        <article class="ax-current-assignment">
          ...
          <div class="ax-current-copy">
            <span class="ax-next-label">Your Next Exploration Assignment</span>
            ...
            <div class="ax-assignment-actions">
              <button class="btn btn-primary" type="button" data-exploration-task-status="complete" data-exploration-task-id="${esc(current.id)}">Complete</button>
              <button class="btn btn-ghost" type="button" data-exploration-task-status="known" data-exploration-task-id="${esc(current.id)}">Already Know / Have This</button>
              <button class="btn btn-ghost" type="button" data-exploration-task-status="skipped" data-exploration-task-id="${esc(current.id)}">Skip for Now</button>
              ${taskLink(current)}
            </div>
          </div>
        </article>`}
      ...
      <details class="ax-assignment-list">
        ...
        <div class="ax-assignment-list-body">
          ${route.tasks.map(task => `<article class="ax-list-task${task.id === assignments.currentTaskId ? ' is-current' : ''}">
            ...
            <div class="ax-list-status status-${esc(task.status)}"><span>${esc(taskStatusLabels[task.status] || task.status)}</span>${task.status !== 'pending' ? `<button type="button" data-exploration-task-status="pending" data-exploration-task-id="${esc(task.id)}">Reopen</button>` : ''}</div>
          </article>`).join('')}
        </div>
      </details>
      ...
      <div class="ax-action-status" data-exploration-action-status></div>`;

    content.querySelector('[data-exploration-another-route]')?.addEventListener('click', () => runAction({ action:'another_route' }, 'Picking another Exploration route…'));
    content.querySelector('[data-exploration-reset-route]')?.addEventListener('click', () => {
      if (!window.confirm(`Reset all progress on ${route.title}?\n\nThis only resets this Exploration route. Your pathway preferences are unchanged.`)) return;
      runAction({ action:'reset_route' }, 'Resetting this Exploration route…');
    });
    content.querySelectorAll('[data-exploration-task-status]').forEach(button => button.addEventListener('click', () => {
      const status = button.dataset.explorationTaskStatus;
      const taskId = button.dataset.explorationTaskId;
    ...
    if (loading) loading.hidden = true;
    suppressGenericExplorationCard();
  }

  async function runAction(body, workingText) {
    if (busy) return;
    busy = true;
    const status = content.querySelector('[data-exploration-action-status]');
    ...
    try {
      ...
      render();
      const nextStatus = content.querySelector('[data-exploration-action-status]');
      if (nextStatus) { nextStatus.textContent = 'Progress saved.'; nextStatus.dataset.state = 'success'; }
    } catch (error) {
      console.error('Could not update Exploration pathway assignment', error);
      if (status) { status.textContent = error?.message || 'Could not update the Exploration assignment.'; status.dataset.state = 'error'; }
  ...
  async function load() {
    try {
      ...
      root.hidden = !savedSelected.has('exploration');
      if (!savedSelected.has('exploration')) {
        assignments = null;
        content.innerHTML = '';
        return;
      }
      if (loading) { loading.hidden = false; loading.textContent = 'Building your Exploration assignment…'; }
      assignments = await assignmentApi();
      render();
    } catch (error) {
      root.hidden = true;
      console.error('Could not load Exploration pathway assignments', error);
    }
  }

  if (previewRoot) {
    new MutationObserver(suppressGenericExplorationCard).observe(previewRoot, { childList:true, subtree:true });
  }
