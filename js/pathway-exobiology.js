(() => {
  const root = document.querySelector('[data-exobiology-pathway]');
  const loading = document.querySelector('[data-exobiology-loading]');
  const content = document.querySelector('[data-exobiology-content]');
  ...
  async function assignmentApi(method = 'GET', body = null) {
    ...
    if (body) {
      ...
      options.body = JSON.stringify({ ...body, activity:'exobiology' });
    }
    const response = await fetch(`/api/pathway/assignments?activity=exobiology&_=${Date.now()}`, options);
  ...
  function suppressGenericExobiologyCard() {
    if (root.hidden || !previewRoot) return;
    previewRoot.querySelectorAll('.pathway-recommendation').forEach(card => {
      if (card.querySelector('h3')?.textContent?.trim() === 'Exobiology') card.remove();
    });
  }

  function render() {
    if (!assignments?.eligible) {
      content.innerHTML = '<div class="pathway-empty"><strong>Exobiology is not in your saved pathway yet.</strong><br>Select Exobiology above and save your pathway to generate an Exobiology assignment chain.</div>';
    ...
    content.innerHTML = `
      <article class="ax-route-card">
        <div class="ax-route-head">
          <div><span class="ax-route-kicker">Your Exobiology development route</span><h3>${esc(route.title)}</h3><p>${esc(route.subtitle)}</p></div>
          <div class="pathway-recommendation-badges"><span class="pathway-badge">${esc(route.band || experienceLabels[assignments.experience] || 'Exobiology')}</span><span class="pathway-badge is-improve">Exobiology Route</span></div>
        </div>
        ...
        <div class="ax-route-actions">${assignments.canChooseAnother ? '<button class="btn btn-ghost" type="button" data-exobiology-another-route>Give Me Another Route</button>' : ''}<button class="ax-text-button" type="button" data-exobiology-reset-route>Reset this route</button></div>
      </article>

      ${allDone ? `
        <article class="ax-current-assignment is-graduate">
          <span class="ax-assignment-stage">Route Complete</span>
          <h3>Exobiology route complete</h3>
        ...
        <article class="ax-current-assignment">
          ...
          <div class="ax-current-copy">
            <span class="ax-next-label">Your Next Exobiology Assignment</span>
            ...
            <div class="ax-assignment-actions">
              <button class="btn btn-primary" type="button" data-exobiology-task-status="complete" data-exobiology-task-id="${esc(current.id)}">Complete</button>
              <button class="btn btn-ghost" type="button" data-exobiology-task-status="known" data-exobiology-task-id="${esc(current.id)}">Already Know / Have This</button>
              <button class="btn btn-ghost" type="button" data-exobiology-task-status="skipped" data-exobiology-task-id="${esc(current.id)}">Skip for Now</button>
              ${taskLink(current)}
            </div>
          </div>
        </article>`}
      ...
      <details class="ax-assignment-list">
        ...
        <div class="ax-assignment-list-body">
          ${route.tasks.map(task => `<article class="ax-list-task${task.id === assignments.currentTaskId ? ' is-current' : ''}">
            ...
            <div class="ax-list-status status-${esc(task.status)}"><span>${esc(taskStatusLabels[task.status] || task.status)}</span>${task.status !== 'pending' ? `<button type="button" data-exobiology-task-status="pending" data-exobiology-task-id="${esc(task.id)}">Reopen</button>` : ''}</div>
          </article>`).join('')}
        </div>
      </details>
      ...
      <div class="ax-action-status" data-exobiology-action-status></div>`;

    content.querySelector('[data-exobiology-another-route]')?.addEventListener('click', () => runAction({ action:'another_route' }, 'Picking another Exobiology route…'));
    content.querySelector('[data-exobiology-reset-route]')?.addEventListener('click', () => {
      if (!window.confirm(`Reset all progress on ${route.title}?\n\nThis only resets this Exobiology route. Your pathway preferences are unchanged.`)) return;
      runAction({ action:'reset_route' }, 'Resetting this Exobiology route…');
    });
    content.querySelectorAll('[data-exobiology-task-status]').forEach(button => button.addEventListener('click', () => {
      const status = button.dataset.exobiologyTaskStatus;
      const taskId = button.dataset.exobiologyTaskId;
    ...
    if (loading) loading.hidden = true;
    suppressGenericExobiologyCard();
  }

  async function runAction(body, workingText) {
    if (busy) return;
    busy = true;
    const status = content.querySelector('[data-exobiology-action-status]');
    ...
    try {
      ...
      render();
      const nextStatus = content.querySelector('[data-exobiology-action-status]');
      if (nextStatus) { nextStatus.textContent = 'Progress saved.'; nextStatus.dataset.state = 'success'; }
    } catch (error) {
      console.error('Could not update Exobiology pathway assignment', error);
      if (status) { status.textContent = error?.message || 'Could not update the Exobiology assignment.'; status.dataset.state = 'error'; }
  ...
  async function load() {
    try {
      ...
      root.hidden = !savedSelected.has('exobiology');
      if (!savedSelected.has('exobiology')) {
        assignments = null;
        content.innerHTML = '';
        return;
      }
      if (loading) { loading.hidden = false; loading.textContent = 'Building your Exobiology assignment…'; }
      assignments = await assignmentApi();
      render();
    } catch (error) {
      root.hidden = true;
      console.error('Could not load Exobiology pathway assignments', error);
    }
  }

  if (previewRoot) {
    new MutationObserver(suppressGenericExobiologyCard).observe(previewRoot, { childList:true, subtree:true });
  }
