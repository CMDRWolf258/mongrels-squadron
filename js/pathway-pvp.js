(() => {
  const root = document.querySelector('[data-pvp-pathway]');
  const loading = document.querySelector('[data-pvp-loading]');
  const content = document.querySelector('[data-pvp-content]');
  ...
  async function assignmentApi(method = 'GET', body = null) {
    ...
    if (body) {
      ...
      options.body = JSON.stringify({ ...body, activity:'pvp' });
    }
    const response = await fetch(`/api/pathway/assignments?activity=pvp&_=${Date.now()}`, options);
  ...
  function suppressGenericPvpCard() {
    if (root.hidden || !previewRoot) return;
    previewRoot.querySelectorAll('.pathway-recommendation').forEach(card => {
      if (card.querySelector('h3')?.textContent?.trim() === 'PvP') card.remove();
    });
  }

  function render() {
    if (!assignments?.eligible) {
      content.innerHTML = '<div class="pathway-empty"><strong>PvP is not in your saved pathway yet.</strong><br>Select PvP above and save your pathway to generate a PvP assignment chain.</div>';
    ...
    content.innerHTML = `
      <article class="ax-route-card">
        <div class="ax-route-head">
          <div><span class="ax-route-kicker">Your PvP development route</span><h3>${esc(route.title)}</h3><p>${esc(route.subtitle)}</p></div>
          <div class="pathway-recommendation-badges"><span class="pathway-badge">${esc(route.band || experienceLabels[assignments.experience] || 'PvP')}</span><span class="pathway-badge is-improve">PvP Route</span></div>
        </div>
        ...
        <div class="ax-route-actions">${assignments.canChooseAnother ? '<button class="btn btn-ghost" type="button" data-pvp-another-route>Give Me Another Route</button>' : ''}<button class="ax-text-button" type="button" data-pvp-reset-route>Reset this route</button></div>
      </article>

      ${allDone ? `
        <article class="ax-current-assignment is-graduate">
          <span class="ax-assignment-stage">Route Complete</span>
          <h3>PvP route complete</h3>
          <p>${esc(route.outcome)}</p>
          <a class="btn btn-primary" href="../pvp/">Open PvP Hub</a>
        </article>` : `
        <article class="ax-current-assignment">
          ...
          <div class="ax-current-copy">
            <span class="ax-next-label">Your Next PvP Assignment</span>
            ...
            <div class="ax-assignment-actions">
              <button class="btn btn-primary" type="button" data-pvp-task-status="complete" data-pvp-task-id="${esc(current.id)}">Complete</button>
              <button class="btn btn-ghost" type="button" data-pvp-task-status="known" data-pvp-task-id="${esc(current.id)}">Already Know / Have This</button>
              <button class="btn btn-ghost" type="button" data-pvp-task-status="skipped" data-pvp-task-id="${esc(current.id)}">Skip for Now</button>
              ${taskLink(current)}
            </div>
          </div>
        </article>`}
      ...
      <details class="ax-assignment-list">
        ...
        <div class="ax-assignment-list-body">
          ${route.tasks.map(task => `<article class="ax-list-task${task.id === assignments.currentTaskId ? ' is-current' : ''}">
            ...
            <div class="ax-list-status status-${esc(task.status)}"><span>${esc(taskStatusLabels[task.status] || task.status)}</span>${task.status !== 'pending' ? `<button type="button" data-pvp-task-status="pending" data-pvp-task-id="${esc(task.id)}">Reopen</button>` : ''}</div>
          </article>`).join('')}
        </div>
      </details>
      ...
      <div class="ax-action-status" data-pvp-action-status></div>`;

    content.querySelector('[data-pvp-another-route]')?.addEventListener('click', () => runAction({ action:'another_route' }, 'Picking another PvP route…'));
    content.querySelector('[data-pvp-reset-route]')?.addEventListener('click', () => {
      if (!window.confirm(`Reset all progress on ${route.title}?\n\nThis only resets this PvP route. Your pathway preferences are unchanged.`)) return;
      runAction({ action:'reset_route' }, 'Resetting this PvP route…');
    });
    content.querySelectorAll('[data-pvp-task-status]').forEach(button => button.addEventListener('click', () => {
      const status = button.dataset.pvpTaskStatus;
      const taskId = button.dataset.pvpTaskId;
    ...
    if (loading) loading.hidden = true;
    suppressGenericPvpCard();
  }

  async function runAction(body, workingText) {
    if (busy) return;
    busy = true;
    const status = content.querySelector('[data-pvp-action-status]');
    ...
    try {
      ...
      render();
      const nextStatus = content.querySelector('[data-pvp-action-status]');
      if (nextStatus) { nextStatus.textContent = 'Progress saved.'; nextStatus.dataset.state = 'success'; }
    } catch (error) {
      console.error('Could not update PvP pathway assignment', error);
      if (status) { status.textContent = error?.message || 'Could not update the PvP assignment.'; status.dataset.state = 'error'; }
  ...
  async function load() {
    try {
      ...
      root.hidden = !savedSelected.has('pvp');
      if (!savedSelected.has('pvp')) {
        assignments = null;
        content.innerHTML = '';
        return;
      }
      if (loading) { loading.hidden = false; loading.textContent = 'Building your PvP assignment…'; }
      assignments = await assignmentApi();
      render();
    } catch (error) {
      root.hidden = true;
      console.error('Could not load PvP pathway assignments', error);
    }
  }

  if (previewRoot) {
    new MutationObserver(suppressGenericPvpCard).observe(previewRoot, { childList:true, subtree:true });
  }
