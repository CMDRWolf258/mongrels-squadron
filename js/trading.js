(() => {
  ...
  const quantityLabel=value=>{
    ...
    return digits?Number(digits).toLocaleString()+' t':raw;
  };
  ...
  function card(route) {
    ...
    const quantity = route.quantity ? `<div><span>Quantity</span><strong>${safe(quantityLabel(route.quantity))}</strong></div>` : '';
    const hasReturn=Boolean(route.returnCommodity||n(route.returnProfitPerTon)||route.returnQuantity);
    const returnProfit=n(route.returnProfitPerTon)>0?`${fmt(route.returnProfitPerTon)} Cr/t`:'Profit not listed';
    const returnQuantity=route.returnQuantity?quantityLabel(route.returnQuantity):'';
    const returnLeg=hasReturn?`
      <div class="trade-return-cargo"><span>Return cargo</span><strong>${safe(route.returnCommodity||'Commodity not listed')}</strong>${n(route.returnProfitPerTon)>0?`<em>${safe(returnProfit)}</em>`:''}${returnQuantity?`<em>${safe(returnQuantity)}</em>`:''}</div>
      <div class="trade-route-line trade-route-return"><div><span>Return Deliver</span><strong>${safe(route.originStation||'—')}</strong><small>${originSystem}</small></div><div class="trade-arrow">←</div><div><span>Return Load</span><strong>${safe(route.destinationStation||'—')}</strong><small>${destSystem}</small></div></div>`:'';
  ...
  function render() {
    ...
    let credit=routes.filter(r=>r.category==='credits'&&active(r)).filter(r=>{const hay=[r.title,r.commodity,r.returnCommodity,r.originStation,r.originSystem,r.destinationStation,r.destinationSystem,r.notes,...(r.tags||[])].join(' ').toLowerCase();return(!q||hay.includes(q))&&(pad==='all'||String(r.padSize||'').toLowerCase()===pad);});
  ...
  function payload(){return{id:$('[data-trade-id]').value||undefined,category:$('[data-trade-category]').value,official:$('[data-trade-official]').value==='true',title:$('[data-trade-title]').value,commodity:$('[data-trade-commodity]').value,originSystem:$('[data-trade-origin-system]').value,originStation:$('[data-trade-origin-station]').value,destinationSystem:$('[data-trade-destination-system]').value,destinationStation:$('[data-trade-destination-station]').value,profitPerTon:parseInteger($('[data-trade-profit]').value),estimatedLoopProfit:parseInteger($('[data-trade-loop-profit]').value),padSize:$('[data-trade-pad]').value,distanceLy:$('[data-trade-distance]').value,quantity:String(parseInteger($('[data-trade-quantity]').value)||''),returnCommodity:$('[data-trade-return-commodity]').value,returnProfitPerTon:parseInteger($('[data-trade-return-profit]').value),returnQuantity:String(parseInteger($('[data-trade-return-quantity]').value)||''),expires:$('[data-trade-expires]').value,status:$('[data-trade-status]').value,tags:$('[data-trade-tags]').value,objective:$('[data-trade-objective]').value,notes:$('[data-trade-notes]').value};}
