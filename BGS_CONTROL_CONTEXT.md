...
Member Daily Orders are grouped by **system** into compact expandable cards. Inside each system, every published order is rendered as one unified task/report console. The latest layout uses a compact briefing strip across the top and a full-width report area directly underneath, eliminating side-column dead space. The briefing makes the target faction and workload the dominant information (the full "Regiment of Imperial Mongrels" is displayed operationally as **MONGRELS**), followed by one short execution instruction and a smaller Stop / Review line. System copy lives in the system header, redundant ACTIVE badges are removed, progress headers are compressed, INF reporting uses wide 40/20/40-style minus/count/plus controls, and positive actions are visually emphasized. Desktop uses four INF reward controls across one row; narrower layouts shift to two columns for touch use. The report-entry area now clusters controls on the left and isolates a compact draft-total/Submit panel on the right so Submit is visually separated from adjustment buttons. Credit workloads use a compact value + `M Cr` field beside quick-adjust buttons instead of stretching the amount field across the full card. Member Mission Control intentionally omits Wolf-level Stop / Review thresholds and other strategy math; those remain in Wolf BGS Control. Member briefing copy is execution-only and avoids repeating the already-prominent faction name. Report headings are normalized to REPORT INF / REPORT BOUNTIES / REPORT TRADE / REPORT EXPLORATION, and redundant multi-order system-header task/progress text is suppressed.
