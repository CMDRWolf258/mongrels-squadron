...
- Elite's `MissionAccepted` event identifies the mission giver and destination but does **not** include a dedicated origin-system field. Scout therefore captures the commander's current journal location (system, SystemAddress, station) at acceptance time.
- Relevant accepted-mission origins are persisted on the connected Frontier account for up to 14 days so a mission accepted in one sync/day can still be attributed when completed later.
- `MissionCompleted.FactionEffects[].Influence[]` remains the authority for the exact INF pips and affected faction/system.
- The stored acceptance location is used to resolve the source-faction/origin effect, especially across separate journal fetches or when only the numeric SystemAddress is present.
- Secondary destination/target-faction influence remains a separate effect. One passenger mission can therefore advance both a source-faction order and a destination-faction order without adding the two into one opaque INF number.
- Member Recent Activity now renders each mission effect separately by faction/system and shows the mission origin. The aggregate KPI is labelled **BGS INF Effects** to make clear that it totals all BGS effects, not merely the reward pips selected from the mission giver.
