...
export async function appendRewardEntryWithResult(env, entry) {
  ...
  if (existing) return {entry:existing,created:false,key};
...
export function normalizeRewardEntry(value = {}) {
  ...
  return {
    ...
    kind:['verified_order','manual_adjustment','special_job','colonization_job'].includes(value.kind) ? value.kind : 'verified_order',
    amountCredits,
