...
export async function searchTradeMarkets(env,input,{fetchImpl=fetch,timeoutMs=15000}={}) {
  ...
  const rareCached=await rareSourceCacheFallback(env,query,control,rows).catch(()=>null);
...
async function rareSourceCacheFallback(env,query,control,liveRows=[]){
  if(!query?.rareSourceSearch||!query?.rareSource?.systemName||!query?.rareSource?.stationName)return null;
  const sourceSeen=Array.isArray(liveRows)&&liveRows.some(row=>
  ...
  const fetchedAt=new Date().toISOString();
  const warning='Spansh did not return the known rare source station, so this result is the last matching observation from the Mongrel rare-source cache. Verify the age before travel.';
  await writeMarketHealth(env,{
    lastAttemptAt:fetchedAt,
    lastSuccessfulFetchAt:fetchedAt,
    lastError:'',
