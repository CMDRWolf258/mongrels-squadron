...
export async function searchTradeMarkets(env,input,{fetchImpl=fetch}={}) {
  ...
  const primaryUrl=buildEdDataSearchUrl(query);

  let raw;
  let sourceMode=query.sort==='price'?'price-ranked':'nearby';
  let partial=false;
  let warning='';

  try{
    raw=await fetchEdDataJson(primaryUrl,{fetchImpl,timeoutMs:18000});
  }catch(primaryError){
    if(query.sort!=='price'&&String(primaryError?.message||'')==='market_source_timeout'){
      try{
        raw=await fetchEdDataJson(buildEdDataPriceSearchUrl(query),{fetchImpl,timeoutMs:12000});
        sourceMode='price-ranked-fallback';
        partial=true;
        warning='The broad market query timed out, so results are limited to EDData’s top price-ranked candidates and then re-sorted locally.';
      }catch(fallbackError){
        const cached=await cachedSearchFallback(env,query,control,primaryError);
        if(cached)return cached;
        await writeMarketHealth(env,{
          lastAttemptAt:new Date().toISOString(),
          lastError:friendlySourceError(fallbackError),
      ...
      await writeMarketHealth(env,{
        lastAttemptAt:new Date().toISOString(),
        lastError:friendlySourceError(primaryError),
        lastQuery:publicQuery(query),
        source:'EDData / EDDN',
      }).catch(()=>{});
      throw primaryError;
...
export function buildEdDataPriceSearchUrl(query) {
  ...
  const path=[EDDATA_BASE,'commodity/name',encodeURIComponent(query.commodity),action].join('/');
...
async function cachedSearchFallback(env,query,control,error){
  if(!['market_source_timeout','market_source_http_502','market_source_http_503','market_source_http_504'].some(code=>String(error?.message||'').startsWith(code)))return null;
  const cached=await readMarketObservations(env,query.commodity);
  if(!cached.length)return null;
  const reference=norm(query.referenceSystem);
  const candidates=cached.filter(item=>
    norm(item?.distanceFromSystem)===reference
    && (item?.distanceLy===null||item?.distanceLy===undefined||Number(item.distanceLy)<=query.radiusLy)
  );
  ...
  const fetchedAt=new Date().toISOString();
  const warning='Live EDData lookup was unavailable, so these are matching observations from the Mongrel market cache.';
  await writeMarketHealth(env,{
    lastAttemptAt:fetchedAt,
