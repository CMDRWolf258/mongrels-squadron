...
async function enrichRareSourceAllocations(env,query,observations){
  ...
  return rows.map(item=>{
    ...
    const positive=reportedSupply>0;
    return{
      ...item,
      commanderSensitiveSupply:true,
      reportedSupply,
      allocationSupply:positive?reportedSupply:priorPositiveSupply,
      allocationObservedAt:positive?item.observedAt:priorPositiveAt,
      allocationBuyPrice:positive?Math.max(0,Math.round(Number(item?.buyPrice)||0)):priorPositivePrice,
    };
  });
}
...
function mergeRareStoredObservation(current,item){
  ...
  const positive=candidates[0]||null;
  return{
    ...newest,
    commanderSensitiveSupply:true,
