...
export function rareSourceQualificationStatus(item,query){
  ...
  if(query.direction==='buy'){
    if(Number(item.supply||0)<Number(query.minVolume||0))return Number(item.supply||0)<=0?'no_observed_stock':'supply_below_threshold';
    if(Number(item.buyPrice||0)<=0)return'price_unavailable';
    if(Number(query.price||0)>0&&Number(item.buyPrice||0)>Number(query.price))return'price_above_threshold';
  }
