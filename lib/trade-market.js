...
export async function searchTradeMarkets(env,input,{fetchImpl=fetch}={}) {
  ...
  let payload;
  try{
    payload=await fetchSpanshSearch(query,{fetchImpl,timeoutMs:15000});
  }catch(error){
  ...
  const warning=partial
    ?'Spansh matched more markets than this interactive page returns; results are ranked from the first '+rows.length+' candidates.'
    :'';
...
export async function getTradeCommodityCatalog(env,{fetchImpl=fetch,force=false}={}) {
  ...
  try{
    ...
    const items=names.map(label=>({name:label,label}));
    const value={
    ...
    try{
      ...
      const items=(Array.isArray(raw)?raw:[])
        .map(item=>{
          ...
          if(!name)return null;
          return {name,label:commodityLabel(name)};
        })
        .filter(Boolean)
...
export function buildSpanshSearchBody(query,now=new Date()) {
  const stationTypes=query.carrierMode==='only'
    ?CARRIER_TRADE_TYPES
  ...
  return {
    filters:{
      distance:{min:'0',max:String(query.radiusLy)},
...
export function normalizeSpanshObservation(item,query) {
  ...
  const wanted=normalizeCommodity(query.commodity);
  const entry=(Array.isArray(item.market)?item.market:[]).find(row=>
    normalizeCommodity(row?.commodity)===wanted
  );
...
async function cachedSearchFallback(env,query,control,error){
  ...
  const cached=await readMarketObservations(env,query.commodityKey);
