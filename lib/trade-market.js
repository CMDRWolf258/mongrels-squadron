...
const RARE_ALLOCATION_TIME_ZONE='America/Chicago';
const RARE_ALLOCATION_DAY_FORMATTER=new Intl.DateTimeFormat('en-CA',{
  timeZone:RARE_ALLOCATION_TIME_ZONE,
  year:'numeric',
  month:'2-digit',
  day:'2-digit',
});
...
export async function searchTradeMarkets(env,input,{fetchImpl=fetch,timeoutMs=15000}={}) {
  ...
  const normalizedObservations=rows
    ...
    .filter(Boolean);
  const observations=query.rareSourceSearch
    ?await enrichRareSourceAllocations(env,query,normalizedObservations)
  ...
  const visibleRareSource=visibleRareSourceObservation(observations,query,matches);
  const visibleResults=visibleRareSource?[visibleRareSource]:matches.map(item=>({...item,qualifies:true}));
  ...
  await writeMarketHealth(env,{
    ...
    lastReturnedCount:visibleResults.length,
  ...
  return {
    ...
    sourceResultCount:observations.length,
    sourceMatchCount:totalMatches,
    qualifyingMatchCount:matches.length,
    results:visibleResults.map(item=>presentObservation(item,control,query.priority,query)),
  };
}
...
export function normalizeSpanshObservation(item,query) {
  ...
  const entries=(Array.isArray(item.market)?item.market:[]).filter(row=>
    wanted.has(normalizeCommodity(row?.commodity))
  );
  const entry=selectCommodityMarketEntry(entries,query);
...
function selectCommodityMarketEntry(entries,query){
  const rows=Array.isArray(entries)?entries.filter(Boolean):[];
  if(!rows.length)return null;
  if(!query?.rareSourceSearch)return rows[0];
  const canonical=normalizeCommodity(rareTradeCommodityCanonicalName(query.commodity)||query.commodity);
  return [...rows].sort((a,b)=>{
    const supplyDelta=number(b?.supply)-number(a?.supply);
    if(supplyDelta)return supplyDelta;
    const aCanonical=normalizeCommodity(a?.commodity)===canonical?1:0;
    const bCanonical=normalizeCommodity(b?.commodity)===canonical?1:0;
...
export function visibleRareSourceObservation(observations,query,qualifyingMatches=[]){
  if(!query?.rareSourceSearch||!query?.rareSource?.systemName||!query?.rareSource?.stationName)return null;
  if(Array.isArray(qualifyingMatches)&&qualifyingMatches.length)return null;
  ...
  return{
    ...source,
    qualifies:false,
    rareSourceStatus:rareSourceQualificationStatus(source,query),
  };
}

export function rareSourceQualificationStatus(item,query){
  if(!item||!query?.rareSourceSearch)return'';
  if(query.minPad>0&&Number(item.maxLandingPadSize||0)<query.minPad)return'pad_below_threshold';
  if(query.direction==='buy'){
    const allocation=rareAllocationSupply(item);
    const price=rareAllocationBuyPrice(item);
    if(allocation<Number(query.minVolume||0)){
      return allocation<=0?'no_positive_allocation_observed':'allocation_below_threshold';
    }
    if(price<=0)return'price_unavailable';
    if(Number(query.price||0)>0&&price>Number(query.price))return'price_above_threshold';
  }
  return'filters_not_met';
}

export function rareAllocationSupply(item){
  const remembered=Number(item?.allocationSupply);
  if(Number.isFinite(remembered)&&remembered>0)return Math.round(remembered);
  const reported=Number(item?.supply);
  return Number.isFinite(reported)&&reported>0?Math.round(reported):0;
}

export function rareAllocationBuyPrice(item){
  const remembered=Number(item?.allocationBuyPrice);
  if(Number.isFinite(remembered)&&remembered>0)return Math.round(remembered);
  const reported=Number(item?.buyPrice);
  return Number.isFinite(reported)&&reported>0?Math.round(reported):0;
}
...
export function filterAndSortObservations(observations,query,control) {
  ...
  const filtered=observations.filter(item=>{
    ...
    if(query.direction==='buy'){
      const supply=query.rareSourceSearch?rareAllocationSupply(item):Number(item.supply||0);
      const buyPrice=query.rareSourceSearch?rareAllocationBuyPrice(item):Number(item.buyPrice||0);
      if(supply<query.minVolume)return false;
      if(query.price>0&&buyPrice>query.price)return false;
  ...
  const volume=item=>query.direction==='buy'
    ?(query.rareSourceSearch?rareAllocationSupply(item):item.supply)
    :item.demand;
  const price=item=>query.direction==='buy'
    ?(query.rareSourceSearch?rareAllocationBuyPrice(item):item.buyPrice)
    :item.sellPrice;
  const compare={
...
async function cachedSearchFallback(env,query,control,error){
  ...
  const visibleRareSource=visibleRareSourceObservation(candidates,query,matches);
  const visibleResults=visibleRareSource?[visibleRareSource]:matches.map(item=>({...item,qualifies:true}));
  if(!visibleResults.length)return null;

  const fetchedAt=new Date().toISOString();
  const warning=visibleRareSource
    ?'Live market lookup was unavailable. The known rare source is shown from the Mongrel market cache, but its cached observation does not meet the current stock/price filters.'
    :'Live market lookup was unavailable, so these are matching observations from the Mongrel market cache.';
  await writeMarketHealth(env,{
    lastAttemptAt:fetchedAt,
    lastError:friendlySourceError(error),
    lastWarning:warning,
...
async function enrichRareSourceAllocations(env,query,observations){
  const rows=Array.isArray(observations)?observations:[];
  if(!query?.rareSourceSearch||!rows.length)return rows;
  const cached=await readMarketObservationsForQuery(env,query).catch(()=>[]);
  const priorByMarket=new Map(cached.map(item=>[String(item?.marketId||''),item]));
  return rows.map(item=>{
    const prior=priorByMarket.get(String(item?.marketId||''))||null;
    const reportedSupply=Math.max(0,Math.round(Number(item?.supply)||0));
    const reportedAt=iso(item?.observedAt);
    const reportedDay=rareAllocationDay(reportedAt);

    const priorPositiveSupply=Math.max(
      0,
      Math.round(Number(prior?.allocationSupply)||0),
      Number(prior?.supply)>0?Math.round(Number(prior.supply)):0
    );
    const priorPositiveAt=iso(prior?.allocationObservedAt||(Number(prior?.supply)>0?prior?.observedAt:''));
    const priorPositiveDay=rareAllocationDay(priorPositiveAt);
    const priorPositivePrice=Math.max(
      0,
      Math.round(Number(prior?.allocationBuyPrice)||0),
      Number(prior?.supply)>0?Math.round(Number(prior?.buyPrice)||0):0
    );

    let allocationSupply=priorPositiveSupply;
    let allocationObservedAt=priorPositiveAt;
    let allocationBuyPrice=priorPositivePrice;

    if(reportedSupply>0){
      if(!priorPositiveSupply||reportedDay!==priorPositiveDay||reportedSupply>priorPositiveSupply){
        allocationSupply=reportedSupply;
        allocationObservedAt=reportedAt;
        allocationBuyPrice=Math.max(0,Math.round(Number(item?.buyPrice)||0));
      }else if(reportedDay===priorPositiveDay&&reportedSupply===priorPositiveSupply&&Date.parse(reportedAt)>Date.parse(priorPositiveAt||0)){
        allocationObservedAt=reportedAt;
        allocationBuyPrice=Math.max(0,Math.round(Number(item?.buyPrice)||0))||priorPositivePrice;
      }
    }
...
async function rareSourceCacheFallback(env,query,control,{liveRows=[],liveObservations=[]}={}){
  ...
  const visibleRareSource=visibleRareSourceObservation(sourceCandidates,query,matches);
  const visibleResults=visibleRareSource?[visibleRareSource]:matches.map(item=>({...item,qualifies:true}));
  if(!visibleResults.length)return null;
  ...
  const warning=matches.length
    ...
    :'The known rare source is still identified, but the latest cached observation does not meet the saved stock/price filters. The source is shown for reference rather than treated as a qualifying market.';
  await writeMarketHealth(env,{
    lastAttemptAt:fetchedAt,
    lastSuccessfulFetchAt:fetchedAt,
    lastError:'',
...
async function recordMarketObservations(env,commodity,observations) {
  ...
  for(const item of [...previous,...observations]){
    ...
    if(!id)continue;
    const current=map.get(id);
    const merged=rareCommodity?mergeRareStoredObservation(current,item):item;
    if(!current||Date.parse(item.observedAt||0)>=Date.parse(current.observedAt||0)){
      map.set(id,merged);
    }else if(rareCommodity){
      map.set(id,mergeRareStoredObservation(current,item));
    }
  }
...
function mergeRareStoredObservation(current,item){
  const newest=(!current||Date.parse(item?.observedAt||0)>=Date.parse(current?.observedAt||0))
    ?{...(item||{})}
    :{...(current||{})};
  const candidates=[current,item]
    .filter(Boolean)
    .flatMap(source=>{
      const rows=[];
      const allocation=Number(source?.allocationSupply);
      const allocationAt=iso(source?.allocationObservedAt);
      if(Number.isFinite(allocation)&&allocation>0&&allocationAt){
        rows.push({
          supply:Math.round(allocation),
          buyPrice:Math.max(0,Math.round(Number(source?.allocationBuyPrice)||0)),
          observedAt:allocationAt,
          day:rareAllocationDay(allocationAt),
        });
      }
      const supply=Number(source?.supply);
      const observedAt=iso(source?.observedAt);
      if(Number.isFinite(supply)&&supply>0&&observedAt){
        rows.push({
          supply:Math.round(supply),
          buyPrice:Math.max(0,Math.round(Number(source?.buyPrice)||0)),
          observedAt,
          day:rareAllocationDay(observedAt),
        });
      }
      return rows;
    })
  ...
  if(candidates.length){
    const latestDay=[...candidates].sort((a,b)=>b.day.localeCompare(a.day))[0].day;
    positive=candidates
      .filter(row=>row.day===latestDay)
      .sort((a,b)=>b.supply-a.supply||Date.parse(b.observedAt)-Date.parse(a.observedAt))[0]||null;
  }

  return{
    ...newest,
    commanderSensitiveSupply:true,
    reportedSupply:Math.max(0,Math.round(Number(newest?.supply)||0)),
    allocationSupply:positive?.supply||0,
    allocationObservedAt:positive?.observedAt||'',
    allocationBuyPrice:positive?.buyPrice||0,
  };
}

function rareAllocationDay(value){
  const time=Date.parse(value||'');
  if(!Number.isFinite(time))return'';
  const parts={};
  for(const part of RARE_ALLOCATION_DAY_FORMATTER.formatToParts(new Date(time))){
