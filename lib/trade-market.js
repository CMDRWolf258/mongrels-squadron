...
export async function searchTradeMarkets(env,input,{fetchImpl=fetch,timeoutMs=15000}={}) {
  ...
  if(query.rareSourceSearch)warningParts.push('Rare-source search: radius is ignored so the unique source is not excluded; distance is still measured from '+query.referenceSystem+'.');
