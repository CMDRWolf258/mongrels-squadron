...
function jobSnapshot(value={}) {
  ...
  return {
    ...
    fundingMode:['none','member','squad'].includes(value.fundingMode)?value.fundingMode:'squad',
    fundingApprovalStatus:['not_required','pending','approved','rejected'].includes(value.fundingApprovalStatus)?value.fundingApprovalStatus:'approved',
    rewardBudgetMillions:numberOrZero(value.rewardBudgetMillions),
    postingOwnerId:clean(value.postingOwnerId).slice(0,160),
    postingOwnerName:clean(value.postingOwnerName).slice(0,120),
    postingCommander:clean(value.postingCommander).slice(0,80),
    fundingPayerOwnerId:clean(value.fundingPayerOwnerId).slice(0,160),
    fundingPayerName:clean(value.fundingPayerName).slice(0,120),
    fundingApprovedAt:iso(value.fundingApprovedAt),
    fundingApprovedBy:clean(value.fundingApprovedBy).slice(0,120),
    fundingNote:clean(value.fundingNote).slice(0,500),
...
function jobSummary(value={}) {
  ...
  return {
    ...
    fundingMode:job.fundingMode,
    fundingApprovalStatus:job.fundingApprovalStatus,
    rewardBudgetMillions:job.rewardBudgetMillions,
