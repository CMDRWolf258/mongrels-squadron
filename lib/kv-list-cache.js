...
export async function rememberKeyInListCache(env,{
  cacheKey,
  prefix,
  key,
  maxAgeSeconds=3600,
  ...
  if(!kv||typeof kv.get!=='function'||typeof kv.put!=='function'||!cleanCacheKey||!cleanKey||!cleanKey.startsWith(cleanPrefix))return false;
  try{
    const cached=await kv.get(cleanCacheKey,{type:'json'});
    const cachedAt=Date.parse(cached?.cachedAt||'');
    const maxAgeMs=Math.max(60,Math.floor(Number(maxAgeSeconds)||3600))*1000;
    ...
    const keys=[
      cleanKey,
      ...cached.keys.map(value=>String(value||'')).filter(value=>value.startsWith(cleanPrefix)&&value!==cleanKey),
    ].slice(0,cap);
    await kv.put(cleanCacheKey,JSON.stringify({
      ...cached,
      version:CACHE_VERSION,
    ...
    console.error('Could not update KV list cache',cleanCacheKey,error);
    return false;
  }
}
