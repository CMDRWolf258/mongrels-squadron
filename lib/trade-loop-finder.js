...
export function evaluateSpecificLoop(stations,routeLegs,query) {
  ...
  for(const prior of Array.isArray(routeLegs)?routeLegs:[]){
    ...
    if(query?.mongrelOnly&&(
      norm(source.stationControllingFaction)!==norm(MONGREL_FACTION)
      ||norm(destination.stationControllingFaction)!==norm(MONGREL_FACTION)
    ))return{valid:false,loopProfit:0,legs:[]};
