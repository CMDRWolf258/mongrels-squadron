...
export function buildRewardSummaryDiscordPayload(view,{adminUrl='',rewardsUrl=''}={}){
  ...
  const admin=clean(adminUrl),rewards=clean(rewardsUrl);
  return{
    ...
    embeds:[{
      ...
      ...(admin?{url:admin}:{}),
      description:[
        'Verified squad-funded rewards quietly accumulate here. Individual Discord cards are reserved for members who explicitly request payout.',
        admin?'[Open Reward Administration →]('+admin+')':'',
        rewards?'Members: [open Rewards account →]('+rewards+')':'',
      ].filter(Boolean).join('\n\n'),
      color:COLORS.summary,
      fields:fields.slice(0,25),
      footer:{text:'Regiment of Imperial Mongrels · squad-funded rewards only · mentions disabled'},
    }],
  };
}
...
export function buildPayoutRequestDiscordPayload(member,{
  ...
  if(!resolved){
    ...
    fields.push({name:'Settlement',value:'The requested reward entries have been marked PAID. This card will leave the operations channel on the next Rewards sync.',inline:false});
    ...
    fields.push({name:'Request',value:'This payout request is no longer active. The card will leave the operations channel on the next Rewards sync.',inline:false});
  }
  ...
  return{
    ...
    embeds:[{
      ...
      description:[
        ...
        admin?'[Open Reward Administration →]('+admin+')':'',
        rewards?'Member account: [Rewards →]('+rewards+')':'',
      ].filter(Boolean).join('\n\n'),
