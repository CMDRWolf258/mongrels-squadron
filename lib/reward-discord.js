...
export async function syncRewardDiscordBoard(env,{
  ...
  const personalSummary=await syncPersonalRewardSummaryDiscord(env,{
    state,view,rewardsUrl,createMissing:createMissing||boardSeededHere,
  });
  ...
  return boardResult({configured:true,summary,personalSummary,results,view});
}
...
export async function syncRewardSummaryDiscord(env,{
  ...
  const payload=buildSquadRewardSummaryDiscordPayload(view,{adminUrl,rewardsUrl});
...
export function buildSquadRewardSummaryDiscordPayload(view,{adminUrl='',rewardsUrl=''}={}){
  ...
  if(requestLines.length){
    for(const [index,chunk] of chunkLines(requestLines,900).entries()){
      fields.push({
        name:index===0?'Squad Payout Requests':'Squad Payout Requests · continued',
        value:chunk.join('\n'),
        inline:false,
      });
    }
    ...
    fields.push({
      name:'Squad Payout Requests',
      value:'No members are currently waiting to collect a squad-funded payout.',
      inline:false,
    });
  }
  ...
  if(owedLines.length){
    for(const [index,chunk] of chunkLines(owedLines.slice(0,40),900).entries()){
      fields.push({
        name:index===0?'Squad-Funded Outstanding Balances':'Squad-Funded Outstanding Balances · continued',
        value:chunk.join('\n'),
        inline:false,
      });
    }
    if(owedLines.length>40){
      fields.push({
        name:'More Squad-Funded Rewards',
        value:'+'+(owedLines.length-40)+' additional squad-funded CMDR balances are available in the Rewards account.',
        inline:false,
      });
    }
  }else{
    fields.push({
      name:'Squad-Funded Outstanding Balances',
      value:'No squad-funded reward balance is currently outstanding.',
      inline:false,
    });
  }
  ...
  return{
    ...
    embeds:[{
      title:'💰 Squad-Funded Rewards',
      ...(rewards?{url:rewards}:{}),
      description:[
        '**Paid from the Regiment treasury.** Personal jobs are tracked separately below in their own persistent Discord message.',
        rewards?'[Open Rewards account →]('+rewards+')':'',
      ].filter(Boolean).join('\n\n'),
      color:COLORS.summary,
      fields:fields.slice(0,25),
      footer:{text:'Regiment of Imperial Mongrels · squad treasury only · mentions disabled'},
    }],
  };
}

export function buildPersonalRewardSummaryDiscordPayload(view,{rewardsUrl=''}={}){
  const personal=Array.isArray(view?.personalOutstanding)?view.personalOutstanding:[];
  const personalOwedCredits=personal.reduce((sum,row)=>sum+(Number(row?.owedCredits)||0),0);
  const personalSentCredits=personal.reduce((sum,row)=>sum+(Number(row?.paymentSentCredits)||0),0);
  const personalOutstandingCredits=personal.reduce((sum,row)=>sum+(Number(row?.unsettledCredits)||0),0);
  const recipients=new Set(personal.map(row=>clean(row?.ownerId)||clean(row?.displayName)).filter(Boolean));
  const payers=new Set(personal.map(row=>clean(row?.payerOwnerId)||clean(row?.payerDisplayName)).filter(Boolean));
  const fields=[];
  const lines=personal.map(row=>personalRewardSummaryLine(row));

  if(lines.length){
    const overview=[
      '**'+formatCredits(personalOutstandingCredits)+'** unsettled',
      recipients.size+' recipient'+(recipients.size===1?'':'s'),
      payers.size+' payer'+(payers.size===1?'':'s'),
      personalSentCredits>0?formatCredits(personalSentCredits)+' sent / awaiting confirmation':'',
      personalOwedCredits>0?formatCredits(personalOwedCredits)+' still owed':'',
    ].filter(Boolean).join(' · ');
    for(const [index,chunk] of chunkLines(lines.slice(0,40),850).entries()){
      fields.push({
        name:index===0?'Outstanding Personal Jobs':'Outstanding Personal Jobs · continued',
        value:[index===0?overview:'',...chunk].filter(Boolean).join('\n'),
        inline:false,
      });
    }
    if(lines.length>40){
      fields.push({
        ...
        value:'+'+(lines.length-40)+' additional personal-job reward groups are tracked in the Rewards ledger.',
        inline:false,
      });
    }
  }else{
    fields.push({
      name:'Outstanding Personal Jobs',
  ...
  return{
    username:'Mongrel Mission Control',
    embeds:[{
      title:'🐺 Personal Job Rewards',
      ...(rewards?{url:rewards}:{}),
      description:'**Separate from the Squad Treasury.** These rewards are funded by individual Mongrels. The named payer is responsible for settlement.',
      color:COLORS.requested,
      fields:fields.slice(0,25),
      footer:{text:'Regiment of Imperial Mongrels · individually funded jobs · mentions disabled'},
    }],
  };
}

export function buildRewardSummaryDiscordPayload(view,options={}){
  return buildSquadRewardSummaryDiscordPayload(view,options);
}

async function syncPersonalRewardSummaryDiscord(env,{
  state=null,
  view,
  rewardsUrl='',
  createMissing=false,
}={}){
  if(!discordSquadPayoutsConfigured(env))return summaryResult({configured:false,attempted:false,ok:false,mode:'not_configured',kind:'personal'});
  if(!storageReady(env))return summaryResult({configured:true,attempted:false,ok:false,mode:'tracking_unavailable',kind:'personal'});
  const working=state||await readState(env);
  const webhookId=discordSquadPayoutsWebhookId(env);
  const tracked=working.personalSummary;
  if((!tracked?.messageId||tracked.webhookId!==webhookId)&&!createMissing){
    return summaryResult({configured:true,attempted:false,ok:true,mode:'not_tracked',kind:'personal'});
  }

  const payload=buildPersonalRewardSummaryDiscordPayload(view,{rewardsUrl});
  const fingerprint=payloadFingerprint(payload);
  if(tracked?.messageId&&tracked.webhookId===webhookId&&tracked.fingerprint===fingerprint){
    return summaryResult({configured:true,attempted:false,ok:true,mode:'unchanged',kind:'personal',messageId:tracked.messageId});
  }
  ...
  try{
    ...
    await writeState(env,working);
    return summaryResult({configured:true,attempted:true,ok:true,mode,kind:'personal',messageId:working.personalSummary.messageId});
  }catch(error){
    console.error('Could not sync Personal Job Rewards summary to Discord',error);
    return summaryResult({
      configured:true,attempted:true,ok:false,mode:'failed',kind:'personal',
      error:clean(error?.message)||'discord_personal_rewards_summary_sync_failed',
      discordStatus:Number.isFinite(Number(error?.status))?Number(error.status):null,
    });
  }
}
...
async function migrateLegacyOperationsTracking(env,state,newWebhookId){
  ...
  if(state.personalSummary?.messageId&&state.personalSummary.webhookId===oldOperationsWebhookId){
    try{await deleteOperationsDiscordMessage(env,state.personalSummary.messageId);}
    catch(error){
      if(Number(error?.status)!==404){
        console.error('Could not remove legacy Personal Job Rewards summary from Operations Discord',error);
        throw error;
      }
    }
    state.personalSummary=null;
    changed=true;
  }
...
async function readState(env){
  try{
    ...
    const summary=rawSummary?normalizeTrackedSummary(rawSummary):null;
    const rawPersonalSummary=stored?.personalSummary&&typeof stored.personalSummary==='object'?stored.personalSummary:null;
    const personalSummary=rawPersonalSummary?normalizeTrackedSummary(rawPersonalSummary):null;
    return{version:2,summary,personalSummary,requestCards:cards};
  }catch(error){
    console.error('Could not read Rewards Discord state',error);
    return{version:2,summary:null,personalSummary:null,requestCards:{}};
  }
}
async function writeState(env,state){
  await env.DAILY_ORDERS.put(STATE_KEY,JSON.stringify({
    ...
    personalSummary:state.personalSummary||null,
    requestCards:state.requestCards||{},
  }));
}
function normalizeTrackedSummary(row={}){
  return{
    messageId:clean(row.messageId),
    webhookId:clean(row.webhookId),
    fingerprint:clean(row.fingerprint),
...
function personalRewardSummaryLine(row){
  ...
  return '• **'+escapeMarkdown(row?.displayName||'Mongrel CMDR')+' — '+formatCredits(row?.unsettledCredits)+'**'
    +'\n  ↳ **Paid by:** '+escapeMarkdown(row?.payerDisplayName||'Posting CMDR')+' · **Status:** '+status
...
function boardResult({configured,error='',summary=null,personalSummary=null,results=[],view=null}={}){
  ...
  return{
    ...
    personalOutstandingCredits:Number(view?.summary?.personalOutstandingCredits)||0,
    personalOutstandingGroups:Array.isArray(view?.personalOutstanding)?view.personalOutstanding.length:0,
  };
}
