...
export function buildRewardSummaryDiscordPayload(view,{adminUrl='',rewardsUrl=''}={}){
  ...
  const personalRecipients=new Set(personal.map(row=>clean(row?.ownerId)||clean(row?.displayName)).filter(Boolean));
  const personalPayers=new Set(personal.map(row=>clean(row?.payerOwnerId)||clean(row?.payerDisplayName)).filter(Boolean));
  ...
  if(personalLines.length){
    const overview=[
      ...
      Number(personalRecipients.size)+' recipient'+(personalRecipients.size===1?'':'s'),
      Number(personalPayers.size)+' payer'+(personalPayers.size===1?'':'s'),
    ...
    for(const [index,chunk] of chunkLines(personalLines.slice(0,40),850).entries()){
      personalFields.push({
    ...
    if(personalLines.length>40){
      personalFields.push({
        ...
        value:'+'+(personalLines.length-40)+' additional personal-job reward groups are tracked in the Rewards ledger.',
        inline:false,
      });
    }
  }else{
    ...
    embeds:[
      {
        ...
        ...(rewards?{url:rewards}:{}),
        description:[
          '**Paid from the Regiment treasury.** Personal jobs are not included in any totals on this card.',
          rewards?'[Open Rewards account →]('+rewards+')':'',
        ].filter(Boolean).join('\n\n'),
        color:COLORS.summary,
        fields:squadFields.slice(0,25),
      ...
      },
      {
        ...
        fields:personalFields.slice(0,25),
