...
export async function buildRewardDryRun({
  ...
  const publicationCycleId=cycleId;
  ...
  for(const accountRow of Array.isArray(accounts)?accounts:[]){
    ...
    for(const preview of previews){
      ...
      const sourceCycleId=clean(preview.sourceCycleId)||cycleId;
      const provenanceCycleId=clean(order?.rewardHistoryPublicationCycleId)||publicationCycleId;
...
function collectOrderMatches(matched,preview){
  ...
  const wantedCycle=clean(preview?.sourceCycleId);
  const rows=[];
  ...
  for(const event of Array.isArray(matched?.events)?matched.events:[]){
    for(const match of Array.isArray(event?.orderMatches)?event.orderMatches:[]){
      ...
      if(wantedCycle&&clean(match?.sourceCycleId)!==wantedCycle)continue;
      rows.push({
