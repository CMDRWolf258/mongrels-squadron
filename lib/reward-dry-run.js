...
export async function buildRewardDryRun({
  ...
  for(const accountRow of Array.isArray(accounts)?accounts:[]){
    ...
    for(const preview of previews){
      ...
      const sourceEventIds=unique(preview.sourceEventIds||[]);
      ...
      const id=await deterministicRewardId(ownerId,sourceCycleId,logicalKey);
      const blockers=[];
      ...
      const duplicateSuppressed=deltaCredits<=0&&entitlementCredits>0;
      const readyForLive=blockers.length===0&&deltaCredits>0;
      const plannedEntry={
        version:2,
        id,
        ownerId,
        ...
        ruleSnapshot:ruleSnapshot(preview,rewardSettings),
        status:'owed',
...
async function deterministicRewardId(ownerId,cycleId,logicalKey) {
  const digest=await digestText(['verified-order-v1',ownerId,cycleId,logicalKey].join('|'));
