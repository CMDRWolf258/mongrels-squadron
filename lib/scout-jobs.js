...
export function normalizeScoutJobSettings(value={}){
  ...
  const lastScoutLocation=viewerLastScoutLocation(boundTokens,rows);
  ...
  try{
    ...
    return rows
      ...
      .map(row=>({
        ...
        lastSystem:clean(row.lastSystem).slice(0,140),
        lastSeenAt:iso(row.lastSeenAt),
        lastEventAt:iso(row.lastEventAt),
        lastCoords:normalizeCoordinates(row.lastCoords),
      }));
  }catch(error){
...
export async function buildScoutJobBoard(env,{
  ...
  for(const raw of Array.isArray(systems)?systems:[]){
    ...
    const liveCoords=normalizeCoordinates(raw?.coords);
    const scoutCoords=normalizeCoordinates(snapshot?.starPos||snapshot?.coords);
    const coords=scoutCoords||liveCoords;
    const coordinateSource=scoutCoords?'Live Scout':(liveCoords?(clean(raw?.coordsSource)||'EDSM'):'');
  ...
  return {
    ...
    summary:{
      ...
      coordinates:rows.filter(row=>normalizeCoordinates(row.coords)).length,
    },
    jobs:rows,
    settings,
  };
...
function viewerLastScoutLocation(tokens,rows){
  const candidates=[...(Array.isArray(tokens)?tokens:[])]
    .filter(row=>clean(row?.lastSystem))
    .sort((a,b)=>String(b?.lastEventAt||b?.lastSeenAt||'').localeCompare(String(a?.lastEventAt||a?.lastSeenAt||'')));
  for(const token of candidates){
    const system=clean(token.lastSystem);
    const row=(rows||[]).find(item=>norm(item?.system)===norm(system));
    const coords=normalizeCoordinates(token.lastCoords)||normalizeCoordinates(row?.coords);
    if(!coords)continue;
    return{
      system:row?.system||system,
      coords,
      observedAt:token.lastEventAt||token.lastSeenAt||null,
      coordinateSource:normalizeCoordinates(token.lastCoords)?'Live Scout':(row?.coordinateSource||'System catalog'),
    };
  }
  return null;
}
