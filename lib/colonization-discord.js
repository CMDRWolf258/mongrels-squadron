...
export async function buildColonizationJobView(env,job){
  ...
  const views=await buildColonizationJobViews(env,jobs);
  return views.find(view=>String(view.id)===String(job?.id))||{...job,squadTons:0,payableTons:0,squadEvents:0,contributorCount:0,ambiguousEvents:0,suppressedEvents:0,lastContributionAt:null,members:[]};
}
