...
export async function syncColonizationMutationDiscord(env,{
  ...
  if(!discordOperationsConfigured(env))return mutationResult({configured:false});
...
export async function syncColonizationArchiveJobDiscord(env,{
  ...
  if(!discordColonizationArchiveConfigured(env))return archiveResult({configured:false,attempted:false,ok:false,mode:'not_configured'});
  ...
  try{
    ...
    state.jobs[job.id]={
      messageId:clean(sent.messageId),
      webhookId:discordColonizationArchiveWebhookId(env),
...
export async function syncAllCompletedColonizationArchiveDiscord(env,{
  ...
  const store=await readColonizationJobs(env);
  const jobs=(Array.isArray(store?.jobs)?store.jobs:[]).filter(job=>clean(job?.status)==='completed');
  const views=await buildColonizationJobViews(env,jobs);
  const byId=new Map(views.map(view=>[String(view.id),view]));
  const results=[];
...
export async function syncColonizationSummaryDiscord(env,{
  ...
  if(!discordOperationsConfigured(env))return summaryMessageResult({configured:false,attempted:false,ok:false,mode:'not_configured'});
  ...
  try{
    ...
    console.error('Could not sync Colonization Operations summary to Discord',error);
...
export async function syncAllColonizationJobsDiscord(env,{
  ...
  if(!discordOperationsConfigured(env))return syncSummary({configured:false});
...
export function buildColonizationJobDiscordPayload(job,{controlUrl=''}={}){
  ...
  return {
    ...
    embeds:[{
      ...
      description:[
        status==='completed'
          ? 'Colonization hauling job completed. This card will leave the operations channel on the next Colonization sync.'
          : status==='paused'
...
export function buildColonizationSummaryDiscordPayload(views,{controlUrl=''}={}){
  ...
  if(lines.length){
    ...
    fields.push({
      ...
      value:'No active Colonization Jobs. Completed work remains available in Mission Control history.',
      inline:false,
    });
  }
  ...
  return {
    username:'Mongrel Mission Control',
    embeds:[{
      title:'Colonization Operations',
      ...(link?{url:link}:{}),
      description:[
        'Live squad colonization work. Individual active-job cards carry full details; completed jobs leave this operations channel after their completion state has been shown.',
        link?'[Open Colonization Control →]('+link+')':'',
      ].filter(Boolean).join('\n\n'),
      color:COLORS.summary,
      fields:fields.slice(0,25),
      footer:{text:'Regiment of Imperial Mongrels · persistent operations summary'},
...
export function buildColonizationJobRemovedPayload(job,{controlUrl=''}={}){
  ...
  return {
    ...
    embeds:[{
      ...
      footer:{text:'Regiment of Imperial Mongrels · Mission Control history retained'},
    }],
  };
}
...
function mutationResult({configured,error='',job=null,summary=null}={}){
  return{
    ...
    job,
    summary,
    ok:Boolean((job?.ok??true)&&(summary?.ok??true)&&!error),
    attempted:Boolean(job?.attempted||summary?.attempted),
    mode:job?.mode||summary?.mode||'unchanged',
  };
}
...
function archiveSummary({configured,error='',results=[]}={}){
  ...
  return{
    ...
    error:error||null,
    results:rows,
    archived:rows.filter(row=>row.mode==='archived').length,
