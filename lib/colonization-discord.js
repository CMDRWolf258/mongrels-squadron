...
export async function syncColonizationJobDiscord(env,{
  ...
  if(status==='completed'&&!tracked?.messageId){
    return result({configured:true,attempted:false,ok:true,mode:'completed_untracked',jobId:job.id});
  }
  ...
  if(!OPERATIONAL_STATUSES.has(status)&&status!=='completed'){
    return result({configured:true,attempted:false,ok:true,mode:'not_operational',jobId:job.id});
  }
