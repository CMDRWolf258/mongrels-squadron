...
export async function syncColonizationJobDiscord(env,{
  ...
  const view=await buildColonizationJobView(env,job);
  const payload=buildColonizationJobDiscordPayload(view,{actor,controlUrl});
...
export async function syncAllColonizationJobsDiscord(env,{
  ...
  for(const job of candidates){
    results.push(await syncColonizationJobDiscord(env,{job,actor,controlUrl,createMissing}));
  }
...
export async function buildColonizationJobView(env,job){
  ...
  const jobsStore=await readColonizationJobs(env);
  const arbitrationByMember=memberRows.map(member=>({
    ...member,
    arbitration:arbitrateColonizationContributions(member.events,jobsStore.jobs),
  }));
