...
function qualifies(order,component,event,cycleOffset) {
  ...
  const removedAt=Date.parse(order?.rewardHistoryRemovedAt||'');
  const cycleEnd=Date.parse(cycle.cycleEndsAt||'');
  // An order published partway through a BGS cycle can still recover work from
  // earlier in that same cycle, but it must never reach backward into cycles
  // that had already ended before the order existed.
  if(Number.isFinite(createdAt)&&Number.isFinite(cycleEnd)&&createdAt>=cycleEnd)return false;
  // Archived orders are eligible only for work completed before their removal.
  // This is what lets a late Elite sync recover Cycle 1 work without allowing
  // that old order to absorb activity performed after it left Mission Control.
  if(Number.isFinite(removedAt)&&Number.isFinite(eventTime)&&eventTime>=removedAt)return false;
  // Scout verification belongs to the BGS work cycle, not to the moment an
  // order happened to be published. A system plan can be generated/revised
  // after work was already completed in the same tick cycle; exact
  // type/system/faction matching still prevents unrelated work from attaching.
  const start=Date.parse(cycle.cycleStartedAt||cycle.acceptFromAt||'');
