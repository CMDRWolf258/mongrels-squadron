...
import { EXPLORATION_ACTIVITY_ID, eligibleExplorationRoutes, getExplorationRoute } from './pathway-exploration.js';
import { EXOBIOLOGY_ACTIVITY_ID, eligibleExobiologyRoutes, getExobiologyRoute } from './pathway-exobiology.js';
import { PVE_ACTIVITY_ID, eligiblePveRoutes, getPveRoute } from './pathway-pve.js';
import { PVP_ACTIVITY_ID, eligiblePvpRoutes, getPvpRoute } from './pathway-pvp.js';
import { OPERATIONS_ACTIVITY_ID, eligibleOperationsRoutes, getOperationsRoute } from './pathway-operations.js';
import { COLONIZATION_ACTIVITY_ID, eligibleColonizationRoutes, getColonizationRoute } from './pathway-colonization.js';
import { SQUADRON_COORDINATION_ACTIVITY_ID, eligibleSquadronCoordinationRoutes, getSquadronCoordinationRoute } from './pathway-squadron-coordination.js';
import {
...
import { buildPowerThermalEngineeringDependencyNodes } from './engineering-campaign-power-thermal.js';
import { buildWeaponEngineeringDependencyNodes } from './engineering-campaign-weapons.js';
...
const PROVIDERS = {
  [PVE_ACTIVITY_ID]: {
    ...
    keywords:['pve pathway','pve assignment','pve task','my pve','current pve','pve combat','combat assignment','bounty hunter route','conflict zone route'],
  },
  [PVP_ACTIVITY_ID]: {
    label:'PvP',
    seedVersion:'pvp-v1',
    ...
    keywords:['pvp pathway','pvp assignment','pvp task','my pvp','current pvp','pvp step','duelist route','precision fighter','wing fighter','pvp training'],
  },
  [OPERATIONS_ACTIVITY_ID]: {
    ...
    keywords:['operations pathway','operations assignment','operations task','my operations','current operations','operation runner','merc coin','hard operation','operation scenario','multi-role operator'],
  },
  [COLONIZATION_ACTIVITY_ID]: {
    label:'Colonization',
    seedVersion:'colonization-v1',
    eligibleRoutes:eligibleColonizationRoutes,
    getRoute:getColonizationRoute,
    keywords:['colonization pathway','colonisation pathway','colonization assignment','colonisation assignment','colonization task','colonisation task','my colonization','my colonisation','current colonization','current colonisation','construction operator','system architect route','colony developer'],
  },
  [SQUADRON_COORDINATION_ACTIVITY_ID]: {
    ...
    keywords:['squadron coordination','coordination pathway','coordination assignment','coordination task','my coordination','current coordination','daily orders','mission control pathway','squad contributor','cross-activity coordinator','mission planner','squad operations pathway'],
  },
  [AX_ACTIVITY_ID]: {
    label:'Anti-Xeno',
    seedVersion:'ax-v2',
  ...
  [EXPLORATION_ACTIVITY_ID]: {
    ...
    keywords:['exploration pathway','exploration assignment','exploration task','my exploration','current exploration','exploration step','my exploration step'],
  },
  [EXOBIOLOGY_ACTIVITY_ID]: {
    ...
    keywords:['exobiology pathway','exobiology assignment','exobiology task','my exobiology','current exobiology','exobiology step','my exobiology step','bio survey task'],
  },
};
...
const ACTIVITY_LABELS = {
  ...
  powerplay:'Powerplay',
  operations:'Squadron Coordination',
};
...
export function assistantPathwayIntent(message) {
  ...
  return [
    ...
    'power and heat campaign','power & heat','improve power and heat','improve power & heat','power plant campaign','my power plant','thermal campaign',
    'weapon package campaign','improve weapon package','my weapon package','weapon campaign','hardpoint package','hardpoint campaign',
...
export async function buildAssistantPathwayContext(env, session, message) {
  ...
  const assignments = [];
  for (const activity of activityIds.slice(0, 12)) {
  ...
  const wantsEngineeringCampaign = requested.includes(ENGINEERING_ACTIVITY_ID)
    || /campaign|shield|engineering|jump range|\bfsd\b|mobility|thrusters|dirty drives|clean drives|power distributor|\bdistributor\b|capacitor|power plant|power and heat|power & heat|thermal|weapon package|hardpoint|multi-cannon|multicannon|rail gun|laser|plasma accelerator|overcharged|low emissions|armoured|current step|this step|prep tracker/.test(query)
    || requested.length === 0;
...
function requestedActivities(query) {
  ...
  if (/\bpve\b|npc combat|bounty hunt|conflict zone|combat zone/.test(query) && !matches.includes(PVE_ACTIVITY_ID)) matches.push(PVE_ACTIVITY_ID);
  if (/\bpvp\b|duel|player combat|open play combat|wing pvp|fixed weapon practice/.test(query) && !matches.includes(PVP_ACTIVITY_ID)) matches.push(PVP_ACTIVITY_ID);
  if (/operation runner|merc coin|operations pathway|operations assignment|operations task|my operations|current operations|hard operation|operation scenario|multi-role operator/.test(query) && !matches.includes(OPERATIONS_ACTIVITY_ID)) matches.push(OPERATIONS_ACTIVITY_ID);
  if (/colonization pathway|colonisation pathway|colonization assignment|colonisation assignment|colonization task|colonisation task|my colonization|my colonisation|current colonization|current colonisation|construction operator|colony developer|system architect route/.test(query) && !matches.includes(COLONIZATION_ACTIVITY_ID)) matches.push(COLONIZATION_ACTIVITY_ID);
  if (/squadron coordination|coordination pathway|coordination assignment|coordination task|my coordination|current coordination|daily orders|mission control pathway|squad contributor|cross-activity coordinator|mission planner|squad operations pathway/.test(query) && !matches.includes(SQUADRON_COORDINATION_ACTIVITY_ID)) matches.push(SQUADRON_COORDINATION_ACTIVITY_ID);
  ...
  if (/\bexploration\b|explorer|survey route|deep space|neutron route/.test(query) && !matches.includes(EXPLORATION_ACTIVITY_ID)) matches.push(EXPLORATION_ACTIVITY_ID);
  if (/\bexobiology\b|\bexobio\b|bio survey|genetic sampler|vista genomics|biological signal|biological heatmap/.test(query) && !matches.includes(EXOBIOLOGY_ACTIVITY_ID)) matches.push(EXOBIOLOGY_ACTIVITY_ID);
  if (/\bengineering\b|shield campaign|improve shields|jump range campaign|improve jump range|\bfsd\b|mobility campaign|improve mobility|improve speed|thrusters|dirty drives|clean drives|power distributor|\bdistributor\b|capacitor campaign|power plant|power and heat|power & heat|thermal campaign|weapon package|hardpoint|multi-cannon|multicannon|rail gun|laser|plasma accelerator|overcharged|low emissions|armoured|campaign planner/.test(query) && !matches.includes(ENGINEERING_ACTIVITY_ID)) matches.push(ENGINEERING_ACTIVITY_ID);
