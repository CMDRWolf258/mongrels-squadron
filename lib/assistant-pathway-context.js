...
const PROVIDERS = {
  ...
  [ENGINEERING_ACTIVITY_ID]: {
    ...
    keywords:['engineering pathway','engineering assignment','my engineering','engineering task'],
  },
};
...
export function assistantPathwayIntent(message) {
  ...
  return [
    ...
    'campaign planner','engineering campaign','shield campaign','improve shields','prep tracker','engineering prep','how do i do this','how do i complete this',
...
export async function buildAssistantPathwayContext(env, session, message) {
  ...
  const wantsEngineeringCampaign = requested.includes(ENGINEERING_ACTIVITY_ID)
    || /campaign|shield|engineering|current step|this step|prep tracker/.test(query)
    || requested.length === 0;
...
function requestedActivities(query) {
  ...
  if (/\bengineering\b|shield campaign|improve shields|campaign planner/.test(query) && !matches.includes(ENGINEERING_ACTIVITY_ID)) matches.push(ENGINEERING_ACTIVITY_ID);
