...
export function assistantPathwayIntent(message) {
  ...
  return [
    ...
    'prep tracker','engineering prep','how do i do this','how do i complete this',
...
export async function buildAssistantPathwayContext(env, session, message) {
  ...
  const wantsEngineeringCampaign = requested.includes(ENGINEERING_ACTIVITY_ID)
    || /campaign|shield|engineering|jump range|\bfsd\b|mobility|thrusters|dirty drives|clean drives|current step|this step|prep tracker/.test(query)
    || requested.length === 0;
  ...
  return {
    authority:'Read-only personalized context. My Pathway and the Campaign Planner remain authoritative for saved progress and completion.',
...
function requestedActivities(query) {
  ...
  if (/\btrade\b|\bhauling\b/.test(query) && !matches.includes(TRADE_ACTIVITY_ID)) matches.push(TRADE_ACTIVITY_ID);
  ...
  if (/\bengineering\b|shield campaign|improve shields|jump range campaign|improve jump range|\bfsd\b|mobility campaign|improve mobility|improve speed|thrusters|dirty drives|clean drives|campaign planner/.test(query) && !matches.includes(ENGINEERING_ACTIVITY_ID)) matches.push(ENGINEERING_ACTIVITY_ID);
