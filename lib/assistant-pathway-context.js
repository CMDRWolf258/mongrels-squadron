...
export async function buildAssistantPathwayContext(env, session, message) {
  ...
  const wantsEngineeringCampaign = requested.includes(ENGINEERING_ACTIVITY_ID)
    || /campaign|shield|engineering|jump range|\bfsd\b|mobility|thrusters|dirty drives|clean drives|power distributor|\bdistributor\b|capacitor|power plant|power and heat|power & heat|thermal|overcharged|low emissions|armoured|current step|this step|prep tracker/.test(query)
    || requested.length === 0;
