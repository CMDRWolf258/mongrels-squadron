...
export async function buildMissionControlData(request, env, session) {
  ...
  const [live, strategy, scoutState] = await Promise.all([
    fetchStaticJson(request, '/data/live-bgs.json'),
    readBgsStrategy(env),
    readScoutSnapshots(env),
  ]);

  const systems = mergeSystems(live, strategy, scoutState);
...
function mergeSystems(live, strategy, scoutState = { systems: {} }) {
  ...
  for (const observed of liveRows) {
    ...
    const row = deriveSystem({ ...withScout, present: withScout.present !== false && !withScout.formerPresence, ...policy, name: withScout.name }, priorityNames.has(key), strategy.defaults);
    merged.push(row);
    seen.add(key);
  }
  ...
  for (const snapshot of scoutByName.values()) {
    ...
    merged.push(deriveSystem({ ...observed, ...policy, name }, priorityNames.has(key), strategy.defaults));
    seen.add(key);
  }
