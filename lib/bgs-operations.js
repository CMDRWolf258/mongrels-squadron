import { readScoutSnapshots } from './scout-jobs.js';
...
export async function buildMissionControlData(request, env, session) {
  ...
  const [live, strategy, scoutState, timingControl] = await Promise.all([
  ...
  const systems = mergeSystems(live, strategy, scoutState, timingControl);
  ...
  return {
    ...
    meta: {
      ...
      scoutActiveCount: systems.filter(system => system.freshestSource === 'Mongrel Scout / EDMC').length,
      newestScoutAt: systems.map(system => system.scoutUpdatedAt).filter(Boolean).reduce((latest,value) => newerTimestamp(latest,value), null),
    },
    systems,
...
function compactSystem(system) {
  return {
    ...
    freshestUpdatedAt: system.freshestUpdatedAt,
    freshestSource: system.freshestSource,
    scoutUpdatedAt: system.scoutUpdatedAt,
    scoutLabel: system.scoutLabel,
    dataCondition: system.dataCondition,
    freshnessCycle: system.freshnessCycle,
...
function mergeSystems(live, strategy, scoutState = { systems: {} }, timingControl = {}) {
  ...
  for (const [name,snapshot] of Object.entries(scoutState?.systems && typeof scoutState.systems === 'object' ? scoutState.systems : {})) {
    if (!snapshot || typeof snapshot !== 'object') continue;
    const systemName = String(snapshot.system || name || '').trim();
    if (!systemName) continue;
    scoutByName.set(normalizeName(systemName), snapshot);
  }
  const merged = [];
  const seen = new Set();
  ...
  for (const observed of liveRows) {
    ...
    const withScout = mergeScoutSnapshot(observed, scoutByName.get(key));
    const row = deriveSystem({ ...withScout, present: withScout.present !== false && !withScout.formerPresence, ...policy, name: withScout.name }, priorityNames.has(key), strategy.defaults, timingControl);
    merged.push(row);
    seen.add(key);
  }

  // A recent Live Scout snapshot is valid system-board evidence even before the
  // public EDDN presence snapshot catches up. Keep that system visible when the
  // snapshot itself confirms the Mongrels are present.
  for (const snapshot of scoutByName.values()) {
    const name = String(snapshot?.system || '').trim();
    const key = normalizeName(name);
    if (!name || seen.has(key) || !scoutHasMongrels(snapshot)) continue;
    const policy = strategyByName.get(key) || {};
    const observed = mergeScoutSnapshot({ name, present: true, live: true }, snapshot);
    merged.push(deriveSystem({ ...observed, ...policy, name }, priorityNames.has(key), strategy.defaults, timingControl));
    seen.add(key);
  }
  ...
  for (const policy of strategy.systems) {
    ...
    const scout = scoutByName.get(key);
    const base = scout && scoutHasMongrels(scout)
      ? mergeScoutSnapshot({ name: policy.name, present: true, live: true }, scout)
      : { name: policy.name, present: null, live: false, stale: true };
    merged.push(deriveSystem({
      ...base,
      ...policy,
      name: policy.name,
    }, priorityNames.has(key), strategy.defaults, timingControl));
  }

  return merged.sort((a, b) => String(a.name).localeCompare(String(b.name)));
}

function mergeScoutSnapshot(observed = {}, snapshot = null) {
  if (!snapshot || typeof snapshot !== 'object') return observed;
  const scoutUpdatedAt = normalizedTime(snapshot.updatedAt);
  const externalUpdatedAt = normalizedTime(observed.sourceUpdated || observed.lastSeen || observed.fetchedAt);
  const useScout = Boolean(scoutUpdatedAt) && (!externalUpdatedAt || Date.parse(scoutUpdatedAt) >= Date.parse(externalUpdatedAt));
  const mongrel = Array.isArray(snapshot.factions)
    ? snapshot.factions.find(row => normalizeName(row?.name) === normalizeName('Regiment of Imperial Mongrels'))
    : null;
  const controller = String(snapshot?.systemFaction?.name || observed.control || '').trim();

  const merged = {
    ...observed,
    scoutUpdatedAt,
    scoutReceivedAt: normalizedTime(snapshot.receivedAt),
    scoutLabel: String(snapshot.scoutLabel || '').trim(),
    freshestUpdatedAt: newerTimestamp(externalUpdatedAt, scoutUpdatedAt),
    freshestSource: useScout ? 'Mongrel Scout / EDMC' : (observed.source || 'EliteHub Vault / EDDN'),
  };
  if (!useScout || !mongrel) return merged;

  return {
    ...merged,
    influence: finiteOrNull(mongrel.influence),
    state: mongrel.state || observed.state || '',
    activeStates: Array.isArray(mongrel.activeStates) ? mongrel.activeStates : (observed.activeStates || []),
    pendingStates: Array.isArray(mongrel.pendingStates) ? mongrel.pendingStates : (observed.pendingStates || []),
    recoveringStates: Array.isArray(mongrel.recoveringStates) ? mongrel.recoveringStates : (observed.recoveringStates || []),
    control: controller || observed.control || '',
    controlled: normalizeName(controller) === normalizeName('Regiment of Imperial Mongrels'),
    sourceUpdated: scoutUpdatedAt,
    fetchedAt: normalizedTime(snapshot.receivedAt) || scoutUpdatedAt,
    lastSeen: scoutUpdatedAt,
...
function scoutHasMongrels(snapshot) {
  return Array.isArray(snapshot?.factions)
    && snapshot.factions.some(row => normalizeName(row?.name) === normalizeName('Regiment of Imperial Mongrels'));
}
...
function newerTimestamp(a,b) {
  ...
  if (!Number.isFinite(ams)) return Number.isFinite(bms) ? new Date(bms).toISOString() : null;
  if (!Number.isFinite(bms)) return new Date(ams).toISOString();
  return new Date(Math.max(ams,bms)).toISOString();
}

function deriveSystem(system, priorityFromList, defaults, timingControl = {}) {
  ...
  const freshnessCycle = resolveSystemWorkCycle(system.name, timingControl, { now:new Date() });
  const freshnessStamp = system.freshestUpdatedAt || system.sourceUpdated || system.fetchedAt || system.lastSeen || null;
  const freshnessTime = Date.parse(freshnessStamp || '');
  const cycleStart = Date.parse(freshnessCycle?.cycleStartedAt || '');
  const cycleEnd = Date.parse(freshnessCycle?.cycleEndsAt || '');
  const dataCondition = Number.isFinite(freshnessTime) && Number.isFinite(cycleStart) && Number.isFinite(cycleEnd)
    && freshnessTime >= cycleStart && freshnessTime < cycleEnd
    ? 'current'
    : 'stale';
  ...
  return {
    ...
    freshnessCycle,
    dataCondition,
    attention: Boolean(watch || belowTarget || aboveTarget || conflict || expansionRisk || retreatRisk || dataCondition === 'stale'),
