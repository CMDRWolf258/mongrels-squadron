...
export function parseJournal(text, targetSystem = TEST_SYSTEM, options = {}) {
  ...
  for (const line of lines) {
    ...
    if (entry.event === 'Undocked') { currentStation = ''; currentStationFaction = ''; currentStationType = ''; }

    const inTarget = norm(currentSystem) === norm(targetSystem);
    if (diagnosticMode && !DIAGNOSTIC_NOISE_EVENTS.has(entry.event)) diagnostics.push(diagnosticEvent(entry, currentSystem, currentStation, currentStationType));
    if (entry.event === 'MissionCompleted') {
      ...
      for (const effect of Array.isArray(entry.FactionEffects) ? entry.FactionEffects : []) {
        for (const inf of Array.isArray(effect?.Influence) ? effect.Influence : []) {
          ...
          const matches = targetAddress !== null
            ? String(address) === String(targetAddress)
            : inTarget;
          if (!matches) continue;
      ...
      if (effects.length) relevant.push(baseEvent(entry,currentSystem,currentAddress,currentStation,{
        type:'mission_inf',
    ...
    if (entry.event === 'RedeemVoucher') {
      ...
      if (voucherType === 'bounty' || voucherType === 'combatbond') {
        ...
        const normalized = baseEvent(entry,currentSystem,currentAddress,currentStation,{
  ...
  return {events:relevant,excluded,diagnostics,lastEventAt,lastSystem:currentSystem,targetAddress};
}
...
export function summarizeEvents(events) {
  const summary = {
    ...
    tradeTonnage:0,tradeSales:0,tradeProfit:0,tradeProfitKnownSales:0,tradeProfitLegacySales:0,tradeProfitUnavailableSales:0,
    explorationSales:0
  };
  for (const event of events) {
