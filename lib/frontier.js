...
export async function listFrontierAccounts(env) {
  ...
  do {
    const page=await env.DAILY_ORDERS.list({prefix:ACCOUNT_PREFIX,cursor,limit:1000});
      ...
      for (const effect of Array.isArray(entry.FactionEffects) ? entry.FactionEffects : []) {
        for (const inf of Array.isArray(effect?.Influence) ? effect.Influence : []) {
          ...
          if (address !== null && address !== undefined) {
            const resolved=addressToSystem.get(String(address)) || '';
            affectedSystem=targetByNorm.get(norm(resolved)) || '';
          } else if (inTarget) {
            affectedSystem=canonicalCurrent;
          }
