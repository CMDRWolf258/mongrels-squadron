...
export function parseJournal(text, targetSystem = TEST_SYSTEM, options = {}) {
  ...
  for (const line of lines) {
    ...
    if (entry.event === 'Undocked') { currentStation = ''; currentStationFaction = ''; }
    ...
    if (entry.event === 'RedeemVoucher' && ['Bounty','CombatBond'].includes(entry.Type)) {
      ...
      const avgPricePaid = Number(entry.AvgPricePaid)||0;
      const costBasis = avgPricePaid * count;
...
export async function mergeEvents(env, userId, events) {
  ...
  await env.DAILY_ORDERS.put(key, JSON.stringify({version:2,events:merged}));
  return merged;
}
...
export function summarizeEvents(events) {
  const summary = {missionInf:0,bounties:0,combatBondsRedeemed:0,czBondAwards:0,tradeTonnage:0,tradeSales:0,tradeProfit:0,explorationSales:0};
  for (const event of events) {
    ...
    if (event.type === 'market_sell') {
      ...
      const inferredProfit = Number(event.total||0) - (Number(event.avgPricePaid||0) * Number(event.count||0));
      summary.tradeProfit += Number.isFinite(Number(event.profit)) ? Number(event.profit) : inferredProfit;
    }
