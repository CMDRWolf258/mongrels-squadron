...
export function parseJournal(text, targetSystem = TEST_SYSTEM) {
  ...
  for (const line of lines) {
    ...
    if (entry.event === 'ReceiveText' && String(entry.Channel || '').toLowerCase() === 'npc') {
      const message = cleanText(entry.Message, '', 300);
      const messageLocalised = cleanText(entry.Message_Localised, '', 300);
      const from = cleanText(entry.From_Localised || entry.From, '', 160);
      relevant.push(baseEvent(entry,currentSystem,currentAddress,currentStation,{
        ...
        message,
        messageLocalised,
        possibleReputation:/reputation|rep\b|increased|decreased|increase|decrease/i.test(message + ' ' + messageLocalised),
        diagnostic:true,
      }));
    } else if (entry.event === 'RedeemVoucher' && ['Bounty','CombatBond'].includes(entry.Type)) {
      relevant.push(baseEvent(entry,currentSystem,currentAddress,currentStation,{
  ...
  return {events:relevant,lastEventAt,lastSystem:currentSystem,targetAddress};
}
