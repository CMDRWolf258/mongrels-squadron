...
// Keep only truly generic setup here. Material planning belongs in the goal-specific
// graph because the module/blueprint must be chosen before the material job can be
// defined accurately.
...
export function buildEngineeringCampaignView(stateValue, dependencyNodes = []) {
  ...
  // Completion and dependency readiness are intentionally separate. A dependency
  // can be satisfied either by a manual campaign completion or by a shared fact
  // (for example an Engineer already unlocked in an earlier campaign). Building
  // this status map first lets fact-completed nodes unlock downstream work just as
  // reliably as manually completed nodes.
  const completionById = new Map(nodes.map(node => {
    const manual = Boolean(campaign.completedNodes?.[node.id]);
    const automatic = node.factCompletion ? factSatisfied(node.factCompletion, state.facts) : false;
    return [node.id, { complete:manual || automatic, manual, automatic }];
  }));
  const evaluated = nodes.map(node => evaluateNode(node, campaign, completionById));
...
function evaluateNode(node, campaign, completionById) {
  const completion = completionById.get(node.id) || { complete:false, manual:false, automatic:false };
  const dependenciesMet = node.dependsOn.every(id => Boolean(completionById.get(id)?.complete));
  return {
    ...node,
    status:completion.complete ? 'complete' : 'pending',
    completionSource:completion.manual ? campaign.completedNodes[node.id]?.source || 'manual' : completion.automatic ? 'fact' : '',
    dependenciesMet,
  };
}
