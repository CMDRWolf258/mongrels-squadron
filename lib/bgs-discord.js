...
export async function loadBgsDiscordView(request,env){
  const [live,boards,scouts]=await Promise.all([
  ...
  return buildBgsDiscordView({live,boards,scouts});
}

export function buildBgsDiscordView({live={},boards={},scouts={}}={}){
  const activeSystems=(Array.isArray(live?.systems)?live.systems:[])
    .filter(row=>row?.name&&row.present!==false&&row.formerPresence!==true);
  const boardMap=new Map((Array.isArray(boards?.systems)?boards.systems:[]).map(row=>[norm(row?.name),row]));
...
export function buildBgsSummaryDiscordPayload(view,{missionControlUrl=''}={}){
  ...
  return{
    ...
    embeds:[{
      title:'BGS Alerts & Opportunities',
      ...(link?{url:link}:{}),
      description:[
      ...
      footer:{text:'Regiment of Imperial Mongrels · persistent BGS operations summary · mentions disabled'},
    }],
  };
}
...
export function buildBgsActionDiscordPayload(action,{missionControlUrl='',resolved=false,tracked=null}={}){
  ...
  const description=resolved
    ? 'This BGS alert has cleared. The card will leave the operations channel on the next BGS alert refresh.'
    : family==='retreat'
      ? 'Mongrel Retreat condition detected. Check Mission Control before applying BGS work in this system.'
      : 'Mongrel conflict detected. Check Mission Control for pairing, conflict-day tracking, and current orders.';
  ...
  return{
    ...
    embeds:[{
      ...
      fields,
      footer:{text:'Regiment of Imperial Mongrels · BGS operational alert'},
    }],
  };
}
...
function actionSummaryLine(action){
  ...
  return '• **'+escapeMarkdown(action.system)+'** — '+phase+' '+escapeMarkdown(action.detail)+opponent+score;
}
function opportunitySummaryLine(row){
  ...
  return '• **'+escapeMarkdown(row.system)+'** — '+states.join(' · ');
}
