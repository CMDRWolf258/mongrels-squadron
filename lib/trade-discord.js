...
export function buildTradeWatchDiscordPayload(watch,{origin='',control=null,subscriberCount=0}={}) {
  ...
  const knownRareSource=evaluation.knownRareSource&&typeof evaluation.knownRareSource==='object'?evaluation.knownRareSource:null;
  ...
  if(best){
    ...
    if(rareSource&&best.commanderSensitiveSupply){
      fields.push({
        name:'Latest Commander Report',
        value:number(best.reportedVolume)+' t'
          +(Number(best.reportedVolume)===0?' · ignored for allocation tracking':'')
          +(best.allocationObservedAt?'\nLast positive: '+discordRelativeTime(best.allocationObservedAt):''),
        inline:true,
      });
    }
  ...
  }else if(rareSource&&knownRareSource){
    fields.push({
      name:'📍 Known Rare Source',
      value:'**'+clean(knownRareSource.stationName||'Known source').slice(0,120)+'**\n'
        +clean(knownRareSource.systemName||'').slice(0,120)
        +'\nLatest commander report: **'+number(knownRareSource.reportedVolume)+' t**'
        +(Number(knownRareSource.allocationVolume)>0?'\nLast positive allocation: **'+number(knownRareSource.allocationVolume)+' t**':'')
        +' · '+number(knownRareSource.price)+' Cr/t'
        +'\n⚠ '+rareSourceStatusLabel(knownRareSource.rareSourceStatus),
      inline:false,
    });
  }else{
    fields.push({
  ...
  const description=[
    ...
    rareSource
      ?'Status: **'+state+'** · Rare-source timer · **hourly from 1–8 PM CT**'
  ...
  const fields=[{
    ...
    value:'Matches: **'+number(evaluation.matchCount)+'**\nCadence: **'
      +(isRareTradeCommodity(q.commodity)&&q.direction==='buy'
        ?'Hourly 1–8 PM CT'
        :tradePriorityProfile(config,q.priority).label)
      +'**',
    inline:true,
  },{
    name:'Watch Criteria',
...
export async function sendTradeWatchTransitionAlert(env,{watch,transition,origin='',control=null}={}) {
  ...
  if(!['condition_met','condition_cleared','best_market_changed','rare_allocation_changed'].includes(transition.type)){
  ...
  const titleMap={
    ...
    rare_allocation_changed:'Rare Allocation Changed',
  };
  ...
  const embed={
    ...
    description,
    color:transition.type==='condition_met'?0x5ee6a8:transition.type==='condition_cleared'?0xe06c75:transition.type==='rare_allocation_changed'?0x9b7cff:0xf5b942,
    fields:best?[{
      name:'Current Best',
      value:'**'+clean(best.stationName||'Unknown station').slice(0,120)+'**\n'+clean(best.systemName||'Unknown system').slice(0,120),
      inline:true,
      ...
      name:isRareTradeCommodity(q.commodity)&&q.direction==='buy'?'Tracked Allocation':(q.direction==='buy'?'Supply':'Demand'),
      value:number(best.volume)+' t',
      inline:true,
    }]:knownRareSource?[{
      name:'Known Rare Source',
      value:'**'+clean(knownRareSource.stationName||'Known source').slice(0,120)+'** · '+clean(knownRareSource.systemName||'').slice(0,100)
        +'\nLatest report: '+number(knownRareSource.reportedVolume)+' t'
        +(Number(knownRareSource.allocationVolume)>0?' · last positive allocation '+number(knownRareSource.allocationVolume)+' t':'')
        +' · '+number(knownRareSource.price)+' Cr/t'
        +'\n'+rareSourceStatusLabel(knownRareSource.rareSourceStatus),
      inline:false,
    }]:[],
...
export function buildTradeDiscordPayload(route,{origin='',control=null,subscriberCount=0}={}) {
  ...
  if(managed){
    const currentProfit=Number(route.optimizer.currentProfit||route.estimatedLoopProfit)||0;
    const baselineProfit=Number(route.optimizer.baselineProfit)||0;
    const dropPercent=Number(route.optimizer.thresholdDropPercent)||25;
    const floor=baselineProfit>0?Math.round(baselineProfit*(1-dropPercent/100)):0;
    const state=clean(route.optimizer.state||'healthy');
    fields.push({
      name:'🧭 Managed Loop',
      value:'**'+number(currentProfit)+' Cr / loop** · '+route.legs.length+' legs\n'
        +'Baseline: '+number(baselineProfit)+' Cr · Alert below: '+number(floor)+' Cr'
        +(route.optimizer.mongrelOnly?'\n🐺 Faction routing: **Mongrel-owned stations only**':'')
        +(state!=='healthy'?'\n⚠ Status: **'+title(state)+'**':''),
      inline:false,
    });
    for(let index=0;index<route.legs.length;index++){
      ...
      fields.push({
        name:'Leg '+(index+1)+' · '+clean(leg.commodity||'Cargo').slice(0,90),
        value:'**'+clean(leg.sourceStation||'Unknown').slice(0,90)+'** · '+clean(leg.sourceSystem||'').slice(0,80)
          +'\n→ **'+clean(leg.destinationStation||'Unknown').slice(0,90)+'** · '+clean(leg.destinationSystem||'').slice(0,80)
          +'\nBuy '+number(leg.buyPrice)+' · Sell '+number(leg.sellPrice)+' Cr/t · **+'+number(leg.profitPerTon)+' Cr/t**'
          +'\n'+number(leg.quantity)+' t used · '+number(leg.tripProfit)+' Cr leg profit',
        inline:false,
      });
    }
    if(clean(route.padSize))fields.push({name:'Pad',value:title(clean(route.padSize)),inline:true});
    if(clean(route.distanceLy))fields.push({name:'Loop Distance',value:clean(route.distanceLy).slice(0,40)+' ly',inline:true});
    const alternative=route.optimizer.alternative;
    if(alternative&&Number(alternative.loopProfit)>currentProfit){
      fields.push({
        name:'↪ Better Matching Loop',
        value:'**'+number(alternative.loopProfit)+' Cr / loop**\n'
          +(Array.isArray(alternative.legs)?alternative.legs.map(leg=>clean(leg.commodity)).filter(Boolean).join(' → '):'Alternative available'),
        inline:false,
      });
    }
  }else{
    if(clean(route.commodity)){
      fields.push({
        name:'📦 Outbound Cargo',
        value:'**'+clean(route.commodity).slice(0,100)+'**',
        inline:false,
      });
    }
    ...
    if(Number(route.profitPerTon)>0){
      fields.push({name:'Outbound Profit / t',value:number(route.profitPerTon)+' Cr',inline:true});
    }
    if(clean(route.quantity))fields.push({name:'Outbound Supply / Demand',value:clean(route.quantity).slice(0,100),inline:true});
    if(clean(route.padSize))fields.push({name:'Pad',value:title(clean(route.padSize)),inline:true});
    ...
    if(clean(route.returnCommodity)){
      ...
      if(Number(route.returnProfitPerTon)>0){
        fields.push({name:'Return Profit / t',value:number(route.returnProfitPerTon)+' Cr',inline:true});
      }
      if(clean(route.returnQuantity)){
        fields.push({name:'Return Supply / Demand',value:clean(route.returnQuantity).slice(0,100),inline:true});
      }
    }
  }
...
function watchTransitionDescription(watch,transition){
  ...
  const knownRareSource=evaluation.knownRareSource&&typeof evaluation.knownRareSource==='object'?evaluation.knownRareSource:null;
  const rareSource=isRareTradeCommodity(q.commodity)&&q.direction==='buy';
  if(transition.type==='rare_allocation_changed'){
    const from=Number(transition.fromVolume||transition.from)||0;
    const to=Number(transition.toVolume||transition.to)||0;
    const direction=to>from?'increased':'decreased';
    return '**'+clean(q.commodity||'Rare commodity')+'** tracked allocation **'+direction+'** from **'+number(from)+' t → '+number(to)+' t**.'
      +'\n\nZero/partial commander depletion reports are ignored; this alert is based on the positive daily allocation tracked during the 1–8 PM CT observation window.'
      +(best?'\n\n**'+clean(best.stationName)+' · '+clean(best.systemName)+'** · '+number(best.price)+' Cr/t.':'');
  }
  if(transition.type==='condition_met'){
    if(rareSource){
      return '**'+clean(q.commodity||'Rare commodity')+'** at its known source now meets the saved Watch thresholds.'
        +(best?'\n\n**'+clean(best.stationName)+' · '+clean(best.systemName)+'** tracked allocation: **'+number(best.volume)+' t** at **'+number(best.price)+' Cr/t**.':'');
    }
  ...
  if(transition.type==='condition_cleared'){
    if(rareSource&&knownRareSource){
      return 'The known **'+clean(q.commodity||'rare commodity')+'** source has **not disappeared**. **'
        +clean(knownRareSource.stationName||'Known source')+' · '+clean(knownRareSource.systemName||'')
        +'** currently does not meet the saved Watch thresholds.'
        +'\n\nLatest commander report: **'+number(knownRareSource.reportedVolume)+' t**.'
        +(Number(knownRareSource.allocationVolume)>0?' Last positive allocation: **'+number(knownRareSource.allocationVolume)+' t**.':'')
        +' '+rareSourceStatusLabel(knownRareSource.rareSourceStatus)
        +'\n\nRare-source buys are searched at all distances; **'+clean(q.referenceSystem||'the reference system')+'** is used only for distance measurement.';
    }
    if(rareSource){
      return 'The known source for **'+clean(q.commodity||'this rare commodity')+'** is still fixed, but the current market observation does not satisfy the saved Watch thresholds. Rare-source buys are not radius-limited.';
    }
...
function rareSourceStatusLabel(value){
  const state=clean(value);
  if(state==='no_positive_allocation_observed')return'No positive allocation has been observed yet; zero commander reports are not treated as station depletion.';
  if(state==='allocation_below_threshold')return'The tracked positive allocation is below the saved minimum.';
  if(state==='price_above_threshold')return'The latest observed buy price is above the saved maximum.';
  if(state==='price_unavailable')return'The latest observation does not include a usable buy price.';
  if(state==='pad_below_threshold')return'The known source does not meet the saved pad-size filter.';
  return'The latest observation does not meet the saved filters.';
}
