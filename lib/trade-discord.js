  ...
  if(best){
    ...
    fields.push({
      ...
      value:'**'+clean(knownRareSource.stationName||'Known source').slice(0,120)+'**\n'
        ...
        +'\nCurrent observation: **'+number(knownRareSource.volume)+' t** supply · '+number(knownRareSource.price)+' Cr/t'
  ...
  const embed={
    ...
    fields:best?[{
      ...
      value:'**'+clean(knownRareSource.stationName||'Known source').slice(0,120)+'** · '+clean(knownRareSource.systemName||'').slice(0,100)
        +'\n'+number(knownRareSource.volume)+' t supply · '+number(knownRareSource.price)+' Cr/t'
        +'\n'+rareSourceStatusLabel(knownRareSource.rareSourceStatus),
      inline:false,
    }]:[],
  ...
  if(transition.type==='condition_met'){
    if(rareSource){
      return '**'+clean(q.commodity||'Rare commodity')+'** at its known source now meets the saved Watch thresholds.'
        +(best?'\n\n**'+clean(best.stationName)+' · '+clean(best.systemName)+'** currently reports **'+number(best.volume)+' t** supply at **'+number(best.price)+' Cr/t**.':'');
    }
  ...
  if(transition.type==='condition_cleared'){
    if(rareSource&&knownRareSource){
      return 'The known **'+clean(q.commodity||'rare commodity')+'** source has **not disappeared**. **'
        ...
        +'\n\nCurrent observation: **'+number(knownRareSource.volume)+' t** supply at **'+number(knownRareSource.price)+' Cr/t**.'
        +' '+rareSourceStatusLabel(knownRareSource.rareSourceStatus)
...
function rareSourceStatusLabel(value){
  const state=clean(value);
  if(state==='no_observed_stock')return'The latest community observation reports no qualifying stock.';
  if(state==='supply_below_threshold')return'The latest observed supply is below the saved minimum.';
