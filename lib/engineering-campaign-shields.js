...
export function buildShieldEngineeringDependencyNodes({ campaign = null, facts = {} } = {}) {
  if (!campaign || campaign.goalId !== 'shields') return [];

  const hasFact = factId => Boolean(facts?.[factId]?.value);
  const dwellerProof = hasFact(SHIELD_CAMPAIGN_FACTS.leiUnlocked)
    ? SHIELD_CAMPAIGN_FACTS.leiUnlocked
    : hasFact(SHIELD_CAMPAIGN_FACTS.leiReferralReady)
      ? SHIELD_CAMPAIGN_FACTS.leiReferralReady
      : hasFact(SHIELD_CAMPAIGN_FACTS.dwellerUnlocked)
        ? SHIELD_CAMPAIGN_FACTS.dwellerUnlocked
        : '';
  const referralProof = hasFact(SHIELD_CAMPAIGN_FACTS.leiUnlocked)
    ? SHIELD_CAMPAIGN_FACTS.leiUnlocked
    : SHIELD_CAMPAIGN_FACTS.leiReferralReady;
  const leiProof = hasFact(SHIELD_CAMPAIGN_FACTS.leiUnlocked) ? SHIELD_CAMPAIGN_FACTS.leiUnlocked : '';
  const g2Proof = hasFact(SHIELD_CAMPAIGN_FACTS.leiG3Ready)
    ? SHIELD_CAMPAIGN_FACTS.leiG3Ready
    : SHIELD_CAMPAIGN_FACTS.leiG2Ready;
  ...
  return [
    ...
    {
      id:'shields.gather-g2-materials',
      kind:'prepare',
      title:'Gather only the planned shield materials',
      objective:'Acquire or trade for the materials in your G1 → G2 plan and stop when that small job is covered. Do not turn this into a general material-cap grind. If your current inventory already covers the plan, mark this step complete immediately.',
      ...
      meta:{
        ...
        resourceUrl:INARA_ENGINEERS,
        resourceLabel:'Inara Engineers / Crafting',
        note:'The purpose is to fund this shield improvement, not to fill every material bin.',
    ...
    },
    {
      ...
      objective:'Build The Dweller meeting requirement in small pieces. Use five different black markets; repeat visits to the same black market do not increase the count. If your tracker already shows 5, this step clears automatically. If you already have The Dweller or Lei Cheung unlocked, record that instead of inventing an old black-market total.',
      dependsOn:['shields.gather-g2-materials'],
    ...
    },
    {
      ...
      meta:{
        ...
        alternateFacts:[
          { factId:SHIELD_CAMPAIGN_FACTS.leiUnlocked, label:'Lei Cheung Is Already Unlocked' },
        ],
      },
    },
    {
      ...
      factCompletion:{ factId:referralProof, operator:'truthy', target:true },
      meta:{
        stage:'Engineer Access',
    ...
    },
    {
      ...
      objective:'Trade at distinct commodity markets until your cumulative market count reaches the tracked 50-market milestone. At 50, check the Engineer panel; current references differ between “at least 50” and “over 50,” so make one additional unique-market trade if the invitation has not registered. If Lei is already unlocked, record that instead of reconstructing years of market history.',
      dependsOn:['shields.gather-g2-materials'],
      factCompletion:leiProof
        ? { factId:leiProof, operator:'truthy', target:true }
        : { factId:'trade.markets-visited-distinct', operator:'gte', target:50 },
      backgroundActivities:['trade'],
      chunkSize:5,
      meta:{
    ...
    },
    {
      ...
      objective:'Craft only enough useful work with Lei Cheung to open Grade 2 for the Shield Generator blueprint you chose. Stop when G2 is available instead of pushing reputation for its own sake. If you already have G3 or higher access, record that and skip this reputation step.',
      dependsOn:['shields.lei.unlock'],
      factCompletion:{ factId:g2Proof, operator:'truthy', target:true },
      meta:{
        ...
        resourceLabel:'Lei Cheung on Inara',
        alternateFacts:[
          { factId:SHIELD_CAMPAIGN_FACTS.leiG3Ready, label:'I Already Have G3+ Shield Access' },
        ],
      },
    },
    {
      id:'shields.refresh-g2-materials',
      kind:'plan',
      title:'Check the exact G2 material shortfall',
      objective:'Now that Lei Cheung access and reputation are known, refresh the G1 → G2 plan once. If your earlier material run already covers it, mark this complete. If you are short, gather or trade only the missing amount before engineering the module.',
      ...
      meta:{
        ...
        resourceUrl:INARA_ENGINEERS,
        resourceLabel:'Inara Engineers / Crafting',
        note:'This catches reputation-dependent roll differences without sending you back into a broad material grind.',
        stoppingPoint:'Cover only the remaining G2 shortage, then stop.',
      },
    },
    {
      ...
      objective:'Apply the chosen Shield Generator blueprint through Grade 2 using the targeted material plan. Do not continue automatically just because higher grades exist.',
      dependsOn:['shields.refresh-g2-materials'],
      meta:{
        stage:'Engineer',
