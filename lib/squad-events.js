...
export function squadEventsConfig(env){
  return {
    channelId:clean(env?.DISCORD_SQUAD_EVENTS_CHANNEL_ID),
    configured:Boolean(env?.DISCORD_BOT_TOKEN&&clean(env?.DISCORD_SQUAD_EVENTS_CHANNEL_ID)),
  };
}
...
export async function syncSquadEventDiscord(env,event,{origin='',createIfMissing=false}={}){
  ...
  try{
    let message;
    let mode;
    if(trackedId&&trackedChannelId===config.channelId){
      try{
        message=await discordRequest(env,`/channels/${encodeURIComponent(config.channelId)}/messages/${encodeURIComponent(trackedId)}`,{
          method:'PATCH',
        ...
        message=await createEventMessage(env,config.channelId,payload);
        mode='recreated';
      }
    }else{
      message=await createEventMessage(env,config.channelId,payload);
      mode='created';
      if(trackedId&&trackedChannelId&&trackedChannelId!==config.channelId){
    ...
    return {
      ...
      attempted:true,
      mode:'failed',
      error:clean(error?.message||'discord_event_sync_failed').slice(0,500),
    };
  }
}
...
export async function readProjectsBoard(env){
  ...
  try{
    const stored=await env.PROJECTS.get(BOARD_KEY,{type:'json'});
