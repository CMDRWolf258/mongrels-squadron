...
export async function resolveSquadEventsChannelId(env){
  ...
  const eligible=(Array.isArray(channels)?channels:[]).filter(channel=>{
    const type=Number(channel?.type);
    return type===0||type===5;
  });
  const decorated=eligible.find(channel=>squadEventsChannelName(channel?.name)==='decorated');
  const plain=eligible.find(channel=>squadEventsChannelName(channel?.name)==='plain');
...
export async function syncSquadEventDiscord(env,event,{origin='',createIfMissing=false}={}){
  ...
  if(!channelId){
    return result(false,'squad_events_channel_not_found',{
      configured:true,
      attempted:false,
      error:'Could not find Discord channel 📅〡squad-events (plain squad-events is also accepted).',
    });
  }
...
function friendlyDiscordEventError(error){
  ...
  if(/Missing Access/i.test(message)||/"code"\s*:\s*50001/.test(message)){
    return 'The Imperial Mongrels Website bot cannot access 📅〡squad-events. Grant it View Channel, Send Messages, Embed Links, and Read Message History, then save the event again to retry.';
  }
  if(/Missing Permissions/i.test(message)||/"code"\s*:\s*50013/.test(message)){
    return 'The Imperial Mongrels Website bot can see 📅〡squad-events but is missing a required channel permission. Grant Send Messages, Embed Links, and Read Message History, then save the event again to retry.';
  }
...
export function buildSquadEventDiscordPayload(event,{origin=''}={}){
  ...
  if(clean(event.system))fields.push({name:'📍 System / Location',value:clean(event.system).slice(0,100),inline:false});
  ...
  fields.push({
    name:'🐺 RSVP',
    value:[
      `✅ **Going** — ${rsvp.counts.going}`,
      `🤔 **Maybe** — ${rsvp.counts.maybe}`,
      `❌ **Can’t Make It** — ${rsvp.counts.cant}`,
    ].join('\n'),
    inline:false,
  });
  ...
  const eventImageUrl=discordEmbedImageUrl(event.eventImageUrl,origin,event.updatedAt||event.createdAt);
  if(eventImageUrl)embed.image={url:eventImageUrl};
...
function squadEventsChannelName(value){
  const name=clean(value).toLowerCase().replace(/\ufe0f/g,'');
  if(name==='📅〡squad-events'||name.endsWith('〡squad-events'))return'decorated';
  if(name==='squad-events')return'plain';
  return'';
}
...
function discordEmbedImageUrl(value,origin,versionSource=''){
  const raw=clean(value);
  if(!isPublicHttpsUrl(raw))return'';
  try{
    const url=new URL(raw);
    const siteOrigin=clean(origin);
    if(siteOrigin){
      const site=new URL(siteOrigin);
      if(url.origin===site.origin){
        const stamp=Date.parse(versionSource);
        if(Number.isFinite(stamp))url.searchParams.set('v',String(stamp));
      }
    }
    return url.toString();
  }catch{return raw;}
