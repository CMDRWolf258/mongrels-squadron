...
export async function resolveTrainingChannels(env){
  ...
  if(!chatChannelId){
    const preferred=eligible.find(channel=>channelKind(channel?.name)==='chat_primary');
    const legacy=eligible.find(channel=>channelKind(channel?.name)==='chat_legacy');
    chatChannelId=clean((preferred||legacy)?.id);
  }
  return{resourcesChannelId,chatChannelId,guildId:config.guildId};
}
...
export async function syncTrainingResourcesDiscord(env,{origin=''}={}){
  ...
  if(!resolved.resourcesChannelId){
    return{
      ...
      error:'Could not find Discord channel 📚〡training-resources (plain training-resources is also accepted).',
    };
  }
...
export function buildTrainingResourcesDiscordPayload({origin='',guildId='',chatChannelId=''}={}){
  ...
  const fields=[
    ...
    },
    {
      name:'💬 Need Help?',
      value:chatUrl
        ?'Use **Training Grounds** for questions, mentoring, builds, screenshots and troubleshooting. Use **Ask the Mongrels** when you want the site to route you to the right knowledge.'
        :'Use the squad training channel for questions, mentoring, builds, screenshots and troubleshooting. Use **Ask the Mongrels** when you want the site to route you to the right knowledge.',
      inline:false,
    },
  ];
  ...
  const row2=[
    ...
    linkButton('Training Grounds',chatUrl,'☕'),
  ].filter(Boolean);
...
function channelKind(value){
  ...
  if(name==='☕〡training-grounds'||name==='training-grounds'||name.endsWith('〡training-grounds'))return'chat_primary';
  if(name==='training-chat'||name.endsWith('〡training-chat'))return'chat_legacy';
  return'';
}
function friendlyError(error){
  const message=clean(error?.message||'discord_training_resources_sync_failed');
  if(/Missing Access/i.test(message)||/"code"\s*:\s*50001/.test(message))return'The Imperial Mongrels Website bot cannot access 📚〡training-resources.';
  if(/Missing Permissions/i.test(message)||/"code"\s*:\s*50013/.test(message))return'The Imperial Mongrels Website bot needs View Channel, Send Messages, Embed Links, and Read Message History in 📚〡training-resources.';
