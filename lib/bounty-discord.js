...
export async function resolveBountyBoardChannelId(env){
  ...
  if(!config.guildId||!env?.DISCORD_BOT_TOKEN)return config.channelId||'';
  ...
  const configured=config.channelId?eligible.find(channel=>clean(channel?.id)===config.channelId):null;
  if(configured&&bountyChannelName(configured?.name))return config.channelId;
  ...
  return clean((decorated||plain)?.id)||config.channelId;
}
