...
export const GALLERY_TAGS=[
  'AX','BGS','Carriers','Colonization','Combat','Community','Engineering',
  'Events','Exobiology','Exploration','Mining','Operations','PvP','Scenic',
  'Ships','Trade'
];
const TAG_SET=new Set(GALLERY_TAGS);

export async function readGallerySubmissions(env,{fresh=false}={}){
...
export function galleryPreviewUrl(request,key){
  if(!isGalleryImageKey(key))return'';
  const origin=new URL(request.url).origin;
  return `${origin}/api/gallery/image?key=${encodeURIComponent(key)}`;
}
...
export function memberGallerySubmission(item,request){
  ...
  return {
    ...
    tags:item.tags,
    status:item.status,
    url:item.status==='approved'&&isGalleryImageKey(item.imageKey)
      ?galleryImageUrl(request,item.imageKey)
      :item.status==='pending'&&isGalleryImageKey(item.imageKey)
        ?galleryPreviewUrl(request,item.imageKey)
        :'',
...
function normalizeStored(raw){
  ...
  const status=['pending','approved','rejected','removed'].includes(raw.status)?raw.status:'pending';
  return {
    id,
    ownerId,
    ...
    removedAt:validIso(raw.removedAt),
    removedBy:cleanGalleryText(raw.removedBy,100),
  };
}
