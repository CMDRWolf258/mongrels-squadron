...
export function buildOrderRewardPolicies(orders, settings = DEFAULT_REWARD_SETTINGS) {
  const rules=normalizeRewardSettings(settings);
  const out={};
  for(const order of Array.isArray(orders)?orders:[]){
    const orderId=String(order?.id||'').trim();
    if(!orderId)continue;
    const type=String(order?.reporting?.type||'').trim();
    const target=finiteOrNull(order?.reporting?.target);
    const eligible=['trade','inf','bounties'].includes(type);
    if(!eligible){
      out[orderId]={
        orderId,
        ...
        eligible:false,
        target,
        goalRewardMillions:0,
        capMillions:0,
      };
      continue;
    }
    const goalRewardMillions=target===null
      ? 0
      : rewardEntitlementMillions(type,target,target,rules);
    const capMillions=rewardCapMillions(type,target,rules);
    const policy={
      orderId,
      type,
      eligible:true,
    ...
    if(type==='trade'){
      policy.profitBlockMillions=rules.trade.profitBlockMillions;
      policy.rewardPerBlockMillions=rules.trade.rewardPerBlockMillions;
    }else if(type==='inf'){
      policy.rewardPerInfBeforeGoalMillions=rules.inf.rewardPerInfBeforeGoalMillions;
      policy.rewardPerInfAfterGoalMillions=rules.inf.rewardPerInfAfterGoalMillions;
    }else if(type==='bounties'){
      policy.rewardPerRedeemedMillion=rules.bounties.rewardPerRedeemedMillion;
    }
    out[orderId]=policy;
  }
  return out;
