...
async function evaluateOne(env,watch,control,{now,fetchImpl}){
  ...
  try{
    ...
    const matches=visibleResults.filter(item=>item?.qualifies!==false);
    const qualifyingMatchCount=Number.isFinite(Number(result?.qualifyingMatchCount))
      ?Math.max(0,Math.round(Number(result.qualifyingMatchCount)))
      :matches.length;
    const rankedMarkets=matches
      .slice(0,TRADE_WATCH_SHORTLIST_SIZE)
      .map((item,index)=>snapshotMarket(item,watch.query,index+1));
    const knownRareSource=visibleResults.find(item=>item?.qualifies===false&&item?.rareSourceStatus)
      ?snapshotMarket(visibleResults.find(item=>item?.qualifies===false&&item?.rareSourceStatus),watch.query,1)
      :null;
    const currentBest=rankedMarkets[0]||null;
