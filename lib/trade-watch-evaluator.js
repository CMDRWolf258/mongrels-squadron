...
function evaluateRareSourceAllocation(watch,previous,visibleResults,attemptedAt){
  ...
  const allocationVolume=Math.max(0,Math.round(Number(source?.allocationVolume)||0));
  const allocationObservedAt=iso(source?.allocationObservedAt);
  const allocationDay=allocationObservedAt
    ?rareSourceWatchParts(Date.parse(allocationObservedAt)).day
    :'';
  const positiveToday=allocationVolume>0&&allocationDay===nowParts.day;

  if(positiveToday&&allocationVolume>dayHigh){
    dayHigh=allocationVolume;
    dayHighObservedAt=allocationObservedAt;
  }
  ...
  if(value<=0&&dayHigh>0){
    ...
    value=dayHigh;
    changed=true;
  }else if(nowParts.hour>=RARE_SOURCE_WATCH_END_HOUR&&dayHigh>0&&dayHigh<value){
    value=dayHigh;
    changed=true;
  }
  ...
  const price=Math.max(
    ...
    Math.round(Number(priorState.price)||0),
    Math.round(Number(priorBest?.price)||0)
  );
  ...
  const currentBest=qualifies&&baseSource?{
    ...
    price,
    observedAt:dayHighObservedAt||iso(priorState.observedAt)||baseSource.observedAt||'',
    allocationObservedAt:dayHighObservedAt||iso(priorState.observedAt)||baseSource.allocationObservedAt||'',
  ...
  const rareAllocation={
    value,
    price,
    observedAt:dayHighObservedAt||iso(priorState.observedAt)||iso(priorBest?.observedAt),
    dayKey:nowParts.day,
    dayHigh,
    dayHighObservedAt,
    previousValue:changed&&previousValue>0&&value!==previousValue?previousValue:Math.max(0,Math.round(Number(priorState.previousValue)||0)),
    lastReportedSupply:reportedVolume,
    lastReportAt:iso(source?.observedAt)||iso(priorState.lastReportAt),
  };
