...
export async function readDailyOrderTimingControl(env){
  ...
  return {defaults,systemSettings};
}
...
export function resolveOrderWorkCycle(order,control,{now=new Date(),offset=0}={}){
  const settings=systemTimingSettings(order?.system,control);
  const clock=cycleForTick(settings.tick,settings.transitionMinutes,now,offset);
  const system=clean(order?.system)||'Squad-wide';
  const createdAt=iso(order?.createdAt);
  const acceptFromAt=laterIso(clock.cycleStartedAt,createdAt);
  return {
    version:1,
    cycleId:workCycleId(system,clock.cycleStartedAt),
    system,
    tickUtc:settings.tick,
...
function cycleForTick(tick,transitionMinutes,now,offset=0){
  const instant=now instanceof Date?now:new Date(now);
  const nowMs=Number.isFinite(instant.getTime())?instant.getTime():Date.now();
  const date=new Date(nowMs);
  const {hour,minute}=tickParts(tick);
  const todayTick=Date.UTC(date.getUTCFullYear(),date.getUTCMonth(),date.getUTCDate(),hour,minute,0,0);
  const transitionMs=transitionMinutes*60_000;
  let tickMs,phase;
  if(nowMs<todayTick){
    tickMs=todayTick;phase='pre_tick';
  }else if(nowMs<todayTick+transitionMs){
    tickMs=todayTick;phase='transition';
  }else{
    tickMs=todayTick+DAY_MS;phase='pre_tick';
  }
  const back=Math.max(0,Math.floor(Number(offset)||0))*DAY_MS;
  tickMs-=back;
  if(back)phase='closed';
  return {
    phase,
    estimatedTickAt:new Date(tickMs).toISOString(),
    cycleStartedAt:new Date(tickMs-DAY_MS+transitionMs).toISOString(),
...
function workCycleId(system,startAt){
  ...
  return ('tick-v1:'+slug+':'+stamp).slice(0,100);
}
