...
export async function resolveSquadStructureChannelId(env){
  ...
  const decorated=eligible.find(channel=>squadStructureChannelName(channel?.name)==='decorated');
  const plain=eligible.find(channel=>squadStructureChannelName(channel?.name)==='plain');
...
export async function syncSquadStructureDiscord(env,state,{origin=''}={}){
  ...
  if(!channelId){
    return result(false,'squad_structure_channel_not_found',{
      configured:true,
      attempted:false,
      error:'Could not find Discord channel 🏛️〡squad-structure (plain squad-structure is also accepted).',
    });
  }
...
function friendlyError(error){
  ...
  if(/Missing Access/i.test(message)||/"code"\s*:\s*50001/.test(message)){
    return 'The Imperial Mongrels Website bot cannot access 🏛️〡squad-structure. Grant View Channel, Send Messages, Embed Links, and Read Message History.';
  }
  if(/Missing Permissions/i.test(message)||/"code"\s*:\s*50013/.test(message)){
    return 'The Imperial Mongrels Website bot can see 🏛️〡squad-structure but is missing Send Messages, Embed Links, or Read Message History.';
  }
...
function squadStructureChannelName(value){
  const name=clean(value).toLowerCase().replace(/\ufe0f/g,'');
  if(name==='🏛〡squad-structure')return'decorated';
  if(name==='squad-structure')return'plain';
  return'';
}
