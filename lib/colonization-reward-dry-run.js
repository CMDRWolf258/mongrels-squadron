    ...
    for(const event of events){
      ...
      for(const assignment of arbitration.assignments||[]){
        ...
        assigned.push({
          ...
          retroactiveStart:Boolean(meta.retroactiveStart),
        });
      }
...
function resolveJobForEvent(catalogRow,event){
  ...
  // The visible Reward Start Date / Time is the authoritative eligibility
  // boundary. A later approval/publication may deliberately backdate the rule
  // so recent verified hauling can count. Pending squad funding still creates
  // no debt until approved; once approved, the approved revision may cover
  // evidence at or after its explicit startsAt value.
  if(revisions.length){
    let latestIndex=0;
    for(let i=1;i<revisions.length;i++){
      if(Number(revisions[i]?.revision||0)>Number(revisions[latestIndex]?.revision||0))latestIndex=i;
    }
    const latest=revisions[latestIndex];
    const start=Date.parse(latest?.job?.startsAt||'');
    const latestFundingMode=['none','member','squad'].includes(latest?.job?.fundingMode)?latest.job.fundingMode:'squad';
    const latestFundingApproval=['not_required','pending','approved','rejected'].includes(latest?.job?.fundingApprovalStatus)
      ? latest.job.fundingApprovalStatus
      : 'approved';
    const latestRewardAllowed=latestFundingMode!=='squad'||latestFundingApproval==='approved';

    const selected=index>=0?revisions[index]:null;
    const selectedFundingMode=['none','member','squad'].includes(selected?.job?.fundingMode)?selected.job.fundingMode:'squad';
    const selectedFundingApproval=['not_required','pending','approved','rejected'].includes(selected?.job?.fundingApprovalStatus)
      ? selected.job.fundingApprovalStatus
      : 'approved';
    const selectedRewardAllowed=selectedFundingMode!=='squad'||selectedFundingApproval==='approved';

    if(latestRewardAllowed&&Number.isFinite(start)&&time>=start&&(index<0||!selectedRewardAllowed)){
      index=latestIndex;
  ...
  return {job,ruleRevision,bindingRevision,retroactiveStart};
}
...
async function buildJobObligation({ownerId,commander,jobId,assignments,ledgerIndex}){
  ...
  return {
    ...
    retroactiveStartCredit:assignments.some(row=>row?.retroactiveStart===true),
    sourceEventIds,
    eventCount:sourceEventIds.length,
    evidenceDigest,
    rewardRuleDigest,
