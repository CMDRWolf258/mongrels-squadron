...
function resolveJobForEvent(catalogRow,event){
  ...
  // A Colonization Job may deliberately be backdated so already-completed
  // hauling can count. When the event predates the first publication, the
  // latest approved job definition is the authoritative retroactive rule.
  // Provenance still points to the exact archived revision that granted it.
  if(index<0&&revisions.length){
    let latestIndex=0;
    ...
    const fundingMode=['none','member','squad'].includes(latest?.job?.fundingMode)?latest.job.fundingMode:'squad';
    const fundingApprovalStatus=['not_required','pending','approved','rejected'].includes(latest?.job?.fundingApprovalStatus)
      ? latest.job.fundingApprovalStatus
      : 'approved';
    const rewardAllowed=fundingMode!=='squad'||fundingApprovalStatus==='approved';
    if(rewardAllowed&&Number.isFinite(start)&&time>=start){
      index=latestIndex;
