import {
  ...
  discordMissionControlWebhookId,
  discordOperationsWebhookId,
  editMissionControlDiscordMessage,
...
export async function syncDailyOrdersDiscord(env,{
  ...
  if(!discordMissionControlConfigured(env)){
  ...
  const webhookId=discordMissionControlWebhookId(env);
  ...
  try{
    ...
    if(current?.messageId&&current?.webhookId===webhookId){
      try{
        sent=await editMissionControlDiscordMessage(env,current.messageId,payload);
        mode='edited';
      }catch(error){
        if(Number(error?.status)!==404)throw error;
        sent=await createMissionControlDiscordMessage(env,payload);
        mode='recreated';
      }
    }else{
      sent=await createMissionControlDiscordMessage(env,payload);
...
export async function clearDailyOrdersDiscord(env,{
  ...
  try{
    const sent=await editMissionControlDiscordMessage(
      env,
      current.messageId,
...
export function buildDailyOrdersDiscordPayload(document,{actor='Mongrel Officer',missionControlUrl=''}={}){
  ...
  for(const [system,systemOrders] of groups){
    ...
    const priority=systemPriorityLabel(systemOrders);
    fields.push({
      name:truncate(system+(priority?' · '+priority:''),80),
...
function systemPriorityLabel(orders){
  const priorities=[...new Set((Array.isArray(orders)?orders:[])
    .map(order=>clean(order?.priority).toUpperCase())
    .filter(Boolean))];
  if(priorities.length===1)return truncate(priorities[0],18);
  if(priorities.length>1)return'MIXED';
  return'';
}
...
function orderDiscordLine(order){
  ...
  const prefix=kind?'**'+escapeMarkdown(kind)+'** ':'';
...
async function migrateLegacyOperationsTracking(env,state,newWebhookId){
  ...
  const current=state.current;
  if(!current?.messageId||current.webhookId!==oldOperationsWebhookId)return;

  try{await deleteOperationsDiscordMessage(env,current.messageId);}
  catch(error){
    if(Number(error?.status)!==404){
      console.error('Could not remove legacy Daily Orders message from System Testing Discord',error);
      throw error;
    }
  }
  state.current=null;
