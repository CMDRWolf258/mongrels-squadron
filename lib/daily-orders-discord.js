...
export async function syncDailyOrdersDiscord(env,{
  ...
  try{
    let sent;
    let mode;
    if(current?.cycleId===cycleId&&current?.messageId&&current?.webhookId===webhookId){
      try{
...
export async function clearDailyOrdersDiscord(env,{
  ...
  const webhookId=discordOperationsWebhookId(env);
  if(!current?.messageId||!current?.cycleId||current.webhookId!==webhookId||current.cycleId!==previousCycleId){
...
export function buildDailyOrdersDiscordPayload(document,{actor='Mongrel Officer',missionControlUrl=''}={}){
  ...
  for(const order of orders){
    const system=truncate(clean(order?.system)||'Squad-wide',64);
  ...
  return {
    ...
    embeds:[{
      title:truncate(clean(document?.title)||'Squadron Daily Orders',256),
      ...(link?{url:link}:{}),
...
function orderDiscordLine(order){
  ...
  const task=truncate(clean(order?.task)||'Operational task',108);
  const prefix=label?'**'+escapeMarkdown(label)+'** ':'';
  return '• '+prefix+escapeMarkdown(task);
}
