...
export async function syncScoutDiscordBoard(env,{
  ...
  if(!discordOperationsConfigured(env))return boardResult({configured:false});
...
export async function syncScoutSummaryDiscord(env,{
  ...
  if(!discordOperationsConfigured(env))return summaryResult({configured:false,attempted:false,ok:false,mode:'not_configured'});
...
export function buildScoutSummaryDiscordPayload(selection,{scoutBoardUrl='',setupUrl=''}={}){
  ...
  const fields=[
    {
      ...
      value:priority.length+' priority · '+ordinary.length+' nearest ordinary systems shown'
        +(Number(selection?.totalNeeds)>priority.length+ordinary.length?' · '+Number(selection.totalNeeds)+' total systems currently need scouting':''),
      inline:false,
    },
  ];

  const priorityLines=priority.map(job=>prioritySummaryLine(job,selection?.originCoords));
  if(priorityLines.length){
    for(const [index,chunk] of chunkLines(priorityLines,900).entries()){
      fields.push({
  ...
  const ordinaryLines=ordinary.map(job=>ordinarySummaryLine(job));
  fields.push({
    name:'Nearest Needs Scouting · '+clean(selection?.originSystem||DEFAULT_ORIGIN),
    value:ordinaryLines.length
      ? ordinaryLines.join('\n')
      : 'No ordinary Scout Jobs currently need a fresh board.',
    inline:false,
  });
  ...
  return {
    ...
    embeds:[{
      ...
      description:[
        'Priority Scout Jobs plus up to '+Number(selection?.ordinaryLimit||DEFAULT_NEAREST_LIMIT)+' ordinary systems needing current data. Ordinary work is routed by straight-line distance from **'+escapeMarkdown(clean(selection?.originSystem||DEFAULT_ORIGIN))+'** whenever coordinates are available.',
        '**LIVE SCOUT REQUIRED:** Scout Job credit and rewards require Live Scout to be installed and linked. '+(setupLink?'[Setup directions →]('+setupLink+')':'Setup directions are available on the website.'),
        boardLink?'[Open Scout Board →]('+boardLink+')':'',
      ].filter(Boolean).join('\n\n'),
      color:COLORS.summary,
      fields:fields.slice(0,25),
      footer:{text:'Regiment of Imperial Mongrels · persistent Scout Operations summary'},
    }],
  };
}
...
export function buildPriorityScoutDiscordPayload(job,{
  ...
  return {
    ...
    embeds:[{
      ...
      description:[
        completed
          ? 'Priority scouting is complete for the current cycle. This card will leave the operations channel on the next Scout sync.'
      ...
      fields,
      footer:{text:'Regiment of Imperial Mongrels · Scout Operations'},
...
async function deleteTrackedPriorityCard(env,state,systemKey,tracked,reason){
  try{
    await deleteOperationsDiscordMessage(env,tracked.messageId);
    delete state.priorityCards[systemKey];
    await writeState(env,state);
...
function prioritySummaryLine(job,originCoords){
  const distance=distanceLy(originCoords,coordinates(job?.coords));
  const status=job?.status==='claimed'
    ? 'CLAIMED'+(job?.claim?.commander?' · '+escapeMarkdown(job.claim.commander):'')
    : 'AVAILABLE';
  const reward=Number(job?.reward?.totalMillions)>0?' · '+fmtCredits(job.reward.totalMillions):'';
  const reason=clean(job?.reward?.bonusReason)?' · '+escapeMarkdown(truncate(job.reward.bonusReason,80)):'';
  const dist=Number.isFinite(distance)?' · '+distance.toFixed(1)+' LY':'';
  return '• **'+escapeMarkdown(clean(job?.system)||'Unknown system')+'** — '+status+reward+dist+reason;
}
function ordinarySummaryLine(job){
  const distance=Number(job?.distanceFromOrigin);
  const dist=Number.isFinite(distance)?distance.toFixed(1)+' LY':'distance unavailable';
  const status=job?.status==='claimed'
    ? 'CLAIMED'+(job?.claim?.commander?' · '+escapeMarkdown(job.claim.commander):'')
    : 'AVAILABLE';
  return '• **'+escapeMarkdown(clean(job?.system)||'Unknown system')+'** — '+dist+' · '+status;
}
