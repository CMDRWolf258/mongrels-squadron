...
export function buildScoutSummaryDiscordPayload(selection,{scoutBoardUrl='',setupUrl=''}={}){
  ...
  const priorityNameWidth=priority.length
    ...
    : 6;
  const priorityLines=priority.flatMap(job=>prioritySummaryLines(job,priorityNameWidth));
  if(priorityLines.length){
    const header='SYSTEM'.padEnd(priorityNameWidth)+'  STATUS'.padEnd(12)+'  REWARD'.padStart(10);
...
function prioritySummaryLines(job,nameWidth=20){
  ...
  const reward=(Number(job?.reward?.totalMillions)>0?fmtCredits(job.reward.totalMillions):'—').padStart(10);
  const lines=[system+'  '+status+'  '+reward];
  const detail=[];
