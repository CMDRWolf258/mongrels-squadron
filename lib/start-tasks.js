...
export function getEligibleStartTasks(activity, experience = 'new', playStyle = 'either') {
  ...
  return pool.filter(item => {
    if (!Array.isArray(item.levels) || !item.levels.includes(experience)) return false;
    if (playStyle === 'solo' && item.style === 'group') return false;
    return true;
  }).map(item => ({ ...item, link:item.link || START_ACTIVITY_META[activity]?.link || '/activities/' }));
}
