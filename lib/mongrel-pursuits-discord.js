...
export async function syncMongrelPursuitsDiscord(env,{origin=''}={}){
  ...
  if(!resolved.channelId){
    return result(false,'mongrel_pursuits_channel_not_found',{
      ...
      error:'Could not find Discord channel 🐺〡mongrel-pursuit (plain mongrel-pursuit is also accepted).',
    });
  }
...
function pursuitsChannelName(value){
  ...
  if(name==='🐺〡mongrel-pursuit')return'decorated';
  if(name==='mongrel-pursuit')return'plain';
  return'';
}
...
function friendlyError(error){
  ...
  if(/Missing Access/i.test(message)||/"code"\s*:\s*50001/.test(message))return'The Imperial Mongrels Website bot cannot access 🐺〡mongrel-pursuit.';
