...
async function resolveMongrelPursuitsChannel(env){
  ...
  const match=decorated||plain;
  return{channelId:clean(match?.id),mode:decorated?'decorated':plain?'plain':'not_found'};
}
...
export async function ensureMongrelPursuitRoles(env,{createMissing=true}={}){
  ...
  try{
    for(const pursuit of MONGREL_PURSUITS){
      ...
      if(!role){
        const legacyNames=(pursuit.aliases||[]).map(alias=>'Pursuit · '+alias.toLowerCase());
        role=existing.find(item=>legacyNames.includes(clean(item?.name).toLowerCase()));
        if(role&&clean(role.name)!==name){
          try{
            const renamed=await discordRequest(env,'/guilds/'+encodeURIComponent(guildId)+'/roles/'+encodeURIComponent(role.id),{
              method:'PATCH',
              body:JSON.stringify({name}),
            });
            role=renamed||role;
          }catch(error){
            console.warn('Could not rename legacy Pursuit role',pursuit.id,friendlyError(error));
          }
        }
      }
...
export async function syncMongrelPursuitsDiscord(env,{origin=''}={}){
  ...
  try{
    ...
    return{
      ...
      channelMode:resolved.mode,roleIds:ensured.roleIds,rolesCreated:ensured.created,
...
export function buildMongrelPursuitsDiscordPayload({origin=''}={}){
  ...
  const embed={
    ...
    description:'Choose the Elite activities you enjoy, specialize in, or want to join other Mongrels for. These are **interests, not obligations or squad rank**.\n\nUse **Manage My Pursuits** to update your selections anytime.',
    color:0x22d3ee,
    fields,
...
function friendlyError(error){
  ...
  if(/Missing Permissions/i.test(message)||/"code"\s*:\s*50013/.test(message))return'The Imperial Mongrels Website bot needs View Channel, Send Messages, Embed Links, Read Message History, and Manage Roles. The bot role must sit above Pursuit roles.';
