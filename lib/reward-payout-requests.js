...
export async function requestRewardPayout(env,{
  ...
  const owed=(Array.isArray(entries)?entries:[])
    .filter(entry=>clean(entry?.ownerId)===owner&&entry?.fundingMode!=='member'&&entry?.status==='owed'&&(Number(entry?.amountCredits)||0)>0)
...
export async function reconcileRewardPayoutRequest(env,{
  ...
  const owedIds=new Set(
    (Array.isArray(entries)?entries:[])
      .filter(entry=>clean(entry?.ownerId)===owner&&entry?.fundingMode!=='member'&&entry?.status==='owed')
...
export function rewardPayoutRequestView(record,entries=[]){
  ...
  const owed=rows.filter(entry=>entry?.fundingMode!=='member'&&entry?.status==='owed');
