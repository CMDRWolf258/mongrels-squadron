...
export function colonizationJobPreview(job, events) {
  ...
  for(const event of Array.isArray(events)?events:[]) {
    if(!matchesJob(event,job))continue;
...
function matchesJob(event,job) {
  if(event?.type!=='colonization_contribution')return false;
  if(norm(event.system)!==norm(job.system))return false;
  ...
  if(job.scope==='market' && String(event.marketId||'')!==String(job.marketId||''))return false;
  if(job.commodity) {
    ...
    event={...event,totalTons:filtered.reduce((sum,item)=>sum+(Number(item.amount)||0),0),contributions:filtered};
  }
  return true;
}
