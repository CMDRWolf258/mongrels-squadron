...
export async function provisionAcceptedApplicant(env, application, origin) {
  ...
  // Granting Member is the authoritative provisioning step. If this fails, the
  // application must not be marked Accepted.
  await discordRequest(env, `/guilds/${encodeURIComponent(guildId)}/members/${encodeURIComponent(userId)}/roles/${encodeURIComponent(memberRoleId)}`, {
    method: 'PUT',
  });

  // Applicant/Guest cleanup is intentionally best-effort after Member succeeds.
  // An already-provisioned member should not be rolled back just because a stale
  // onboarding role could not be removed.
  ...
  for (const roleId of [config.applicantRoleId, config.guestRoleId]) {
    ...
    try {
      await discordRequest(env, `/guilds/${encodeURIComponent(guildId)}/members/${encodeURIComponent(userId)}/roles/${encodeURIComponent(roleId)}`, {
        method: 'DELETE',
      });
    } catch (error) {
  ...
  let dm;
  try {
    dm = await sendAcceptanceWelcomeOnce(env, application, origin);
  } catch (error) {
    // Member provisioning already succeeded. A DM/KV failure must not turn a
    // successfully admitted member back into an unaccepted application.
    dm = { status: 'failed', error: String(error?.message || error).slice(0, 500) };
  }
...
export async function sendAcceptanceWelcomeOnce(env, application, origin) {
  ...
  await env.PROJECTS.put(key, JSON.stringify({
    status: 'pending',
    pendingAt: new Date(now).toISOString(),
    pendingUntil: now + ACCEPTANCE_DM_PENDING_MS,
  }));
    ...
    await discordRequest(env, `/channels/${encodeURIComponent(dm.id)}/messages`, {
      ...
      body: JSON.stringify({
        embeds: [{
          ...
          description: `Your application has been **accepted**, CMDR ${commanderName}. Your Mongrel Member role has been granted.`,
          fields: [
            {
              name: 'Activate Website Member Access',
              value: 'Use the button below once to sign in again with Discord. This refreshes your website session so the Member Portal and private squad tools recognize your new role.',
            },
            {
              name: 'In-Game Squadron Membership',
              value: 'The Elite Dangerous Squadron application is accepted separately in-game by leadership. If your in-game membership has not updated yet, allow leadership a little time or contact an officer.',
            },
            {
              name: 'Welcome Aboard',
              value: 'You now have access to the Mongrels member network, current tasking, projects, coordination tools, and member resources.',
            },
          ],
