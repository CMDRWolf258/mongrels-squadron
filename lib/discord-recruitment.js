...
export async function sendAcceptanceTestDm(env, userId, origin, commanderName = 'Wolf258') {
  ...
  const message = await sendAcceptanceDm(
    env,
    target,
    origin,
    clean(commanderName, 'Wolf258', 80).replace(/^CMDR\s+/i, ''),
    true,
  );
    ...
    body: JSON.stringify({
      ...
      embeds: [{
        ...
        fields: [
          ...
          {
            name: 'Final In-Game Step — Confirm Your Acceptance',
            value: 'Elite\'s updated Squadron join flow is two-stage. After Mongrel leadership approves your in-game Squadron application, **you must return to the Squadrons panel and confirm the acceptance / choose Join Squadron** before you are actually added to the in-game squad. Watch your in-game Comms Inbox for the approval notice, then open **Right-hand Panel → Squadrons** to finish joining.',
          },
          {
            name: 'Welcome Aboard',
