...
export async function sendRecruitmentDeclineNotice(env, application, origin) {
  ...
  await env.PROJECTS.put(key, JSON.stringify({
    status: 'pending',
    pendingAt: new Date(now).toISOString(),
    pendingUntil: now + DECLINE_DM_PENDING_MS,
  }));

  const base = origin.replace(/\/$/, '');
  const statusUrl = `${base}/apply/`;
  const commanderName = clean(application?.answers?.commanderName, 'Commander', 80).replace(/^CMDR\s+/i, '');
  const leadershipMessage = clean(
    application?.applicantMessage,
    'Thank you for taking the time to apply. Your application was not accepted at this time. If leadership invited you to reapply later or you would like clarification, please contact us in Discord.',
    1000,
  );
  ...
  try {
    ...
    const message = await discordRequest(env, `/channels/${encodeURIComponent(dm.id)}/messages`, {
      ...
      body: JSON.stringify({
        ...
        components: [{
          type: 1,
          components: [{ type: 2, style: 5, label: 'View Application Status', url: statusUrl, emoji: { name: '📋' } }],
        }],
      }),
    });
    const sentAt = new Date().toISOString();
