...
assert.match(html,/class="trade-market-disclosure"/,'Commodity Search should default to collapsed');
assert.doesNotMatch(html,/class="trade-market-disclosure"[^>]*\sopen(?:\s|>)/,'Commodity Search disclosure should not default open');
...
assert.match(client,/radiusField\.hidden=rareSource/);
assert.match(client,/rareSourceIndicator\.hidden=!rareSource/);
assert.match(html,/Unique rare source · all distances/);
...
assert.match(headersFile,/\/js\/trade-cg\.js/);
assert.match(headersFile,/\/css\/trade-loops\.css/);
assert.match(headersFile,/\/css\/trade-cg\.css/);
