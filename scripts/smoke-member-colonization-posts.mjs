...
assert.match(tradePage,/trading-colonization\.js\?v=8/);
assert.match(tradePage,/trading-colonization\.css\?v=3/);
...
assert.match(colonyUi,/data-colony-link-site/,'Awaiting member jobs must expose a verified-site linking control');
assert.match(colonyUi,/action:'link-site'/,'Member site linking must use a dedicated API action');
assert.match(colonyUi,/Dock at the intended construction depot/,'Awaiting jobs must explain how to discover the site');
assert.match(colonyUi,/payload\?\.observedMarkets/,'Site choices must come from verified observed Frontier markets');
...
assert.match(memberApi,/action==='link-site'/,'Member API must support explicit verified-site linking');
assert.match(memberApi,/existing\.scope!=='market'/,'Only site-specific jobs may be linked');
assert.match(memberApi,/existing\.marketId&&!manager/,'Regular members must not relink an already-bound job');
assert.match(memberApi,/normSystem\(row\.system\)===normSystem\(existing\.system\)/,'Verified site must belong to the same system as the job');
assert.match(memberApi,/colonization_site_not_verified/,'Unverified MarketIDs must be rejected');
assert.match(memberApi,/observedMarkets:buildObservedMarkets\(memberRows\)/,'Member board must receive verified site choices');
assert.match(memberApi,/syncColonizationMutationDiscord/,'Member Colonization mutations must update Discord immediately');
assert.match(memberApi,/action:'create',[\s\S]*job,[\s\S]*actor/,'New member jobs must seed Colonization Discord immediately');
assert.match(memberApi,/action:action==='link-site'\?'create':'update'/,'Linking a site must immediately create or update the Discord card');
