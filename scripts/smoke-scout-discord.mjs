...
const priorityRewardColumns=priorityTableLines.map(line=>line.indexOf('M Cr')).filter(index=>index>=0);
assert.ok(priorityRewardColumns.length>1,'Expected reward values in priority rows');
assert.equal(new Set(priorityRewardColumns).size,1,'Priority reward values must align to the same right-side column');
assert.doesNotMatch(priorityField.value,/\d+\.\d+ LY/,'Priority Scout summary should not spend space on distance');
