...
const priorityRewardRows=priorityTableLines.filter(line=>/M Cr$/.test(line));
assert.ok(priorityRewardRows.length>1,'Expected reward values in priority rows');
assert.ok(priorityRewardRows.every(line=>/\d+(?:\.\d+)?M Cr$/.test(line)),'Priority reward must be the rightmost value');
