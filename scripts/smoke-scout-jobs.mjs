...
assert.match(page,/scout-jobs\.js\?v=6/);
...
assert.match(scoutPage,/scout-jobs\.js\?v=6/);
...
assert.match(scoutUi,/routePreferencesLoaded/);
assert.match(scoutUi,/localStorage\.removeItem/);
assert.match(scoutUi,/useLast\.textContent='Use Last Scout'/);
assert.match(scoutUi,/refreshInFlight/);
assert.match(scoutUi,/consecutiveRefreshFailures/);
assert.match(scoutUi,/if\(payload\)\{/,'a failed refresh must preserve the last good Scout Board');
assert.match(scoutUi,/Live refresh delayed · showing last good board/);
assert.match(scoutUi,/scheduleAutoRefresh/);
assert.doesNotMatch(scoutUi,/setInterval\(/,'Scout Board polling must not use overlapping setInterval refreshes');
