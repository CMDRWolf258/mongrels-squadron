...
const DEFAULT_SPECIALTIES = ['BGS','Combat','PvP','AX','Mining','Exploration','Exobiology','Trade','Colonization','Carriers','Powerplay','Engineering'];
...
export async function onRequestGet({ request, env }) {
  ...
  const visible = profiles
    ...
    .sort(profileSort);
  return reply({ok:true,viewer:viewer(auth.session),canModerate,specialties:DEFAULT_SPECIALTIES,mine:mineRaw?present(mineRaw,auth.session,contributions.get(mineRaw.ownerId)):null,profiles:visible});
}
