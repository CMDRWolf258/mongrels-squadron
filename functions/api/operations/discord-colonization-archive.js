...
import { discordColonizationArchiveConfigured } from '../../../lib/discord-webhook.js';
import { syncAllCompletedColonizationArchiveDiscord } from '../../../lib/colonization-discord.js';
...
export async function onRequestPost({request,env}){
  ...
  if(!discordColonizationArchiveConfigured(env)){
    return reply({ok:false,error:'discord_colonization_archive_webhook_not_configured'},503);
  }

  try{
    const discord=await syncAllCompletedColonizationArchiveDiscord(env,{
    ...
    console.error('Manual Colonization Archive Discord sync failed',error);
    return reply({ok:false,error:'discord_colonization_archive_sync_failed'},502);
  }
}
