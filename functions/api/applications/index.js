...
import { provisionAcceptedApplicant, sendReapplicationOpenedNotice, sendRecruitmentDeclineNotice, sendRecruitmentSubmissionAlert } from '../../../lib/discord-recruitment.js';
...
export async function onRequestGet({ request, env }) {
  ...
  if (!canReview) {
    return reply({ ok:true, viewer:viewer(auth.session), canReview:false, alreadyMember:MEMBER_ACCESS.has(auth.session.access), mine:mine ? presentApplicant(itemForApplicant(mine)) : null });
  }
  return reply({
    ok:true,
...
export async function onRequestPost({ request, env }) {
  ...
  const item = {
    ...
    reapplicationDmStatus:existing?.reapplicationDmStatus || '',
  ...
  if (action === 'submit') {
    try { await sendRecruitmentSubmissionAlert(env, item, new URL(request.url).origin); }
    catch { /* valid submission must not fail because Discord notification is unavailable */ }
  }
...
export async function onRequestPut({ request, env }) {
  ...
  if (body.value?.action === 'allow_reapplication') {
    ...
    let reapplicationDmStatus = 'unknown';
    try {
      const notice = await sendReapplicationOpenedNotice(env, reopened, new URL(request.url).origin);
      reapplicationDmStatus = notice?.status || 'unknown';
    } catch { reapplicationDmStatus = 'failed'; }
    reopened = { ...reopened, reapplicationDmStatus };
    items[index] = reopened;
    await writeApplications(env, items);
    return reply({ ok:true, reapplicationOpened:true, notificationStatus:reapplicationDmStatus, application:presentReviewer(reopened) });
  }
  ...
  const requestedVerification = typeof body.value?.inGameApplicationVerified === 'boolean' ? body.value.inGameApplicationVerified : Boolean(existing.inGameApplicationVerified);
  ...
  if (newlyAccepted && !inGameApplicationVerified && !overrideInGameRequirement) {
    return reply({ ok:false, error:'ingame_application_not_verified', detail:'Verify the Commander has submitted an in-game Squadron application, or explicitly approve as an exception.' },409);
  }

  if (newlyAccepted) {
    try { provisioning = await provisionAcceptedApplicant(env, existing, new URL(request.url).origin); }
    catch (error) { return reply({ ok:false, error:'member_role_assignment_failed', detail:String(error?.message || error).slice(0,700) },502); }
  }

  const requirementOverriddenNow = newlyAccepted && !inGameApplicationVerified && overrideInGameRequirement;
  const applicantMessage = newlyDeclined ? (clean(body.value?.applicantMessage, '', 1000) || DEFAULT_DECLINE_MESSAGE) : (existing.applicantMessage || '');
...
function itemForApplicant(item) {
  const { officerNotes, reviewedBy, acceptedBy, declinedBy, discordProvisionedAt, acceptanceDmStatus, declineDmStatus, reapplicationDmStatus, inGameApplicationVerified, inGameApplicationVerifiedAt, inGameApplicationVerifiedBy, inGameRequirementOverridden, inGameRequirementOverriddenAt, inGameRequirementOverriddenBy, decisionHistory, reapplicationAllowedBy, ...safe } = item;
  return safe;
}
...
async function readApplications(env) {
  ...
  try { const value = await env.PROJECTS.get(STORAGE_KEY, { type:'json' }); return Array.isArray(value) ? value : []; }
  catch { return []; }
}
async function writeApplications(env,items) { await env.PROJECTS.put(STORAGE_KEY, JSON.stringify(items.slice(-500))); }
function requireStorage(env) { return (!env.PROJECTS || typeof env.PROJECTS.put !== 'function') ? reply({ ok:false, error:'application_storage_not_configured' },503) : null; }
...
function allowedList(value,choices,fallback) { const list=Array.isArray(value)?value:fallback; const set=new Set(choices); return [...new Set(list.filter(item=>set.has(item)))].slice(0,choices.length); }
