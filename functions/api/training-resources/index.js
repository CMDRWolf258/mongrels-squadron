...
import {
  readTrainingResourcesDiscordState,
  syncTrainingResourcesDiscord,
  trainingResourcesDiscordStatus,
...
export async function onRequestGet({request,env}){
  ...
  return reply({
    ok:true,
    canManage,
    discord:canManage?trainingResourcesDiscordStatus(state,env):null,
  });
}
...
export async function onRequestPost({request,env}){
  ...
  if(!env?.PROJECTS||typeof env.PROJECTS.put!=='function')return reply({ok:false,error:'training_resources_storage_not_configured'},503);
  ...
  const discord=await syncTrainingResourcesDiscord(env,{origin:new URL(request.url).origin});
  const state=await readTrainingResourcesDiscordState(env);
  return reply({
    ok:Boolean(discord.ok),
    discord:trainingResourcesDiscordStatus(state,env),
    sync:discord,
  },discord.ok?200:502);
}
...
function validateSameOrigin(request){
  ...
  if(origin!==new URL(request.url).origin||request.headers.get('X-Mongrels-Request')!=='training-resources-admin'){
