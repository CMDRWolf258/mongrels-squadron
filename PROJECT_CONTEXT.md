...
- Trade Control presents **Refresh / Fresh through / Aging through** as paired **hours + minutes** inputs while the API/KV model remains total minutes. A minutes entry below 60 combines normally with the hours field (for example, 1 hr + 30 min = 1 hr 30 min). A minutes entry of 60 or more is treated as the duration being entered in that minutes box and **replaces rather than adds to the existing hours** when normalized (for example, 90 min → 1 hr 30 min and 120 min → 2 hr 0 min even if the hours box previously contained another value). On change/save, profile bounds are normalized to the existing backend limits, including the 5-minute refresh floor.
...
- the interactive search primary is **Spansh station search** through `lib/trade-market.js` and `/api/trade-market/search`. Ordinary commodities use one station-index query with reference system, radius, commodity market-side filters, supply/demand, price, station/carrier types, and an exact market-update timestamp range. Known unique rare-source buys may instead query their fixed source system without radius/age/commodity-index filtering and inspect the source station's returned market locally. All paths normalize returned station-market rows into the Mongrel observation model.
...
- Trader’s Outpost distance presentation is normalized to **at most 2 decimal places in light-years** across live search results, posted trade cards, saved Watch Current Best/Fallback Markets, and Discord Watch cards. Large market threshold inputs (price and supply/demand in live search + Watch editor) use comma-grouped whole-number entry such as `5,000,000` while preserving plain numeric values in API/storage.
...
- classic unique-source **rare trade commodities** are classified by `lib/trade-rares.js`. When the user is looking to **buy** one of these rares, the reference system is retained only for distance calculation and neither the radius nor market-age cutoff may hide the unique source. The Spansh request omits both distance and `market_updated_at` filters for rare-source buys, and local filtering keeps an older source eligible while still classifying/displaying its true Fresh/Aging/Stale age. Selling a rare still respects radius and freshness because that is a destination-market search. The UI disables the radius field and explains the all-distance behavior for rare-source searches.
- rare commodity aliases are resolved at both the outbound Spansh filter and inbound market-row matcher. In particular, **Soontill Relics / Soontil Relics** are treated as the same commodity, share one cache key, and both spellings are included in the Spansh search vocabulary so a source cannot disappear because one provider uses the alternate spelling.
- known rare production sources may use a deterministic source-system lookup instead of relying on Spansh's galaxy-wide commodity index. **Soontill Relics** is pinned to **Cheranovsky City · Ngurii**: rare-source searches ask Spansh for market stations in Ngurii, then inspect Cheranovsky City's returned market locally for Soontill/Soontil Relics and apply the user's price/supply criteria. The known source is also stored on saved Watch queries so Trade Control can show the location even when there is no current qualifying market observation.
- if Spansh omits a known rare source station entirely, the backend may use the last matching observation from the Mongrel rare-source cache, including legacy alias cache keys, with an explicit warning. This fallback is intentionally labeled cached/stale rather than presented as a fresh live observation.
- rare-source cache fallback also applies when Spansh returns the known source station but **omits the rare commodity row** on that refresh. A normalized live commodity row remains authoritative: if Spansh explicitly reports the rare with supply below the user's minimum (including supply 0), stale cached stock is **not** substituted. This prevents upstream row omissions from making a source flicker while still respecting explicit fresh no-stock data.
- the browser uses the same Soontill/Soontil alias identity for rare UI behavior, so either spelling disables the irrelevant radius control. Trader's Outpost HTML is served with no-cache/no-store headers on Cloudflare Pages to reduce cross-device stale-client drift; JS/CSS assets remain version-busted in the page.
...
- each successful evaluation stores last attempt/success, next nominal due time, match count, the **top 5 ranked qualifying markets**, current preferred market, source/partial state, and a meaningful transition baseline. Ranking follows the Watch’s saved preference (price / distance / freshness / volume). Recorded transition types are **baseline**, **condition_met**, **condition_cleared**, and **best_market_changed**. A failed lookup records an error but preserves the previous successful market shortlist.
- **ranked replacement-market promotion is enabled:** if rank #1 stops qualifying but another shortlisted/result market remains, the next result automatically becomes `currentBest`, the Watch/Discord cards update their fallback list, and the existing `best_market_changed` transition can alert subscribers. Website and Discord Watch cards both show the full saved fallback set #2–#5.
- **first-pass BGS enrichment is enabled for Spansh market results.** The station search already carries station controlling faction, controlling-faction state, station/system economy fields, and station metadata update time, so these are normalized into each market observation without extra per-result API calls. Live result cards, saved Watch Current Best/fallbacks, and Watch Discord cards can display that context.
- BGS/ownership freshness is deliberately tracked separately from market freshness. `market_updated_at` answers “when was price/stock seen?” while station `updated_at` is treated as the best currently available proxy for “when was controller/state/station metadata seen?”. This is not treated as proof that ownership itself changed at that timestamp.
- **Infrastructure Failure is a first-class low-price trade signal for the entire `Metals` commodity category**, not only Gold. Gold, Silver, Palladium, Cobalt, and any other Spansh market entry categorized as `Metals` receive the same signal when the Watch/search direction is **buy from station** and the station controlling faction is in Infrastructure Failure.
- Infrastructure Failure metal signals always keep the real supply and buy price visible rather than applying a hidden stock threshold; the user's Min Supply criterion determines what quantity is meaningful for that search/Watch.
- ownership-change blind spot policy: if an Infrastructure Failure metal opportunity is detected but station controller/BGS metadata is missing, older than 24 hours, or lags the market observation by more than 24 hours, the result is marked **Ownership needs confirmation**. We do not silently assume a stale controller still owns the port. MongrelScout is the planned first-party source for closing this blind spot with newer station-controller observations.
...
- saved Watch cards show last check, next due, match count, current best market, ranked fallback markets, errors/warnings, and last meaningful transition; Officer **Run Now** forces an immediate one-watch evaluation for validation.
- Discord Watch cards show the full saved fallback set **#2–#5** beneath Current Best when all five ranked markets exist, matching the website’s top-5 Watch intelligence.
...
- Officers/Site Admins have a **Test Alert** action on active Discord-enabled Watches. It requires confirmation, uses the exact same silent-channel + subscriber-DM delivery helpers as real alerts, and is unmistakably labeled **TEST ALERT**. It deliberately does **not** mutate evaluator state, baseline, match count, current-best market, last-transition, or normal alert history.
