...
- Officer Trade Control lists saved watches and supports **Edit**, **Load Search**, **Pause/Resume**, and **Remove**. Edit updates the existing watch in place instead of creating a duplicate; query changes reset evaluation state/current-best data to **Pending Scheduler** so future automation starts cleanly from the new criteria.
- **not yet enabled:** scheduled watch evaluator, automatic Discord watch-card creation from saved watches, trigger-state transitions, BGS enrichment, automatic best-source promotion, live threshold polling, or MongrelScout market writes. Do not label those pieces live until they are wired and production-validated.
